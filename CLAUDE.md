...
| 目錄 | 課數 | 主題 | summary 狀態 |
|------|------|------|------|
| `Claude C++面向對象/` | 35 課 | 面向對象基礎到移動語意 | 35/35 全部完成 ✅ |
...
| 目錄 | 課數 | 主題 | 狀態 |
|------|------|------|------|
| `Claude AI CUDA課程/` | 185 課 / 13 Parts | GPU 硬體、CUDA 程式模型、記憶體階層、平行模式、函式庫、除錯、效能優化、AI 推論引擎系統 | 進行中（0/185） |
...
- **⚠️ 本 repo 是「原始基準」**：CUDA 課同時存在於 `~/AI/AI_Course/AI_CUDA/Claude AI CUDA課程/`（實驗場，可編譯、可改、允許漂移）。
...
| C++11-26 | 15 | 15 | 100% |
| Python | 32 | 31 | 97% |
| **總計（C++/Python）** | **127** | **126** | **99%** |
| CUDA（新，另一套慣例） | 185 課 | — | 0%（尚未開課） |
