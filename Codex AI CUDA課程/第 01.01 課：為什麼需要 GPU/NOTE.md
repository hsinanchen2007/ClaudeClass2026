...
注意：真實 GPU 的 \(P\) 不能簡單等同 CUDA Core 數。記憶體、排程、occupancy、同步與資料依賴都會讓有效平行度低於理想模型。這兩條定律是思考工具，不是效能預測器。
...
`src/scaling_laws.cpp` 會列出不同串行比例與處理器數下的兩種理論加速比，也能接受自訂參數。它是純 C++，即使尚未完成 CUDA Toolkit 課程也能執行。
...
輸入錯誤（例如串行比例 `1.2`）會顯示原因並回傳 exit code 2，而不是輸出沒有意義的數字。
...
把 serial fraction 從 10% 降到 1%，對 Amdahl 上限的影響，遠比把 \(P\) 從 64 增加到 1024 更值得注意。優化加速器系統時，降低 serial/overhead 往往比盲目增加平行資源更有效。
...
CPU 端以 `-O3` 編譯，編譯器可以自動向量化；「單一 CPU thread」不等於刻意關閉 SIMD。每組取數次測量中的最短值，以降低背景干擾，但正式效能工程還應報 median、分位數、暖機策略與誤差。
...
-------------------------------------------------------------------------------------------------
        1024       0.000126       0.003968       0.016678      0.031754x      0.007555x      PASS
     1048576       0.202671       0.034880       1.386278      5.810522x      0.146198x      PASS
    16777216       7.154662       0.564896      21.941225     12.665450x      0.326083x      PASS
```

### 8.4 如何解讀

1. **1,024 元素**：工作太小。GPU kernel 本身約為 CPU 的 31.8%，也就是更慢；固定啟動成本已壓過有效計算。
2. **1,048,576 元素**：kernel-only 約快 5.81 倍，證明大量元素能利用 GPU 吞吐量。
3. **16,777,216 元素**：kernel-only 約快 12.67 倍，但完整 H2D + kernel + D2H 只有 CPU 速度的約 0.326 倍，仍較慢。
...
某工作 10% 不能平行，理想平行資源增加到 1,024。先手算，再用程式驗證：
...
2. **Amdahl 與 Gustafson 哪一個正確？**
   兩者都正確；前者固定問題大小，後者允許問題規模隨硬體擴張。
