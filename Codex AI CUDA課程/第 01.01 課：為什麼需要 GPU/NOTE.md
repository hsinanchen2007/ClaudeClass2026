...
本機課程環境已完成 CUDA 13.3，可用本 Codex 課程自帶工具選擇目前 GPU 架構：
...
本課另提供自足的 `verify.sh`，會編譯兩個範例、核對 Amdahl/Gustafson
數值與錯誤輸入退出碼，並在目前 GPU 執行三組 SAXPY：
...
`../tools/check_build.sh --lesson "第 01.01 課：為什麼需要 GPU"` 會先做
sm_75/sm_89/sm_90/sm_120 編譯稽核，再自動執行這支 verifier。要額外執行
CUDA memcheck，可使用 `CODEX_COMPUTE_SANITIZER=1 ./verify.sh`。
