...
> BUILD_GUIDE.md` 同步。完整工具位於本機 Codex 工作區根層 `tools/`，所以
...
| 工具 | 用途 | 是否保留產物 |
|---|---|---|
| `tools/check_build.sh` | 掃描全部課程或指定一課，稽核 `.cu`/`.cpp` 能否編譯 | 否；使用 `/tmp` 後自動清理 |
...
目前課程基準是 CUDA 13.3、GCC 15、C++20。本機 GPU 是 sm_75；sm_89 與
...
- 掃描 `Codex AI CUDA課程/第 * 課：*/src/` 第一層。
- 每個 `.cu` 分別以 `sm_75`、`sm_89`、`sm_120` 編譯。
- 每個 `.cpp` 以 `g++ -std=c++20 -O2` 編譯。
- 編譯產物放在短命暫存目錄，結束時自動刪除。
- 任一來源失敗即總結為失敗並回傳退出碼 1。
...
從該課目錄執行。預設建立包含 `sm_75 + sm_89 + sm_120` SASS 與
...
nvcc \
  ...
  -std=c++20 -O3 src/latency_throughput.cu \
...
~~~bash
g++ -std=c++20 -O2 src/scaling_laws.cpp -o build/scaling_laws
./build/scaling_laws
~~~
...
需要 CMake、額外 linker flags、cuBLAS、cuDNN、NCCL、PyTorch extension 或其他
函式庫的課程，不能假設通用單檔命令足夠。該課必須在 `NOTE.md` 提供可重現的
建置與執行命令；必要時再新增該課專用 build script。`check_build.sh` 的成功只
代表通用編譯稽核通過，不取代這些專用測試。
...
4. 在本機 repo 提交該課與根層 `PROGRESS.md`。
5. 在 GitHub repo 只提交該 Codex 課程，並以 Git CLI push/驗證遠端。
