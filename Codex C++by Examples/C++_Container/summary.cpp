...
// -----------------------------------------------------------------------------
// 【LeetCode 實戰範例】LeetCode 347. Top K Frequent Elements（前 K 個高頻元素）
// 題目：回傳出現頻率最高的 k 個值；例如 [1,1,1,2,2,3]、k=2 得 [1,2]，順序可任意。
// 為何使用本章主題：unordered_map 聚合頻率，大小 k 的 priority_queue 僅保留最高頻候選。
// 思路：先計數；逐 unique value 放入 min-heap；超過 k 就淘汰最低順位；取出並排序測試結果。
// 複雜度：平均時間 O(N+U log K+K log K)、額外空間 O(U+K)，U 為不同值數。
// 易錯點：k 不應超過 U；同頻 tie-break 不是題目要求；hash 順序未指定，測試需明確正規化。
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// 【日常實務範例】去重、聚合與排名的事件報表管線
// 情境：事件含 id、service、failed；報表要忽略重複 id、列各服務錯誤、最近三個 id 與最高錯誤服務。
// 為何使用本章主題：依需求組合 set/map/deque/heap；各容器各自維持唯一性、排序、窗口與排名契約。
// 設計：seen_ids 去重；errors 聚合失敗；deque 維持三筆窗口；轉 map 排序；heap 選最高錯誤服務。
// 成本：平均時間 O(E+S log S)、額外空間 O(E+S)，E 為事件數、S 為有錯誤的服務數。
// 上線注意：重複 id 的定義與保存期限要明確；同分排名目前依服務字串決定；跨容器更新需同步與錯誤處理。
// -----------------------------------------------------------------------------
struct Event {
    int id;
    std::string service;
