...
// 題目：輸出兩陣列共同元素並保留最小重複次數；例如 [1,2,2,1] 與 [2,2] 得 [2,2]。
// 為何使用本章主題：unordered_multiset 把 right 的每個重複值保留成一筆，命中後可精確消耗一份。
// 思路：由 right 建 remaining；掃描 left；find 命中就加入結果；erase 該 iterator 防止重複使用。
// 複雜度：平均時間 O(N+M)、最壞 O(N*M)，額外空間 O(M)，N/M 為兩陣列長度。
// 易錯點：erase(value) 會移除全部相等項；輸出順序跟隨 left 但題目不要求；hash 碰撞會讓最壞成本退化。
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// 【日常實務範例】按 SKU 件數管理可出貨庫存
// 情境：每個重複的 SKU 節點代表一件實體商品，收貨增加一件，出貨只消耗一件。
// 為何使用本章主題：unordered_multiset 直接表示 bag 語意，比 set 能保留重複件數，且平均快速查找 SKU。
// 設計：receive 插入一筆；ship_one 先 find；缺貨回 false；命中則 erase(iterator) 並回 true。
// 成本：單次操作平均 O(1)、最壞 O(N)，空間 O(N)，N 為庫存總件數。
// 上線注意：大量同 SKU 用 map<Sku,count> 更省節點；併發出貨需原子化，且失敗後不得產生負庫存或部分扣減。
// -----------------------------------------------------------------------------
class InventoryBag {
public:
