...
// 題目：找排序後第 k 大的值而非第 k 個不同值；例如 [3,2,1,5,6,4]、k=2 回傳 5。
// 為何使用本章主題：大小至多 k 的 min-heap 讓 top 始終是目前保留值中最小者，也就是第 k 大候選。
// 思路：逐值 push；heap 超過 k 就 pop 最小值；掃描結束回傳 top。
// 複雜度：時間 O(N log K)、額外空間 O(K)，N 為元素數、K 為排名參數。
// 易錯點：k 必須在 [1,N]；重複值照常計名次；空 heap 呼叫 top 是未定義行為，正式 API 不可只靠 assert。
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// 【日常實務範例】可重現的優先權工作排程
// 情境：派送工作時高 priority 先執行，同優先權則依較早 sequence 維持可重現順序。
// 為何使用本章主題：priority_queue 可 O(1) 讀最高順位、O(log J) 更新；複合 comparator 明確處理同分規則。
// 設計：Job 保存 priority、sequence、name；比較器先比優先權再反向比序號；每次從 top 派送。
// 成本：提交/移除 O(log J)、查看 O(1)、空間 O(J)，J 為待辦工作數。
// 上線注意：Compare 必須形成 strict weak ordering；入 heap 後不得修改排序欄位，sequence 也需防溢位與重複。
// -----------------------------------------------------------------------------
struct Job {
    int priority;
    int sequence;
