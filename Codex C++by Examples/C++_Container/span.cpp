...
// -----------------------------------------------------------------------------
// 【LeetCode 實戰範例】LeetCode 53. Maximum Subarray（最大子陣列和）
// 題目：找連續且非空子陣列的最大總和；例如 [-2,1,-3,4,-1,2,1,-5,4] 回傳 6。
// 為何使用本章主題：span 讓同一 Kadane 實作零拷貝接受 array、vector 或 C array 的連續資料。
// 思路：以首值初始化 ending_here/best；每步選擇另起子陣列或接續前段；更新全域最佳。
// 複雜度：時間 O(N)、額外空間 O(1)，N 為元素數。
// 易錯點：原題要求非空；本 API 對空輸入回 nullopt；中間和提升為 int64_t，span 來源須保持有效。
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// 【日常實務範例】零拷貝解析固定格式網路封包
// 情境：封包格式為 [version][type][payload...][checksum]，解析端要驗長度與 checksum 後提供 payload view。
// 為何使用本章主題：span 能切出 payload 子範圍而不複製 bytes，比回傳新 vector 降低配置與記憶體流量。
// 設計：先拒絕短封包；累加標頭與 payload 驗 checksum；以 subspan 建 payload；回傳 PacketView。
// 成本：驗證時間 O(B)、解析額外空間 O(1)，B 為封包 byte 數；payload 本身不複製。
// 上線注意：PacketView 不擁有來源，bytes 必須活得更久且位址不變；還需限制最大長度並區分錯誤原因。
// -----------------------------------------------------------------------------
struct PacketView {
    std::uint8_t version;
    std::uint8_t type;
