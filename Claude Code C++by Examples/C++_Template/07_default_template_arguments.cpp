...
// 【面試題】預設模板引數
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. 預設模板引數可以引用前面的模板參數嗎？
//     答：可以，而且這是 STL 容器的標準寫法：
//         template<typename T, typename Alloc = std::allocator<T>>。本檔的
//         Container = std::vector<T> 是同一招 ── 使用者只需填核心型別 T，
//         仍保留替換底層策略的彈性，這也是 policy-based design 的基礎手法。
//
// 🔥 Q2. 函式模板可以有預設模板引數嗎？從哪個標準開始？
//     答：C++11 起可以，C++03 不行。所以 template<typename T = int> T zero();
//         才能讓 zero() 完全不帶引數呼叫。注意類別模板的預設模板引數 C++98 就有了，
//         兩者時間點不同，是常見的追問點。
//
// Q3. 「有預設值的參數，右邊也必須都有預設值」這條規則，函式模板為何較寬鬆？
//     答：類別模板沒有引數推導，沒填的參數只能靠預設值補，所以必須從右往左連續。
//         函式模板可以從函式引數推導出後面的模板參數，因此中間帶預設值有時仍合法。
// ═══════════════════════════════════════════════════════════════════════════
