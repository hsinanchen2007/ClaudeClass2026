...
//     追問：左摺與右摺什麼時候結果不同？（對【整數】的 +、*、&&、|| 相同；
//         對 -、/、<< 這種非結合運算，順序會直接改變結果。⚠️ 浮點的 + 與 *
//         【不滿足結合律】——捨入誤差會讓左右摺不同，自訂 operator 更沒保證）
//
...
int main() {
    ...
    //    A：對【整數】的 +、*、&&、|| 結果相同；但對非結合運算
    //       （-、/、<<）順序會影響結果。例如 (... - args) 是 ((a-b)-c)，
    //       (args - ...) 則是 (a-(b-c))。實作前要分清楚。
    //       ⚠️ 陷阱：IEEE-754 浮點的 + 與 * 並【不】滿足結合律（每步都有
    //       捨入）。本機以 pack = {1.0, 1e100, -1e100} 實測：左摺得 0、
    //       右摺得 1。自訂 operator+ 更是完全沒有結合律保證。
    //       所以「+、* 可以隨便換順序」只對整數成立。
    //
