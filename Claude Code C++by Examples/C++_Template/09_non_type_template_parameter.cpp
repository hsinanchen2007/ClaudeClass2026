...
// 【面試題】非型別模板參數（NTTP）
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. 哪些型別可以當 NTTP？哪些不行？
//     答：整數與列舉型別、指標、左值參考、成員指標都可以。C++17 之前，浮點數、
//         字串字面值、一般 class type 都不行。C++20 才開放浮點數與 literal class
//         type（structural type），於是才有把字串字面量塞進模板參數的 fixed_string 做法。
//     追問：為什麼限制這麼嚴？（NTTP 是型別 identity 的一部分，須編譯期確定且可比較）
//
// 🔥 Q2. template <auto V> 是哪個標準？它解決什麼問題？
//     答：C++17。在它之前，要寫一個能同時接 int / char / enum 的常數包裝，必須為
//         每種型別各寫一份模板（template<int>、template<char>…）或改用巨集。
//         auto NTTP 讓編譯器自己推導 V 的型別，一份模板就搞定。
//
// Q3. pow_n 內有 for 迴圈，為什麼還能寫 constexpr auto p3 = pow_n<3>(2)？
//     答：靠 C++14 的 relaxed constexpr。C++11 的 constexpr 函式本體實質上只能是
//         單一 return，不能有區域變數或迴圈，同樣的計算得寫成遞迴。C++14 起放寬，
//         才寫得出這種自然的迴圈版本 ── 這是 C++14 不是 C++11。
// ═══════════════════════════════════════════════════════════════════════════
