...
// ═══════════════════════════════════════════════════════════════════════════
// 【面試題】Concepts 入門（C++20）
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. Concepts 是哪個標準？和 SFINAE / enable_if 差在哪？
//     答：C++20（本檔開頭的 #error 就卡 202002L）。concept 是「具名的編譯期布林
//         謂詞」，可直接拿來約束模板；約束失敗時編譯器會指出「哪一條不滿足」，
//         而 SFINAE 只丟一整串 substitution failure 讓你自己推。
//         另一個關鍵差異：concept 之間有偏序（subsumption），能自然分流重載。
//     追問：那四種等價的約束寫法分別長什麼樣？
//
// 🔥 Q2. 約束一個模板有哪幾種寫法？
//     答：四種，語意等價：(1) template<Numeric T>；(2) template<typename T>
//         requires Numeric<T>；(3) 寫在函式簽名尾的 trailing requires；
//         (4) 縮寫語法 auto f(Numeric auto a)，每個 auto 都是隱含的模板參數。
//     追問：縮寫語法的兩個 auto 參數會被推導成同一個型別嗎？
//         （不會，各自獨立——本檔 sum_two(1, 2.5) 就是 int 配 double）
//
// ⚠️ 陷阱. requires 子句是「編譯期的 if」嗎？條件不成立時會跳過那段程式碼嗎？
//     答：都不是。requires 只決定這個模板「要不要參與重載決議 / 能不能被實例化」，
//         不會產生任何 runtime 分支，也不會讓函式本體的某一段被丟棄。
//         要在函式「內部」依型別走不同分支，用的是 if constexpr（C++17）。
//     為什麼會錯：看到 requires 後面接一個布林運算式，就直接套用一般 if 的心智模型。
// ═══════════════════════════════════════════════════════════════════════════
