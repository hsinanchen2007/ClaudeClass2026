...
// 【面試題】自製容器：模板技巧的綜合應用
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. emplace_back 為什麼要寫成 variadic template + perfect forwarding？
//     答：template<typename... Args> T& emplace_back(Args&&... args) 讓呼叫端把
//         「建構子參數」原封不動（型別、const、左/右值性）轉發到容器記憶體裡就地
//         建構，省掉一個臨時物件的建構與移動。若少了 std::forward，具名的參數
//         全都是左值，move-only 型別會直接編譯失敗。
//     追問：什麼時候 emplace_back 和 push_back 其實沒差？
//         （已經有現成物件時，兩者都是一次拷貝或移動）
//
// 🔥 Q2. 為什麼 reserve / grow 要用 placement new 加上顯式 ~T()，不能直接賦值？
//     答：::operator new 只給了「原始記憶體」，上面還沒有任何 T 物件存在，對尚未建構
//         的儲存空間做賦值是 UB。所以要 ::new (p) T(...) 就地建構、用完再顯式呼叫
//         p->~T() 解構。這也是自製容器必須把「配置」與「建構」分開看的原因，
//         標準庫則是用 allocator 做同一件事。
//
// 🔥 Q3. 擴容搬移元素時，標準的 std::vector 為什麼不敢無條件用 move？
//     答：搬到一半若移動建構子拋例外，舊緩衝區已被破壞、無法回滾，就給不出 strong
//         exception guarantee。所以 vector 用 std::move_if_noexcept：只有元素的移動
//         建構子是 noexcept（或該型別根本不可拷貝）時才移動，否則退回拷貝。
//         本檔的 Vector / SmallVector 為求精簡是直接 move，這正是與標準容器的差距。
//
// ⚠️ 陷阱. SmallVector<int, 8> 與 SmallVector<int, 16> 是同一個型別嗎？
//     答：不是。N 是非型別模板參數，會參與型別身分——兩者不能互相賦值、不能傳給同一
//         個非模板函式，而且各自生成一整份程式碼，是 code bloat 的典型來源。
//     為什麼會錯：把 N 當成「建構時傳進去的容量」；它其實跟 T 一樣是型別的一部分。
// ═══════════════════════════════════════════════════════════════════════════
