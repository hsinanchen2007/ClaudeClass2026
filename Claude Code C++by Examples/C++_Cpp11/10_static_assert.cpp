...
// ═══════════════════════════════════════════════════════════════════════════
// 【面試題】static_assert（C++11）
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. static_assert 是什麼？訊息字串可以省略嗎？
//     答：編譯期斷言，條件必須是常數表達式，為 false 就直接編譯失敗並印出訊息。
//         ⚠️ C++11 要求「必須」提供訊息字串；C++17 起才可省略（編譯器拿條件的
//         字面文字當訊息）。本機 g++ -pedantic-errors 實測：static_assert(cond);
//         在 -std=c++11 與 -std=c++14 皆報錯，-std=c++17 才通過。
//     追問：典型用途？（平台/ABI 假設如 sizeof(void*)==8、序列化結構大小、
//           編譯期計算結果驗證、template 的型別約束）
//
// 🔥 Q2. static_assert 與 <cassert> 的 assert 差在哪？
//     答：assert 是執行期檢查，失敗才終止程式，且 NDEBUG（release）下通常被整個
//         關掉；static_assert 在編譯期檢查，debug/release 一律生效，錯誤在編譯
//         當下就爆出來，根本不會產出有問題的執行檔。
//     追問：那該用哪個？（能在編譯期知道的假設一律用 static_assert；依賴執行期
//           輸入的前置條件才用 assert）
//
// ⚠️ 陷阱. 用 static_assert 給 template 加型別約束，等同 enable_if / concepts 嗎？
//     答：不等同。static_assert 是 hard error——它只能讓「已經被選中的」實例化
//         失敗，無法把該候選從重載決議中剔除，所以完全不參與 SFINAE。
//     為什麼會錯：兩者看起來都在「限制 T」，但作用層次不同：SFINAE / concepts
