...
// ═══════════════════════════════════════════════════════════════════════════
// 【面試題】raw string literal（C++11）
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. raw string 的語法是什麼？解決什麼問題？
//     答：R"delimiter(內容)delimiter"，內容中的跳脫序列一律不生效，\ 與 " 都按
//         字面看。專治正規表示式與 Windows 路徑的反斜線地獄：
//           "(\\d+)\\s*:\\s*(\\d+)"   →   R"((\d+)\s*:\s*(\d+))"
//         也可與編碼前綴組合：u8R"(...)"、LR"(...)"、uR"(...)"、UR"(...)"。
//     追問：裡面要放換行怎麼辦？（直接按 Enter，那個換行就成為內容的一部分，
//           所以 raw string 天生適合內嵌多行 SQL / JSON，本檔 Demo2 即是）
//
// ⚠️ 陷阱. R"(...)" 既然「什麼都不解析」，是不是任何內容都能原封不動放進去？
//     答：不是。結束標記是 )delimiter" 這個序列，所以用預設的空 delimiter 時，
//         內容只要出現 )" 就會提早把字串結束掉：
//           R"(he said )" and left)"   // ❌ error: missing terminating " character
//         解法是換一個內容裡不會出現的 delimiter：
//           R"tag(he said )" and left)tag"
//         delimiter 長度上限為 16 個字元，超過本機 g++ 會直接報 raw string
//         delimiter longer than 16 characters。（以上兩點皆本機實測）
//     為什麼會錯：把「不解析跳脫字元」理解成「沒有任何終止規則」。raw string 仍
//         然得有辦法判斷自己在哪裡結束，delimiter 存在的意義正是讓你挑一個不會
//         與內容相撞的結束標記。
// ═══════════════════════════════════════════════════════════════════════════
