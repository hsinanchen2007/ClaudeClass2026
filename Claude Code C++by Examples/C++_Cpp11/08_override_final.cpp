...
// 【面試題】override / final
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. override 與 final 各有什麼作用？
//     答：`override` 明確標示「我要覆寫基底的虛擬函數」，**簽章對不上就編譯錯**
//         （少寫 const、參數型別不同、回傳型別不相容都會被抓），杜絕「以為覆寫、
//         其實新建了一個函數」的無聲 bug。`final` 用在虛擬函數上禁止再被覆寫，
//         用在類別上禁止被繼承。
//     追問：final 對效能有幫助嗎？（**可能**有。編譯器確定該呼叫不會再分派到別處
//         時，可做去虛擬化並進一步 inline；但這是最佳化機會而非標準保證，實測
//         差距通常很小，主要價值仍在表達設計意圖）
//
// Q2. override 和 final 是保留字嗎？
//     答：不是，兩者都是 **contextual keyword**——只有出現在特定位置才有特殊意義，
//         其餘場合仍是普通識別字。所以 `int override = 10; int final = 2;` 是合法
//         程式碼（本機 g++ -pedantic-errors 實測可編譯），雖然絕對不建議這樣寫。
//         這是為了不破壞既有以此為變數名的舊程式碼。
//
// ⚠️ 陷阱. 基底 `virtual void f(int x = 10)`，子類別 `void f(int x = 99) override`。
//         那麼透過基底參考呼叫 `b.f()`，印出來的 x 是多少？
//     答：**10**，不是 99。本機實測輸出是「Derv::f x=10」——執行的是子類別的函數
//         本體（虛擬分派正確），但套用的卻是**基底**的預設引數。因為虛擬函數是
//         動態綁定，預設引數卻是**靜態綁定**：編譯器在呼叫點依「靜態型別」把預設值
//         填進去，執行期才決定跳到哪個實作。
//     為什麼會錯：大家理所當然地認為「override 幫我檢查過簽章了，所以行為一定一致」。
//         但 override 只檢查**簽章相符**，管不到預設引數的值——這個坑它完全抓不到。
//         正解：虛擬函數不要給預設引數；真的需要就在基底提供非虛擬的公開包裝函數，
//         由它決定預設值，再轉呼叫受保護的虛擬實作（NVI，non-virtual interface）。
// ═══════════════════════════════════════════════════════════════════════════
