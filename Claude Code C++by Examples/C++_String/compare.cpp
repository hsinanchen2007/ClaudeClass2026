...
// ═══════════════════════════════════════════════════════════════════════════
// 【面試題】std::string::compare
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. compare() 回傳什麼?
//     答：回傳 int,標準只保證「負數 / 0 / 正數」三種號誌,**不保證是
//         -1 / 0 / 1**。實作常直接把 char_traits::compare 的結果或兩者的
//         長度差傳回去,實際數值是實作細節。
//     追問：所以正確寫法是什麼?
//         → if (a.compare(b) < 0)、== 0、> 0,絕不可寫 == -1 或 == 1。
//
// 🔥 Q2. compare 是依什麼規則比的?
//     答：逐位元組的字典序 (lexicographical),透過
//         char_traits<char>::compare (memcmp 語意,以 unsigned char 比較),
//         **不受 locale 影響**,也不是人類語感的「字母序」。先比共同前綴,
//         前綴完全相同時,較短的那個較小。
//     追問：為什麼 "Z" < "a" 為真?
//         → ASCII 裡大寫字母的碼位排在小寫之前。
//
// ⚠️ 陷阱. if (a.compare(b) == -1) 為什麼有時測試會過、上線卻炸?
//     答：某些實作在某些輸入下確實回 -1,測試「碰巧」通過;但換編譯器、
//         換標準庫版本,或只是換一組長度不同的輸入,回傳值可能變成 -3、
//         -100,條件突然恆為 false,比較邏輯整段失效。
//     為什麼會錯：把 compare / strcmp 想成「回傳 -1/0/1 的三值函式」。
//         標準承諾的只有正負號,數值大小完全不可依賴。
// ═══════════════════════════════════════════════════════════════════════════
