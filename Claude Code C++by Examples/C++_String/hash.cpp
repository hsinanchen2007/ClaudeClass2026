...
// ═══════════════════════════════════════════════════════════════════════════
// 【面試題】std::hash<std::string>
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. std::string 可以直接當 unordered_map 的 key 嗎?
//     答：可以。標準庫自 C++11 就提供 std::hash<std::string> 特化,所以
//         unordered_map<std::string, V> 開箱即用。性質:相同內容必得相同
//         雜湊值;複雜度 O(n) —— 每次雜湊都要掃過整個字串,長 key 的成本
//         不可忽略。
//     追問：那自訂型別要當 key 得做什麼?
//         → 特化 std::hash 或傳入自訂 hasher,並提供 operator==。
//
// 🔥 Q2. 用 string_view 去查 unordered_map<std::string, V> 會發生什麼?
//     答：預設會先把 string_view 建構成一個臨時 std::string (可能 heap
//         配置) 才查,白白付出一次配置。C++20 起 std::hash<std::string_view>
//         與 std::hash<std::string> 對相同內容產生相同值,配合異質查找
//         (heterogeneous lookup:指定 transparent hash + std::equal_to<>)
//         就能免掉這個臨時物件。
//     追問：有序的 std::map 呢?
//         → 需要 transparent comparator std::less<>,map 自 C++14 就支援;
//           unordered_map 要到 C++20 才有。
//
// ⚠️ 陷阱. 「相同字串雜湊值一定相同,所以可以存進資料庫當指紋」——錯在哪?
//     答：「相同內容 → 相同雜湊值」只在**同一次執行、同一個標準庫實作與
//         版本**內成立。標準**不保證**跨程式執行、跨實作、跨版本穩定,所以
//         絕不可持久化到檔案、資料庫,或寫進網路協定。要指紋就自己指定
//         固定演算法 (SHA-256、xxHash 之類)。
//     為什麼會錯：把「決定性函式」誤讀成「跨環境恆定的常數」。標準只要求
//         單次程式內一致,實作有完全的自由改演算法甚至加隨機種子。
//     追問：雜湊碰撞攻擊?
//         → 標準庫的 hash 不是抗攻擊設計,對外開放的服務應改用帶隨機種子
//           的雜湊,避免被構造大量碰撞而退化成 O(n)。
// ═══════════════════════════════════════════════════════════════════════════
