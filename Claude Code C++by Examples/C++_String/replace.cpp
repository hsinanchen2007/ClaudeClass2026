...
// ═══════════════════════════════════════════════════════════════════════════
// 【面試題】std::string::replace
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. replace 和「先 erase 再 insert」有什麼差別?
//     答:結果相同,但 replace 只做一次 memmove(尾段直接搬到最終位置),
//         erase+insert 會搬兩次(先往前、再往後)。而且 replace 若中途拋
//         例外,字串維持原狀;erase+insert 失敗時會留下改到一半的狀態。
//     追問:那什麼時候還是得用 erase+insert?→ 幾乎沒有;需要「刪一段、
//           在別的位置插入」時才分開做。
//
// 🔥 Q2. 為什麼「新內容長度 == 被取代長度」時 replace 特別快?
//     答:此時新內容剛好填滿被移除的區間,size 不變、後段字元完全不用搬,
//         退化成單純對 [pos, pos+count) 做覆寫。長度不同時,後段要往前或
//         往後 memmove;變長且超過 capacity 時還會多一次 reallocation。
//     追問:複雜度怎麼寫?→ O(N + M),N 是需搬移的後段字元數、M 是新內容
//           長度;長度相等時退化為 O(M)。
//
// 🔥 Q3. replace 之後,舊的 iterator / pointer 還能用嗎?
//     答:不能假設可用。觸發 reallocation 時全部失效;即使沒有 realloc,
//         只要長度改變,pos 之後的 iterator 雖然位置仍合法,指到的字元
//         已經不是原本那個,語意上必須視為失效。
//
// ⚠️ 陷阱. 用 find + replace 做全域取代,為什麼會無窮迴圈?
//     答:替換後若把搜尋位置留在原處,而新內容本身又包含 needle
//         (例如把 "a" 換成 "aa"),下一次 find 立刻找到剛寫進去的那個 "a",
//         永遠前進不了。正解是每次替換後 p += to.size() 跳過新內容;
//         另外 from.empty() 必須先擋掉(空字串在任何位置都找得到)。
//     為什麼會錯:大家習慣寫 p++ 或 p = 0,前者在 to 包含 from 時仍會
//         卡住,後者則把演算法退化成 O(N²) 且一樣可能不終止。
// ═══════════════════════════════════════════════════════════════════════════
