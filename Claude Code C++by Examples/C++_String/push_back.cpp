...
// ═══════════════════════════════════════════════════════════════════════════
// 【面試題】std::string::push_back
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. 對一個空的 std::string 加入第一個字元,正確做法是什麼?
//     答:s.push_back('a')、s += 'a' 或 s.append(1, 'a') —— 這些都會讓
//         size() 增加。不能寫 std::string s; s[0] = 'a';:空字串時 s[0]
//         就是 s[s.size()],C++11 起「讀取」它合法(得到 '\0' 的引用),
//         但寫入非 '\0' 的值是 UB。若真要用索引賦值,得先 s.resize(1)。
//     追問:那 reserve(10) 之後可以 s[0]='a' 嗎?→ 不行。reserve 只動
//         capacity、不動 size(),有效索引範圍仍是 [0, size()],要 resize 才行。
//
// 🔥 Q2. 迴圈裡 push_back 一百萬次,總複雜度是 O(n) 還是 O(n²)?
//     答:O(n)。單次 push_back 是 amortized O(1) —— 容量不足時重新配置,
//         但實作採幾何成長(容量乘上一個大於 1 的倍數),所以 n 次操作的
//         總搬移量是等比級數、與 n 成正比,平均下來每次仍是常數。
//         (成長倍率是 implementation-defined、標準未規定;libstdc++ 約 2 倍。)
//     追問:那為什麼還要先 reserve?→ amortized O(1) 省的是漸進複雜度,
//         reserve 省的是常數項:已知最終長度時先 reserve 可讓全程只配置一次,
//         並避免中途 reallocation 使既有指標/iterator 失效。
//
// 🔥 Q3. push_back 的例外安全等級是什麼?
//     答:strong exception guarantee —— 只有重新配置失敗會丟 std::bad_alloc,
//         而一旦丟出,字串內容與長度維持原狀,不會留下半完成的狀態。
//     追問:那 iterator 呢?→ 若發生 reallocation,既有的 iterator、
//         reference 與 c_str()/data() 指標全部失效,這與有沒有丟例外無關。
// ═══════════════════════════════════════════════════════════════════════════
