...
// ═══════════════════════════════════════════════════════════════════════════
// 【面試題】std::string::rfind
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. rfind() 與 find_last_of() 差在哪?
//     答：rfind 找的是「整個子字串」最後一次出現的位置(回傳子字串開頭索引);
//         find_last_of 找的是「字元集合」中任一字元的最後位置。
//         s.rfind("ab") 要求 a、b 連續相鄰;s.find_last_of("ab") 只要是 a 或 b 就算。
//     追問：rfind 回傳的是反向索引嗎?→ 不是,仍是從 0 起算的 forward 索引。
//
// 🔥 Q2. rfind 找不到時回傳什麼?
//     答：與 find 相同,回傳 std::string::npos(= static_cast<size_type>(-1),
//         64-bit 上為 18446744073709551615)。
//         必須寫 != std::string::npos,不可比 -1、0 或 >= 0(size_type 是無號)。
//
// ⚠️ 陷阱. rfind 的 pos 參數與 find 的 pos 意義一樣嗎?
//     答：不一樣。find(str, pos) 的 pos 是「開始往右搜尋的起點」;
//         rfind(str, pos) 的 pos 是「子字串允許的最大起始位置」——比對本身
//         可以延伸到 pos 之後。兩者預設值也不同(find 是 0,rfind 是 npos)。
//     為什麼會錯：把 rfind 想成「只在 [0, pos] 這段字元裡找」,
//         實際上限制的是起點,不是整個比對範圍。
// ═══════════════════════════════════════════════════════════════════════════
