...
// 【面試題】std::string::find
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. find() 找不到時回傳什麼?正確的判斷寫法是什麼?
//     答：回傳 std::string::npos,其定義是 static_cast<size_type>(-1),
//         在 64-bit 上等於 18446744073709551615(本機實測)。
//         唯一正確的判斷是 if (s.find(x) != std::string::npos)。
//     追問：s.find("") 回傳什麼?→ 0,空字串在任何位置都「找得到」。
//
// 🔥 Q2. 為什麼不可以把 find() 的回傳值存進 int?
//     答：回傳型別是 size_type(無號),存進 int 會被截斷成 -1。
//         看起來「好像對」,但型別已經錯了,後續比較與索引都不再可靠。
//         正確型別是 std::string::size_type,或直接用 auto。
//
// ⚠️ 陷阱. if (s.find(x) >= 0) 為什麼永遠成立?
//     答：find 回傳無號的 size_type,無號數恆 >= 0,所以這個條件恆真。
//         找不到時也會進入分支,再拿 npos(巨大值)當索引 → 越界 UB。
//     為什麼會錯：腦中把回傳值當成「找不到就是 -1」的有號整數,
//         但它其實是 unsigned;-1 只是它 cast 之後的位元樣貌。
// ═══════════════════════════════════════════════════════════════════════════
