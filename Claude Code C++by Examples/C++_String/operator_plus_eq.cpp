...
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. 為什麼 s = s + x 比 s += x 慢?
//     答:s = s + x 先建一個臨時 string 裝下 s+x 的全部內容(可能一次
//         heap 配置 + 完整複製),再把它賦值/搬回 s;s += x 是原地追加,
//         capacity 夠就只是 memcpy 尾巴。前者每次都是 O(size),
//         後者是攤還 O(1)。
//     追問:那 s = std::move(s) + x 呢?→ 仍會走 operator+ 產生新物件,
//           且對自身做 move 賦值語意很危險,不要這樣寫。
//
// 🔥 Q2. 迴圈裡 s += c 一百萬次,總複雜度是 O(n) 還是 O(n²)?
//     答:O(n)。string 的 buffer 是幾何成長,擴容次數只有 O(log n),
//         每次擴容搬移的總量加起來仍是 O(n) → 攤還 O(1) per append。
//         誤答 O(n²) 的人是把它想成「每次都重新配置並複製整條字串」。
//     追問:成長倍率是固定的嗎?→ libstdc++ 約 2 倍、MSVC 約 1.5 倍,
//           但這是 implementation-defined,標準只要求攤還 O(1)。
//
// 🔥 Q3. += 之後,先前取得的 iterator / c_str() 指標還能用嗎?
//     答:不能假設可用。只要 += 觸發 reallocation,所有 iterator、
//         pointer、reference 與 c_str()/data() 回傳的指標全部失效;
//         即使沒 reallocation,end() 也一定失效。
//     追問:怎麼避免?→ 事先 reserve 足夠 capacity,或每次操作後重新取得。
// ═══════════════════════════════════════════════════════════════════════════
