...
// 【面試題】std::string::back
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. back() 等同什麼?空字串上呼叫會怎樣?
//     答:等同 operator[](size() - 1),回傳 reference 所以可原地修改。
//         沒有邊界檢查:標準規定 empty() 為 true 時呼叫 back() 是 UB,
//         不丟例外、不回傳哨兵值。安全寫法是先 if (!s.empty())。
//     追問:有沒有「會檢查」的等價寫法?→ s.at(s.size() - 1),但空字串時
//         s.size() - 1 已經先 underflow 了(見下方陷阱),要先擋 empty()。
//
// 🔥 Q2. 判斷字串是否以某字元/子字串結尾,back() 是最好的寫法嗎?
//     答:單一字元用 back() 可以,但必須自己擋空字串。C++20 起
//         s.ends_with('x') / s.ends_with("txt") 更直觀,而且對空字串
//         安全地回傳 false,不需要額外檢查。要判斷「結尾子字串」時
//         back() 更是不夠用(只看得到一個字元)。
//     追問:C++20 之前呢?→ 自己比對:s.size() >= n && s.compare(s.size()-n,
//         n, suffix) == 0,注意先檢查長度以免 size()-n underflow。
//
// ⚠️ 陷阱. std::string s; 之後 s[s.size() - 1] 會發生什麼?
//     答:size_type 是無號型別(通常是 std::size_t),0 - 1 不是 -1,
//         而是 wrap around 成該型別的最大值(64-bit 下是
//         18446744073709551615)。於是這行變成用一個天文數字當索引 →
//         越界 UB,通常直接 segfault。
//     為什麼會錯:腦中以為 s.size() - 1 是「整數運算」,-1 頂多是個
//         負索引、大不了讀到奇怪的東西。實際上無號算術根本產生不出負數,
//         這也是為什麼 while (i >= 0) 配 size_type 會變成無窮迴圈的同源問題。
//         正確寫法:先 if (!s.empty()) 再取 back()。
// ═══════════════════════════════════════════════════════════════════════════
