...
// ═══════════════════════════════════════════════════════════════════════════
// 【面試題】resize_and_overwrite(C++23)
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. resize_and_overwrite() 解決了什麼問題?它是哪個標準版本?
//     答:C++23 加入。傳統 resize(n) 會把新增的位置填成 '\0',但在「先開空間、
//     再讓 read()/recv() 或 C API 把資料寫進來」的情境,那些零才剛寫好就被覆蓋,
//     純屬浪費。resize_and_overwrite(n, op) 直接把未初始化的 buffer 交給 op,
//     op 回報實際寫了多少字元,省掉這一次 memset。
//     追問:為什麼標準庫非得提供它、使用者做不到?→ 「跳過初始化」需要碰觸
//     string 的內部不變式,標準庫外沒有合法途徑做到。
//
// Q2. 傳給 op 的 callback 有哪些必須遵守的約定?
//     答:op 收到 buffer 指標與可寫的最大長度,必須回傳一個介於 0 到 count 之間
//     的值代表實際寫入長度;回傳值超出這個範圍是 UB。函式回傳後 string 會把
//     size() 設為該值,並保證 buffer[size()] == '\0'——結尾的 NUL 由 string
//     負責,op 不需要也不應該自己寫。
//     追問:op 裡可以修改那個 string 本身嗎?→ 不行,那是 UB。
//
// ⚠️ 陷阱. 呼叫前那 n 個位置的內容是什麼?
//     答:未指定(unspecified)。原有的 [0, size()) 字元會保留,但新擴出來的位置
//     沒有被初始化——讀取尚未寫入的位置是錯的,op 必須自己負責寫滿它回報的長度。
//     為什麼會錯:習慣了 resize 的填零語意,就會假設拿到的是一塊乾淨的零 buffer。
// ═══════════════════════════════════════════════════════════════════════════
