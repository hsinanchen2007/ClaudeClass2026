...
// 【面試題】std::string::at
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. operator[] 和 at() 的差別?
//     答:at() 做邊界檢查,pos >= size() 時丟 std::out_of_range;operator[]
//         不檢查,越界即 UB(唯一例外:s[s.size()] 自 C++11 起合法,回傳
//         結尾 null 字元的引用)。效能上 [] 較快(無分支),at() 較安全。
//         熱路徑用 [] 並自行保證範圍;索引來自不可信輸入時用 at()。
//     追問:為什麼 at() 不是預設?→ C++「不為你沒用到的東西付費」原則。
//
// 🔥 Q2. at() 丟的 std::out_of_range 屬於哪一類例外?
//     答:繼承自 std::logic_error(再往上是 std::exception),語意是
//         「程式邏輯錯誤,理應在寫程式階段避免」,而不是 runtime_error 那種
//         「執行期無法預期的外部失敗」。宣告在 <stdexcept>。
//     追問:那該 catch 還是修好索引?→ 索引來自程式內部就修邏輯;
//         來自外部輸入(檔案、網路、使用者)才適合 catch。
//
// ⚠️ 陷阱. std::string s; 之後 s.at(0) 與 s[0] 各會發生什麼?
//     答:s.at(0) 丟 std::out_of_range(安全地失敗、可被 catch);
//         s[0] 就是 s[s.size()],C++11 起「讀取」合法(得到 '\0' 的引用),
//         但把它「寫入」成非 '\0' 的值是 UB。
//         正確做法:s.resize(1)、s.push_back('a') 或 s += 'a'。
//     為什麼會錯:多數人以為 [] 和 at() 只差「會不會丟例外」,於是認為
//         s[0]='a' 頂多是「少了一次檢查」;實際上那是 UB,編譯器可以假設
//         它不會發生,後果不保證只是「寫壞一個 byte」。
// ═══════════════════════════════════════════════════════════════════════════
