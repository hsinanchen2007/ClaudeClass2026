...
// ═══════════════════════════════════════════════════════════════════════════
// 【面試題】std::string::find_last_not_of
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. find_last_not_of 的典型用途是什麼?
//     答：trim 尾端空白。s.find_last_not_of(" \t\n") 回傳最後一個非空白字元的位置,
//         再 s.substr(0, pos + 1) 就切掉尾端空白。
//     追問：為什麼是 pos + 1?→ substr 第二個參數是長度,要把 pos 這個字元本身含進去。
//
// 🔥 Q2. 怎麼用兩個函式組出完整的 trim?
//     答：b = s.find_first_not_of(ws); 若 b == std::string::npos 代表整串空白 → 回空字串;
//         否則 e = s.find_last_not_of(ws); return s.substr(b, e - b + 1);
//         先判 npos 再切,是標準慣用寫法。
//
// ⚠️ 陷阱. 省略 npos 檢查,行為取決於你怎麼用它。
//     答：整串都是空白時回傳 npos。寫 s.substr(0, npos + 1) 時,npos + 1 無號回繞成 0,
//         得到空字串(碰巧「看起來對」);但寫 s.substr(npos) 就會拋 std::out_of_range。
//     為什麼會錯：以為「沒找到反正就是空的」,
//         實際上同一個 npos 在不同用法下,一個安靜給錯值、一個直接拋例外。
// ═══════════════════════════════════════════════════════════════════════════
