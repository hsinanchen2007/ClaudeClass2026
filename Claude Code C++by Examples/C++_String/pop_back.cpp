...
// ═══════════════════════════════════════════════════════════════════════════
// 【面試題】std::string::pop_back
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. 對空字串呼叫 pop_back() 會發生什麼?
//     答:UB。標準把「字串非空」列為前置條件,pop_back 不做檢查、
//         不丟例外、也不會「安分地什麼都不做」。呼叫前必須自己確認
//         !s.empty()。這與 at() 那種「安全地失敗」是完全不同的設計取向。
//     追問:實測好像沒崩,是不是其實安全?→ 不是。UB 的定義是「標準不規範」,
//         沒崩只是這次的實作剛好把 size 減成 wrap around 的巨值或無害地
//         寫了一個 '\0';換編譯器、換最佳化等級就可能不一樣。
//
// 🔥 Q2. pop_back() 會釋放記憶體嗎?
//     答:不會。它只把 size() 減 1 並在新的結尾寫入 '\0',capacity() 不受
//         影響、heap buffer 不歸還。所以「把字串 pop 到空」跟「釋放它佔的
//         記憶體」是兩件事,後者要靠 shrink_to_fit()(非約束性請求,實作可
//         忽略)或 swap trick:std::string().swap(s)。
//     追問:那 iterator 會失效嗎?→ 至少指向被移除字元(以及 end())的
//         iterator/reference 失效;安全的心智模型是「改長度後就別再用舊的」。
//
// 🔥 Q3. 為什麼 pop_back() 不回傳被移除的那個字元?
//     答:這是 STL 一貫的設計:回傳值必須複製(或搬移)一份物件出來,
//         而那次複製若丟例外,元素已經被移除、卻沒能交到呼叫端手上,
//         狀態就無法回復,強例外保證做不到。拆成 back() 取值 + pop_back()
//         移除兩步,兩者各自都能保持例外安全。
//     追問:同樣道理還出現在哪?→ std::stack::pop、std::vector::pop_back
//         也都回傳 void,是同一個理由。
// ═══════════════════════════════════════════════════════════════════════════
