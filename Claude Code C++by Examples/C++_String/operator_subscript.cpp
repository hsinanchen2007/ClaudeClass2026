...
// ═══════════════════════════════════════════════════════════════════════════
// 【面試題】std::string::operator[]
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. operator[] 和 at() 該怎麼選?
//     答:operator[] 不做邊界檢查,越界是 UB;at() 會檢查,越界丟
//         std::out_of_range。[] 沒有分支、對 vectorization 友善,適合
//         「caller 已由 size()/迴圈邊界保證索引合法」的熱路徑;
//         索引來自外部輸入、無法事先保證時才該用 at()。
//     追問:那為何標準不乾脆讓 [] 都檢查?→「不為你沒用到的東西付費」。
//
// 🔥 Q2. s[s.size()] 合法嗎?
//     答:自 C++11 起合法,回傳結尾 null 字元的引用(const 與非 const 版本
//         皆然)。但在非 const 版本上把它寫成非 '\0' 的值是 UB —— 因為那會
//         破壞「buffer 尾端必為 '\0'」這個標準保證的不變式。
//         注意這只對 pos == size() 成立;pos > size() 仍然是 UB。
//     追問:這跟 data()[size()] == '\0' 是同一件事嗎?→ 是,兩者都是
//         C++11 起「string 內部 null-terminated」保證的不同表現面。
//
// ⚠️ 陷阱. std::string s; s[0] = 'a'; 合法嗎?
//     答:不合法,是 UB。s 是空字串,size()==0,所以 s[0] 正好是
//         s[s.size()];C++11 允許「讀取」它,但「寫入」非 '\0' 的值不允許。
//         正確做法:s.resize(1); s[0]='a';,或直接 s.push_back('a') / s += 'a'。
//     為什麼會錯:腦中把 std::string 想成「一塊夠大的 char 陣列」,以為
//         capacity 有空間就能直接用索引賦值。實際上 size() 才是有效範圍,
//         capacity 只是「不必重新配置就能長到多大」,不代表那些位置可寫。
// ═══════════════════════════════════════════════════════════════════════════
