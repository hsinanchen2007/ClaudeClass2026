...
// ═══════════════════════════════════════════════════════════════════════════
// 【面試題】io_operators (operator<< / operator>>)
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. cin >> s 的分隔語意是什麼？
//     答：先跳過前導空白，接著讀到「下一個空白」為止，並且把那個空白留在緩衝區。
//         因此 >> 永遠讀不到含空白的一整行，要整行請用 std::getline。
//     追問：>> 會不會保留 s 原本的內容？→ 不會，operator>> 先 str.erase() 再填入。
//
// 🔥 Q2. 怎麼替自訂型別多載 operator<<？簽名長什麼樣？
//     答：寫成 non-member 函式（第一參數是 stream，所以不可能是成員函式）：
//         std::ostream& operator<<(std::ostream& os, const T& v) { os << …; return os; }
//         重點是取 ostream& 也回傳 ostream&，才能串接 os << a << b；
//         需要存取私有成員時宣告為 friend。
//     追問：能不能在裡面順手加 std::endl？→ 不要。換行與 flush 的時機屬於呼叫端的
//           決定，內建就加會讓輸出無法組合，而且 endl 的 flush 是效能地雷。
//
// ⚠️ 陷阱. 為什麼讀取迴圈要寫 while (cin >> x)，而不是 while (!cin.eof())？
//     答：eof() 只在「已經嘗試讀取並碰到檔尾」之後才為 true。
//         先檢查 eof() 會讓最後一次失敗的讀取仍然進到迴圈體，
//         於是最後一筆資料被重複處理一次。
//     為什麼會錯：把 eof() 想成「還有沒有資料」的事前預測，但它其實是
//         「上一次讀取的事後狀態」。正確做法是把讀取本身當條件——
//         它回傳的 istream& 已同時涵蓋 EOF 與 fail 兩種情況。
// ═══════════════════════════════════════════════════════════════════════════
