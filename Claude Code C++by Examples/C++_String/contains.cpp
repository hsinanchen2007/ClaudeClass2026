...
// ═══════════════════════════════════════════════════════════════════════════
// 【面試題】std::string::contains
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. contains 是哪個標準加入的?在那之前怎麼寫?
//     答：C++23。在那之前只能寫 s.find(x) != std::string::npos——
//         語意正確但囉嗦,而且每次都得記得跟 npos 比(忘了就是經典 bug)。
//         contains 直接回傳 bool,意圖清楚。
//     追問：starts_with / ends_with 也是 C++23 嗎?→ 不是,那兩個更早,是 C++20。
//
// 🔥 Q2. 為什麼說 contains 可以避免不必要的 substr?
//     答：想知道「有沒有包含」卻用 substr 去比,會建立一個新的 std::string:
//         複製 O(len),長度超過 SSO 門檻時還多一次 heap 配置
//         (SSO 門檻是 implementation-defined,標準未規定任何數字)。
//         contains 只是掃描,零配置。
//
// ⚠️ 陷阱. contains 會比 find 快嗎?
//     答：不會。標準把 contains 定義為等價於 find(...) != npos,
//         它買到的是可讀性與「不會寫錯 npos 判斷」,不是演算法上的加速。
//     為什麼會錯：把「比較新的 API」直覺等同於「比較快的 API」。
// ═══════════════════════════════════════════════════════════════════════════
