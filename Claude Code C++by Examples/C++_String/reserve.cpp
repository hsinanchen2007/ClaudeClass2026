...
// 【面試題】reserve 與字串串接效能
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. reserve() 做什麼?它會改變 size() 嗎?
//     答:reserve(n) 確保 capacity() >= n,只動 capacity、完全不動 size();
//     要改 size() 是 resize() 的事。C++20 起 reserve 不再會縮小容量;
//     C++20 之前 reserve(n) 且 n < capacity() 是非約束性的縮小請求,實作可忽略。
//     追問:reserve 之後原本的 iterator / c_str() 指標還有效嗎?→ 若真的發生
//     重新配置就全部失效。
//
// 🔥 Q2. 字串串接的效能怎麼優化?
//     答:a + b + c + d 會產生多個臨時 string,每個都可能各自配置。最佳做法是
//     先 reserve 總長度,再連續 += / append,全程只配置一次。+= 與 append 因
//     幾何成長而是攤還 O(1),所以迴圈裡 s += c 一百萬次是 O(n) 不是 O(n²)。
//     追問:為什麼 s = s + x 比 s += x 慢?→ 前者建臨時物件再賦值回去,
//     後者原地追加。
//
// ⚠️ 陷阱. reserve(1000) 之後,s[500] = 'x' 合法嗎?
//     答:不合法,是 UB。reserve 只擴大 capacity,size() 仍是原值,s[500] 落在
//     有效範圍之外。要能寫入必須先 resize(1000)。
//     為什麼會錯:很多人把 capacity 當成「可用索引範圍」,但決定合法索引的一直
//     是 size()。
// ═══════════════════════════════════════════════════════════════════════════
