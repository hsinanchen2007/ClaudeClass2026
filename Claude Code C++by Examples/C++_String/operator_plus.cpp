...
// ═══════════════════════════════════════════════════════════════════════════
// 【面試題】std::string 的 operator+
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. a + b + c + d 的成本是多少?怎麼改比較好?
//     答:每個 + 都產生一個新的臨時 std::string,長度超過 SSO 門檻時各自
//         heap 配置一次 → 多次配置 + 多次複製。編譯器對右值重載有優化,
//         但仍非最佳。最佳做法:reserve(總長度) 之後連續 += / append,
//         全程只配置一次。
//     追問:那為什麼 STL 不自動優化成 expression template?→ 那會讓
//           operator+ 回傳非 string 的代理型別,配合 auto 極易產生 dangling。
//
// 🔥 Q2. operator+ 有哪些重載?回傳什麼?
//     答:回傳一個全新的 std::string(值),不是引用。左右運算元可為
//         string / const char* / char,C++17 起也接受 string_view-like。
//         因為回傳新物件,operator+ 不會使任何既有 iterator 失效。
//     追問:為什麼 hot loop 裡要避免 operator+?→ 每次迭代都建臨時物件,
//           改用 += 可原地追加、走攤還 O(1) 路徑。
//
// ⚠️ 陷阱. 為什麼 auto x = "a" + "b"; 編不過?
//     答:兩個字面量的型別是 const char[2],會退化成 const char*,
//         "a" + "b" 變成「指標 + 指標」的指標算術 → 編譯錯誤。
//         只要任一側是 std::string,才會選中 std::string 的 operator+ 重載。
//         解法:std::string("a") + "b"、"a"s + "b"(C++14 string literal)。
//     為什麼會錯:腦中把 "abc" 當成「字串物件」,但它其實是 char 陣列;
//         C++ 從來沒有替原生陣列/指標定義過字串串接。
// ═══════════════════════════════════════════════════════════════════════════
