...
// 【面試題】empty
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. empty() 和 size() == 0 有差別嗎?
//     答:對 std::string 而言語意完全相同、都是 O(1),差別在可讀性與泛型一致性:
//     所有 STL 容器都提供 empty(),用同一個動詞表達同一件事。真正有效能差異的是
//     別的容器——例如 std::list 的 size() 在 C++11 前不保證 O(1),用 empty()
//     才穩妥。C++20 起 empty() 加上 [[nodiscard]],忘記用回傳值會被警告。
//     追問:為什麼 [[nodiscard]] 有用?→ 擋掉 s.empty(); 這種把「查詢」誤當成
//     「清空」的手誤,真的要清空是 s.clear()。
//
// Q2. s.empty() 為真,代表 s.capacity() 也是 0 嗎?
//     答:不代表。empty() 只看 size(),capacity() 是另一回事:因為 SSO,空字串在
//     libstdc++ 上 capacity() 就是 15(這是 implementation-defined,標準未規定
//     任何數字)。clear() 之後 size() 歸零但 capacity() 通常原封不動。
//
// ⚠️ 陷阱. UTF-8 下 empty() 判斷的是「沒有字元」嗎?
//     答:empty() 等價於 size() == 0,而 size() 回傳的是位元組數不是字元數,
//     所以「非空」只保證至少有一個位元組。同理 "中文".size() 是 6 不是 2。
//     為什麼會錯:把 std::string 想成「字元序列」,但它其實是位元組序列,
//     標準庫不做任何編碼解讀。
// ═══════════════════════════════════════════════════════════════════════════
