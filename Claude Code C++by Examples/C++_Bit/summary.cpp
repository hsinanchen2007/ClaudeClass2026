...
// 【面試題】位元運算總覽
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. 關於位運算，面試官最想聽到的一句話是什麼？
//     答：「位運算一律用 unsigned；n & (n-1) 清最低位的 1、n & -n 取最低位的 1；
//     位移量為負或 >= 型別寬度，在任何標準版本都是 UB，C++20 只修好了有號數的『值語意』
//     ——左移改為模 2^N 折返、負數右移確定為算術右移。」把「位移量違規」與「有號數值
//     語意」分開講，是這個主題最重要的區辨點。
//     追問：C++20 有沒有一併修好有號數的加減乘溢位？（沒有，那仍然是 UB；P0907 只規定
//     了補數表示法與位移語意）
//
// 🔥 Q2. 為什麼「我編過也跑出正確答案」不能證明位運算程式碼合法？
//     答：因為 UB 最典型的表現就是「平常都對」。例如 1 << 31 在 C++17 及以前是 UB，
//     但 UBSan 並不會診斷這種溢位（本機已實測），x86 上 x >> 32 也只是硬體取位移量低
//     5 bit 的結果，不是語言保證——一旦換編譯器、換最佳化等級或換平台就可能改變。
//     合法性的判準是語言規則，不是執行結果。實務作法：位遮罩一律寫 1u／1ull，並在
//     測試時開 -fsanitize=undefined 抓那些「確實會被診斷」的違規（如位移量超界）。
//
// Q3. C++20 <bit> 讓哪些手寫技巧可以退休？
//     答：popcount 取代逐位計數與 SWAR、has_single_bit 取代 n>0 && (n&(n-1))==0、
//     countr_zero 取代 __builtin_ctz、bit_width／bit_ceil／bit_floor 取代手算 log2、
//     rotl／rotr 取代有 UB 邊界的手寫循環位移、bit_cast 取代 memcpy 與
//     reinterpret_cast 的 type punning。它們全部 constexpr、可攜、且通常直接映射到硬體
//     指令；但都要求無號引數。
// ═══════════════════════════════════════════════════════════════════════════
...
//
// [demo_basic_ops]
//   a     = 00000000000000000000000011001010 (202)
//   b     = 00000000000000000000000010101100 (172)
//   a & b = 00000000000000000000000010001000
//   a | b = 00000000000000000000000011101110
//   a ^ b = 00000000000000000000000001100110
//   ~a    = 11111111111111111111111100110101
//   a<<2  = 00000000000000000000001100101000
//   a>>2  = 00000000000000000000000000110010
//   precedence: wrong=0, right=1
//
// [demo_common_tricks]
//   x=00000000000000000000000010110000
//   lsb(x) = 00000000000000000000000000010000 (16)
//   x&(x-1)= 00000000000000000000000010100000
//   is_pow2(16)=1, is_pow2(18)=0
//   popcount(x)=3
//
// [demo_flags]
// …（後略，完整輸出共 28 行）
