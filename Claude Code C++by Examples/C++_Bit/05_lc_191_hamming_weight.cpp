...
// ═══════════════════════════════════════════════════════════════════════════
// 【面試題】LeetCode 191（popcount）
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. 計算一個整數的 popcount 有哪幾種方法？
//     答：① 逐位檢查——for (int i = 0; i < 32; ++i) if (n >> i & 1) ++cnt; 固定 O(32)
//     ② Brian Kernighan——while (n) { n &= n - 1; ++cnt; }，迴圈次數等於 set bit 的
//     個數，稀疏數字上明顯較快 ③ C++20 的 std::popcount（<bit>，要求無號型別）或
//     GCC/Clang 的 __builtin_popcount，通常直接編成單一 POPCNT 指令。另有查表法（每
//     8 bit 一張 256 項的表）與 SWAR 分治法，嵌入式面試偶爾要求手寫。
//     追問：n &= n - 1 為什麼能消掉最低位的 1？（n - 1 把最低位的 1 變 0、其右方全部
//     0 變 1，AND 後該位與其右方全為 0）
//
// 🔥 Q2. 為什麼 std::popcount(n) 在 n 是 int 時編譯不過？
//     答：<bit> 的所有函式都要求 unsigned integer type，這是刻意的設計——有號型別在
//     位運算上有太多陷阱（符號位、算術右移、負值的補數表示）。要用就明確寫
//     std::popcount(static_cast<unsigned>(n))，這個轉型同時也是在宣告「我知道我要的是
//     補數表示下的位元」。
//
// ⚠️ 陷阱. while (n) { n &= n - 1; ++cnt; } 傳入負的 int 會怎樣？
//     答：多數情況能跑，結果也確實是補數表示下的 set bit 數（例如 -1 得到 32），因為
//     AND 本身沒有 UB。但 n - 1 在 n == INT_MIN 時是有號溢位，那是 UB——C++20 只把
//     位移與表示法規定清楚，並沒有改變算術溢位仍是 UB 這件事。LeetCode 191 的簽名
//     int hammingWeight(int n) 就正好會踩到這個坑。正解是一律轉成 unsigned 再做。
//     為什麼會錯：測了幾個負數都沒事，就以為安全；UB 的特性正是「平常看起來都對」。
// ═══════════════════════════════════════════════════════════════════════════
