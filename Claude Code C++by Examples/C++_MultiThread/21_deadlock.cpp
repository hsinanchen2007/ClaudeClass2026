...
// ═══════════════════════════════════════════════════════════════════════════
// 【面試題】Deadlock：成因、預防與偵測
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. Deadlock 的四個必要條件？如何避免？
//     答：互斥（mutual exclusion）、佔有並等待（hold and wait）、不可剝奪（no
//         preemption）、環路等待（circular wait），四者同時成立才會死鎖。破壞任一即可：
//         破壞「佔有並等待」→ 一次性取得所有鎖（std::scoped_lock / std::lock）；
//         破壞「環路等待」→ 全域固定的鎖順序（工程上最實用，例如依 mutex 位址或 ID
//         排序）；破壞「不可剝奪」→ try_lock 失敗就退讓重試，或用 timed_mutex 設超時。
//     追問：實務上最常見的死鎖長什麼樣？（兩條執行緒以相反順序鎖同兩把 mutex）
//
// 🔥 Q2. std::scoped_lock(m1, m2) 與依序 m1.lock(); m2.lock(); 差在哪？
//     答：後者固定了取鎖順序，若別處以相反順序取鎖就會死鎖。scoped_lock 內部走
//         std::lock 的免死鎖演算法：嘗試取得全部，任一失敗就把已取得的全部釋放、
//         退讓後重試，因此無論各處傳入的參數順序如何都不會形成環路等待。單把 mutex
//         時它等價於 lock_guard。
//     追問：那還需要全域鎖順序嗎？（需要。scoped_lock 只能解決「同一次呼叫中一起取得」
//           的情況；跨函式、跨模組分別取得的鎖仍需靠約定的順序）
//
// 🔥 Q3. 死鎖發生了怎麼診斷？
//     答：先確認是死鎖而非單純很慢：CPU 使用率接近 0 但程式沒有進展，就高度可疑。
//         用 gdb 附掛上去執行 thread apply all bt，看每條 thread 卡在哪一個 lock 呼叫、
//         各自持有什麼，通常能直接畫出等待環。ThreadSanitizer 也能報部分 lock order
