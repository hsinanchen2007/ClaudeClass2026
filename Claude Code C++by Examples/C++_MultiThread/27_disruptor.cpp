...
// 【面試題】Disruptor 模式
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. Disruptor 的核心思想是什麼？
//     答：用「預先配置的環形陣列 + 序號（sequence）」取代傳統佇列。核心洞見是：傳統
//         queue 的瓶頸不在演算法，而在記憶體配置、cache 未命中與鎖爭用。所以
//         Disruptor：(1) 預配置所有 slot，執行期零配置、記憶體佈局連續；(2) 生產者與
//         消費者各自維護 atomic 序號，用 acquire/release 配對而非鎖；(3) 序號變數以
//         padding 隔離避免 false sharing；(4) 消費者可批次處理已可用的整個區間。
//     追問：批次消費為什麼重要？（一次取得「到目前為止全部可用」的區間，把每筆訊息
//           的同步成本攤平，負載越高攤得越平——這是它高吞吐的關鍵之一）
//
// 🔥 Q2. Disruptor 與 SPSC ring buffer 差在哪？
//     答：SPSC 是 1:1、pop 之後位置就被釋放。Disruptor 的 slot 不會被「pop」，只是被
//         「讀」過，因此可以支援多個消費者各自持有 cursor 讀同一份資料（廣播），或
//         組成 pipeline（消費者 A 處理完 B 才能看）。生產者只需要跟「最慢的消費者」
//         同步一次，就能知道哪些格子可以覆寫。
//     追問：多消費者之間需要互相同步嗎？（廣播模式下不需要，各看各的 cursor，這正是
//           它比「多個消費者搶同一條 queue」快的原因）
//
// Q3. 等待策略（wait strategy）如何取捨？
//     答：busy-spin 延遲最低但持續燒 CPU，適合專用核心的低延遲場景；yielding 折衷；
//         blocking（用鎖或 CV 睡眠）最省電，但喚醒延遲高、且延遲分布的尾巴會變長。
//         選擇取決於「這條路徑的延遲值不值得一顆核心」。
//     追問：spin 迴圈裡該加什麼？（x86 上的 pause 指令，或 C++20 之後可考慮
//           atomic::wait；純空轉會浪費發射槽並加劇 cache line 爭用）
// ═══════════════════════════════════════════════════════════════════════════
