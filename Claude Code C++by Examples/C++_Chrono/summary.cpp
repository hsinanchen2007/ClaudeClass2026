...
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. 一句話說明三個時鐘該怎麼選？
//     答：量測經過時間、逾時、限流、retry backoff 一律用 steady_clock（單調、不受調時
//         影響）；顯示時間、寫 log 時間戳、與外部系統交換 timestamp 用 system_clock
//         （可轉日曆，但可能被 NTP 往回調）；high_resolution_clock 只是別名、實作可能
//         指向前兩者之一，語意不可攜，不要用。
//     追問：steady_clock 的 epoch 有意義嗎？（沒有，只能用於相減）
//
// 🔥 Q2. chrono 的型別安全體現在哪？破口又在哪？
//     答：單位被編進型別（duration<Rep, Period>），混用不同單位時編譯器會自動換算或
//         直接報錯，換算在編譯期完成、執行期零成本；time_point 與 duration 分離，
//         point + point 這種無意義運算編譯不過。唯一的破口是 count()——它剝離了單位
//         資訊，應盡量延後到最後輸出或跨 C API 邊界時才呼叫。
//     追問：什麼時候必須顯式 duration_cast？（細 → 粗這種會損失精度的方向；且它是
//           朝零截斷，不是四捨五入，需要時請改用 floor / ceil / round）
// ═══════════════════════════════════════════════════════════════════════════
