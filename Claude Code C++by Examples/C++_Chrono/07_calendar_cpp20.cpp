...
// ═══════════════════════════════════════════════════════════════════════════
// 【面試題】C++20 chrono 日曆與時區
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. C++20 的 chrono 新增了什麼？
//     答：三大類。(1) 日曆型別：year、month、day、year_month_day、weekday，可寫成
//         2026y/July/19d 這種自然語法，取代 C 風格的 std::tm。(2) 時區支援：
//         zoned_time、current_zone()、locate_zone("Asia/Taipei")，基於 IANA 時區資料庫。
//         (3) 新時鐘與轉換：utc_clock（計閏秒）、tai_clock、gps_clock、file_clock，
//         並以 std::chrono::clock_cast 在不同時鐘之間安全轉換；另有 hh_mm_ss 拆解與
//         std::format 對 chrono 型別的格式化支援。
//     追問：為什麼需要這麼多時鐘？（各自的 epoch 與對閏秒的處理方式不同，混用會產生
//           數秒的誤差，因此標準要求透過 clock_cast 明確轉換）
//
// 🔥 Q2. system_clock 與 utc_clock 差在哪？
//     答：system_clock 表示 Unix time，「不計閏秒」——閏秒發生時它會重複或跳過該秒，
//         所以兩個 time_point 相減得到的不是真實經過的 SI 秒數。utc_clock 則計入閏秒，
//         相減才是真正的物理時間差。C++20 起標準明確規定 system_clock 的 epoch 是
//         Unix epoch（1970-01-01 UTC）；在此之前 epoch 是實作定義，只是實務上都一樣。
//     追問：什麼場景真的需要在意閏秒？（天文、衛星定位、精密量測、跨閏秒的長時間間隔
//           計算；一般業務系統用 system_clock 即可）
//
// Q3. 為什麼日期運算要用 year_month_day 而不是自己算天數？
//     答：因為日曆規則充滿特例：閏年、月份長度不同、時區的日光節約時間會讓某一天只有
//         23 小時或 25 小時。標準型別把這些規則封裝起來，還提供 ok() 檢查日期是否合法
//         （例如 2 月 30 日）。自己用「一天 86400 秒」去算，跨越 DST 或閏日時必錯。
//     追問：sys_days 是什麼？（以天為單位的 system_clock time_point，是日曆型別與
//           時間點之間互轉的橋樑）
// ═══════════════════════════════════════════════════════════════════════════
