...
// ═══════════════════════════════════════════════════════════════════════════
// 【面試題】nested namespace
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. namespace a::b::c { } 是什麼？帶來什麼改變？
//     答：C++17 的巢狀命名空間簡化語法，一行取代三層巢狀大括號，寫法也和使用端的
//         a::b::c 對應起來。它純粹是語法糖：不改變 namespace 的 linkage，也不改變
//         名稱查找（含 ADL）規則，等價於逐層展開的舊寫法。
//     追問：裡面還能再開巢狀嗎？（可以，simplified 與傳統寫法可任意混用）
//
// Q2. 能和 inline namespace 結合嗎？
//     答：C++17 的簡化語法只支援普通 namespace，中間不能加 inline。
//         namespace x::inline v1::detail { ... } 要到 C++20 才合法。
//         版本化 API（v1/v2）這種常見用途，正好卡在這個版本差異上。
//
// Q3. 巢狀太深時怎麼在呼叫端減少負擔？
//     答：用 namespace 別名：namespace cpu = company::product::util;
//         注意它的關鍵字是 namespace 而不是 using，寫成 using cpu = ...; 是型別別名，
//         對 namespace 並不適用。
// ═══════════════════════════════════════════════════════════════════════════
