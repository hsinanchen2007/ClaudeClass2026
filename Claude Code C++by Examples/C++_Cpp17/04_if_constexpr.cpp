...
//  另一條【不會被實例化】(instantiate)，所以可以寫「對某些 T 根本無法編譯」
//  的程式碼。但注意：捨棄的分支【仍然要通過語法解析】，不依賴模板參數的
//  語意錯誤也照樣會被診斷 —— 在 discarded 分支塞一個 `@` 進去，g++ 實測
//  仍然報 error: stray '@' in program。被豁免的只有「依賴 T」的那部分。
//
//  ┌────────────────────────────────────────────────────────────┐
//  │ 二、為什麼好用？                                           │
//  └────────────────────────────────────────────────────────────┘
