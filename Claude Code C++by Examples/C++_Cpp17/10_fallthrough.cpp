...
// 【面試題】[[fallthrough]]
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. [[fallthrough]] 解決什麼問題？
//     答：switch 的 case 預設會往下穿透，忘記 break 是經典 bug，因此編譯器有
//         -Wimplicit-fallthrough 警告。但有時穿透是刻意的，這個屬性就是向編譯器
//         與讀者明示「我是故意的」，同時消掉警告。
//     追問：它改變程式行為嗎？（完全不改，穿透本來就會發生；它只是意圖標記）
//
// Q2. 語法上要怎麼寫？
//     答：它是一個 statement，要自成一行並加分號：[[fallthrough]];
//         位置放在該 case 的最後、下一個 case 標籤之前。
//         若後面沒有接著任何 case／default 標籤，寫了反而是 ill-formed。
//
// Q3. 連續 case 標籤（case 1: case 2: 中間沒有敘述）需要標嗎？
//     答：不需要。那是「共用同一段程式」的寫法，編譯器不會對它發穿透警告，
//         只有 case 內已經執行了敘述又往下掉時才需要標記。
// ═══════════════════════════════════════════════════════════════════════════
