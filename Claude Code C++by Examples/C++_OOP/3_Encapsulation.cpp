...
//     答：封裝 — 用 public/protected/private 控制存取，資料私有、以成員函式提供
//     受檢查的入口；繼承 — `class D : public B` 表達 is-a；
//     多型 — virtual 函式（執行期）與 template（編譯期）。
//     追問：封裝的價值是什麼？（把不變條件的維護集中在少數函式，日後可換掉內部
//     表示法而不影響呼叫端）
//
// 🔥 Q2. public / protected / private 的差別？
//     答：public 任何地方可存取；private 只有自己與 friend 能存取；
//     protected 額外開放給 derived class。protected 會把內部設計暴露給所有子類別，
//     過度使用會讓整個繼承層級難以重構。
//     追問：private 成員在物件記憶體中還存在嗎？（存在 — 存取控制是編譯期規則，
//     不影響物件佈局，也不是任何形式的安全防護）
//
// ⚠️ 陷阱. getter 寫成回傳 `T&` 有什麼問題？
//     答：等於把 private 的後門打開 — 呼叫端可以繞過 setter 內所有檢查直接改內部資料，
//     封裝形同虛設。只讀用途應回傳 by value 或 `const T&`。
//     為什麼會錯：只檢查「成員有沒有宣告成 private」，卻沒注意介面自己把一個可寫的
//     參考交了出去；封裝看的是「誰能修改狀態」，不是關鍵字寫在哪。
// ═══════════════════════════════════════════════════════════════════════════
