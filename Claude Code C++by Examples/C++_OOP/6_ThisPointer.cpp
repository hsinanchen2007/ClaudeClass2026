...
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. this 是什麼？型別為何？
//     答：編譯器隱式傳給每個「非 static 成員函式」的物件位址。在 T 的成員函式中型別是
//     `T*`，在 const 成員函式中是 `const T*`（所以 const 函式不能透過 this 改成員）。
//     this 不是可以被指派的變數，寫 `this = ...` 不合法。
//     追問：為什麼 static 成員函式沒有 this？（它屬於類別、不繫結到任何物件）
//
// 🔥 Q2. 為什麼 setter 要回傳 `*this`？
//     答：回傳型別寫 `T&` 並 `return *this;`，就能鏈式呼叫 `obj.setA(1).setB(2)`。
//     若回傳 by value 會複製出一個新物件，後續呼叫改到的是副本、不是原物件。
//     追問：operator= 為什麼也回傳 `T&`？（支援 `a = b = c`，並與內建型別語意一致）
//
// Q3. `delete this;` 合法嗎？
//     答：合法但危險。前提是：物件必須是 new 出來的、之後不得再存取任何成員或呼叫任何
//     成員函式（包含隱式用到 this 的）、且絕不能對 stack 或 static 物件做。
//     實務上幾乎只出現在自我管理生命週期的舊式參考計數物件。
//
// Q4. lambda 捕獲 `[this]` 有什麼風險？
//     答：捕獲的是裸指標，不會延長物件生命週期；若 lambda 被存起來延後執行（callback、
//     thread），物件先死就是 use-after-free。C++17 起可用 `[*this]` 按值複製整個物件；
//     若物件由 shared_ptr 管理，可改捕獲 weak_ptr，執行時再 lock() 檢查目標是否還在。
//
// ⚠️ 陷阱. 透過 null 指標呼叫成員函式一定會 crash 嗎？
//     答：不會「一定」crash，但它是 undefined behavior — 即使該函式完全沒有碰任何成員
//     也一樣。非 virtual 函式在多數實作上「碰巧能跑」，這正是它最危險的地方。
//     為什麼會錯：以為「只要函式內沒有解參考成員就沒事」；把 this 傳進來這件事本身，
//     就已經要求那個物件必須存在。
// ═══════════════════════════════════════════════════════════════════════════
