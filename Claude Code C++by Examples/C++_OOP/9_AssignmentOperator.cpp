...
// 【面試題】賦值運算子 operator=
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. copy assignment 的標準形式為何？為什麼回傳 `T&`？
//     答：`T& operator=(const T& rhs)` — 先處理自我賦值，釋放自己的舊資源，複製新資源，
//     最後 `return *this;`。回傳參考才能支援連鎖賦值 `a = b = c`，
//     並與內建型別的賦值語意保持一致。
//
// 🔥 Q2. 沒處理 self-assignment 會怎樣？
//     答：典型的「先 delete 舊資源、再從來源複製」寫法遇到 `a = a` 時，會先把來源自己
//     刪掉、再去讀它 → 讀取已釋放的記憶體。最直接的解法是開頭加
//     `if (this == &rhs) return *this;`。
//     追問：什麼情境會不小心自我賦值？（別名 — 兩個指標/參考指向同一個物件，
//     例如容器內的 `v[i] = v[j]`，i 與 j 相等時就中獎）
//
// 🔥 Q3. copy-and-swap 是什麼？好在哪？
//     答：`T& operator=(T rhs) { swap(*this, rhs); return *this; }` — 參數按值傳入時就
//     完成了複製，成功之後才 swap，舊資源隨區域副本解構掉。它天然免疫自我賦值，
//     而且提供 strong exception guarantee：複製若失敗，原物件完全沒被動過。
//     追問：缺點是什麼？（自我賦值時仍會做一次多餘的完整複製；且需要一個 noexcept 的 swap）
//
// ⚠️ 陷阱. 手寫了 copy assignment 之後，move assignment 還會自動產生嗎？
//     答：不會。使用者宣告 copy assignment（或 copy constructor、destructor）會抑制隱式
//     move 的產生，所有搬移都會靜默退化成複製 — 而且不會有任何編譯錯誤提醒你，
//     只有效能默默變差。要 move 就得自己 `= default` 或手寫出來。
//     為什麼會錯：以為五個特殊成員彼此獨立、缺哪個補哪個；實際上它們之間有一套
//     互相抑制的產生規則（Rule of Five 的實際痛點）。
// ═══════════════════════════════════════════════════════════════════════════
