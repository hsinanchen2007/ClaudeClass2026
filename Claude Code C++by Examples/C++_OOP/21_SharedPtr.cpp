...
// ═══════════════════════════════════════════════════════════════════════════
// 【面試題】std::shared_ptr 與 std::weak_ptr
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. shared_ptr 的控制區塊裡有什麼？物件與控制區塊分別何時被釋放？
//     答：控制區塊含「指向被管理物件的指標（或物件本身）、型別擦除的 deleter 與
//         allocator、強引用計數、弱引用計數」。強計數歸零時 → 解構被管理的物件；
//         但「控制區塊本身要等弱計數也歸零才釋放」，這就是為什麼 weak_ptr 還活著時
//         `expired()` 仍能正確回報。shared_ptr 物件自己存兩個指標（物件＋控制區塊），
//         所以 sizeof 通常是裸指標的兩倍。
//     追問：`make_shared` 與 `shared_ptr<T>(new T)` 差在哪？（make_shared 一次配置
//           同時容納控制區塊與物件，較快也 exception-safe；但需要自訂 deleter 時
//           不能用，且物件大又有長壽 weak_ptr 時，那塊記憶體會被卡住無法歸還）
//
// 🔥 Q2. 什麼是循環引用？如何解決？
//     答：兩個物件互相持有對方的 shared_ptr，彼此強計數永遠不會降到 0 → 記憶體洩漏
//         且 destructor 永不執行。解法是把其中一邊（通常是反向／父指標／observer
//         那一邊）改成 weak_ptr —— 就像本檔 Node::parent 用 weak_ptr、children
//         用 shared_ptr。要用時再 `lock()` 取得暫時的 shared_ptr。
//     追問：weak_ptr 為什麼不能直接解引用？（它觀察的物件隨時可能消失；必須用
//           `lock()` 以原子操作嘗試提升為 shared_ptr，失敗則回傳空的 shared_ptr）
//
// 🔥 Q3. shared_ptr 是 thread-safe 的嗎？
//     答：要分三層講：① 控制區塊的「引用計數操作」是原子的，多執行緒同時複製／銷毀
//         「不同的 shared_ptr 實例」（即使指向同一物件）是安全的；② 被管理的「物件
//         本身」不受保護，多執行緒讀寫仍要自己同步；③ 多執行緒存取「同一個
//         shared_ptr 物件」且其中有非 const 操作（reset、指派）就是 data race，
//         C++20 起可改用 `std::atomic<std::shared_ptr<T>>`。
//     追問：原子計數的代價？（比非原子遞增顯著昂貴，多核競爭時更明顯 —— 這正是
//           「別到處亂傳 shared_ptr，只讀就傳 const T& 或裸指標」的理由）
//
// ⚠️ 陷阱. 用同一個裸指標建立兩個 shared_ptr 會怎樣？
//     答：`T* p = new T; shared_ptr<T> a(p); shared_ptr<T> b(p);` 會產生「兩個
//         獨立的控制區塊」，各自計數歸零各刪一次 → double free（UB）。
//         `shared_ptr<T> b(a.get());` 是同一個 bug 的另一種寫法。
//     為什麼會錯：多數人以為引用計數是「跟著那塊記憶體」的全域帳本；實際上計數屬於
//         控制區塊，而控制區塊是「建立 shared_ptr 時」產生的。原則：裸指標只能交給
//         smart pointer 一次，最好用 make_shared 讓裸指標根本不出現。
// ═══════════════════════════════════════════════════════════════════════════
