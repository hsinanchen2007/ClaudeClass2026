...
// 【面試題】std::min_element
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. std::min_element 需要幾次比較？有多個「同樣小」的元素時回傳哪一個？
//     答：標準規定「剛好 N-1 次比較」(N 為區間長度，空區間 0 次)，複雜度 O(N)。
//         有多個等小值時回傳「第一個」——因為實作用嚴格 `<` 比較，遇到等值不更新
//         最佳解，所以最左邊的那個會留下來。這是標準保證的行為，不是實作巧合。
//     追問：那 std::max_element 呢？(同樣 N-1 次、同樣回傳第一個；但 minmax_element
//           的 max 回傳的是「最後一個」——見 minmax_element.cpp)
//
// 🔥 Q2. std::min 和 std::min_element 的回傳型別差在哪？什麼時候該用哪個？
//     答：`std::min(a, b)` 回傳 `const T&`（值語意，兩個值的較小者）；
//         `std::min_element(first, last)` 回傳「迭代器」。
//         要「值」用 min；要「位置」(算索引 it - begin、取成員 it->m、
//         或直接 erase(it) 拿掉那個元素) 就必須用 min_element。
//     追問：只是要一堆值裡的最小值呢？(用 `std::min({a, b, c})` 的 ilist 版即可，
//           回傳的是「值」不是 reference，沒有 dangling 問題)
//
// ⚠️ 陷阱. `int m = *std::min_element(v.begin(), v.end());` 有什麼風險？
//     答：v 為空時 min_element 回傳 `last`(即 end())，對 end() 解參考是 UB。
//         必須先 `if (it != v.end())` 才能 `*it`。
//     為什麼會錯：多數人腦中把它想成「回傳最小值」，而數值型別沒有「空」的概念，
//         就忘了它其實回傳的是迭代器、而迭代器有 end() 這個合法但不可解參考的狀態。
// ═══════════════════════════════════════════════════════════════════════════
