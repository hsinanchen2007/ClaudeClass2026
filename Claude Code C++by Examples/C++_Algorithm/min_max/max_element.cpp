...
// 【面試題】std::max_element
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. std::max_element 的比較次數？回傳的是值還是位置？
//     答：剛好 N-1 次比較 (N 為區間長度，空區間 0 次)，O(N) 單次掃描。
//         回傳的是「迭代器」不是值——所以本檔範例才能同時拿到 *it (值) 與
//         it - begin (索引)。要值就寫 `*std::max_element(...)`，但空範圍時
//         回傳 last，直接解參考是 UB。
//     追問：同時要 min 和 max 呢？(用 minmax_element 一次掃描，比較次數從
//           2(N-1) 降到至多 ⌊3(N-1)/2⌋)
//
// 🔥 Q2. 有多個相同的最大值時，max_element 回傳哪一個？
//     答：回傳「第一個」。實作用嚴格 `<`，只有 `目前最佳 < 候選` 才更新，
//         等值時不更新，因此最左邊的勝出。這是標準保證的。
//     追問：想要「最右邊」的最大值怎麼辦？(用 reverse iterator 掃 rbegin/rend，
//           或改用 minmax_element 取它的 .second)
//
// ⚠️ 陷阱. `max_element` 和 `minmax_element` 對重複最大值的行為一樣嗎？
//     答：不一樣。`max_element` 回傳「第一個」最大值，
//         `minmax_element` 的 `.second` 回傳「最後一個」最大值。兩者都是標準明文
//         規定，不是實作差異。所以拿 minmax_element 的 max 去和 max_element 的
//         結果比對索引，在有重複最大值時會對不起來。
//     為什麼會錯：多數人假設「minmax_element 就是 min_element + max_element 的
//         合體」，但標準對這兩者的 tie-breaking 規則刻意訂得不同。
// ═══════════════════════════════════════════════════════════════════════════
