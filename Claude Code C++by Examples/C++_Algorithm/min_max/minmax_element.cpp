...
// ═══════════════════════════════════════════════════════════════════════════
// 【面試題】std::minmax_element
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. 為什麼 minmax_element 比「分別呼叫 min_element + max_element」省？
//     答：分開呼叫是 (N-1) + (N-1) = 2(N-1) 次比較；標準規定 minmax_element
//         至多 ⌊3(N-1)/2⌋ 次。作法是 pairwise：把元素兩兩配對，先讓這一對互比
//         1 次分出 (較小, 較大)，再讓較小者挑戰目前 min、較大者挑戰目前 max
//         (各 1 次)——每處理 2 個元素只花 3 次比較，總計約 1.5N。
//     追問：那為什麼不是所有找極值的場合都用它？(只要一端時 min_element 的
//           N-1 更少；minmax_element 的優勢只在「同時要兩端」)
//
// 🔥 Q2. minmax_element 回傳什麼？空範圍呢？
//     答：回傳 `std::pair<FwdIt, FwdIt>`，即 (min 的迭代器, max 的迭代器)，
//         是「位置」不是「值」。空範圍回傳 `{last, last}`，兩個都不能解參考。
//         C++17 可用結構化繫結 `auto [lo, hi] = ...` 拆開。
//     追問：想要「值」的 pair 呢？(用 std::minmax；但兩值版回傳的是
//           pair<const T&, const T&>，有 dangling 風險——見 minmax.cpp)
//
// ⚠️ 陷阱. 有多個最大值時，minmax_element 的 .second 指向第一個還是最後一個？
//     答：「最後一個」。而 max_element 指向「第一個」。兩者都是標準明文規定。
//         本檔範例 v{3,1,4,1,5,9,2,6,9} 就是刻意佈的局：max_element 得到 index 5，
//         minmax_element 得到 index 8。min 端則兩者一致，都是第一個。
//     為什麼會錯：直覺會假設 minmax_element 只是兩個演算法的合併、行為應該一致；
//         實際上標準對 min 端取「第一個」、max 端取「最後一個」，是不對稱的。
// ═══════════════════════════════════════════════════════════════════════════
