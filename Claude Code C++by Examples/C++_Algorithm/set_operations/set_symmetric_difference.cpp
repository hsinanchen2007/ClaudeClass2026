...
// ═══════════════════════════════════════════════════════════════════════════
// 【面試題】std::set_symmetric_difference
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. 對稱差是什麼?和呼叫兩次 set_difference 有何不同?
//     答：A △ B = (A \ B) ∪ (B \ A) = (A ∪ B) \ (A ∩ B),也就是「只出現在其中
//         一邊」的元素(集合版的 XOR)。一次呼叫就拿到兩邊獨有的聯集,但結果混在
//         一起、無法分辨某個元素來自哪一邊 — 要區分就得用 set_difference 各做一次。
//     追問：它對參數順序敏感嗎?(答：不敏感,交換 A、B 結果相同 — 這正是「對稱」的
//           意思;set_difference 則相反)
//
// 🔥 Q2. 重複元素的規則是什麼?
//     答：某元素在 A 出現 m 次、B 出現 n 次,輸出 |m - n| 次,從「份數較多」的那邊
//         複製(m > n 時取 A 的最後 m - n 份,反之取 B 的)。m == n 時該元素完全
//         消失 — 所以兩個內容相同的範圍,對稱差是空的。
//     追問：這和 set_union / set_intersection 的規則一起記?(答：union 取 max(m,n)、
//           intersection 取 min(m,n)、difference 取 max(m-n,0)、symmetric 取 |m-n|)
//
// Q3. 前置條件與複雜度?
//     答：和整族 set_xxx 一樣 — 兩個輸入都必須已依「同一個」comp 排序(不必是
//         std::set,排序過的 vector 即可),單趟雙指標線性掃描,O(N1 + N2)。
//         輸出寫到 output iterator,最多 N1 + N2 個元素,常搭配 std::back_inserter。
//
// ⚠️ 陷阱. 輸入未排序時,它會告訴你嗎?
//     答：不會。未排序是違反前置條件的 UB,不會拋例外也不會回報錯誤,只會給出
//         一個安靜錯誤的結果。
//     為什麼會錯：習慣了會檢查參數的 API,忘了 STL 演算法的前置條件由呼叫端負責。
// ═══════════════════════════════════════════════════════════════════════════
