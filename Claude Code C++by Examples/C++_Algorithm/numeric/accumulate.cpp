...
// 【面試題】std::accumulate
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. std::accumulate 的結果型別由誰決定?它在哪個標頭檔?
//     答:由「第三個參數 init 的型別 T」決定,不是由元素型別決定 —— 整個累加過程
//         都在 T 上進行。它定義在 <numeric> 而非 <algorithm>,忘了 include 會編譯失敗。
//         標準保證「由左至右依序」累加 (left fold),C++20 起為 constexpr。
//     追問:可以傳自訂 binary op 嗎?(可以,第四參數,例如 std::multiplies<>{} 求乘積)
//
// ⚠️ 陷阱. 對 std::vector<double> 呼叫 accumulate(v.begin(), v.end(), 0) 會得到什麼?
//     答:得到被截斷成 int 的結果。init 寫 0 → T = int → 每一步都做 int 加法,小數全丟掉;
//         本檔範例 {0.5, 1.5} 得 1 而非 2.0。正解是寫 0.0。
//         同理 int 元素但總和可能溢位時,要寫 0LL,而不是 0。
//     為什麼會錯:多數人以為「accumulate 會自己推導成元素型別」,但樣板參數 T 只從 init
//         推導,元素型別完全不參與型別決定。
//
// 🔥 Q2. accumulate 保證由左至右,帶來什麼能力與什麼代價?
//     答:能力是可安全用於「非結合、非交換」的運算 —— 字串串接、減法都成立,結果唯一確定。
//         代價是無法平行化:accumulate 沒有 execution policy 多載。需要平行請改用 C++17 的
//         std::reduce,但它反過來要求 op 必須 associative 且 commutative。
//     追問:那浮點加總兩者結果會一樣嗎?(不保證 —— 浮點加法不滿足結合律)
//
// 🔥 Q3. 用自訂 op 時,init 該怎麼挑?
//     答:挑該運算的「單位元」:加法 0、乘法 1、字串串接 std::string{}、取 max 用足夠小的值
//         (如 std::numeric_limits<int>::min())。挑錯會直接汙染結果 —— 例如求乘積卻把 init
//         寫成 0,答案恆為 0,而且完全不會報錯。
// ═══════════════════════════════════════════════════════════════════════════
