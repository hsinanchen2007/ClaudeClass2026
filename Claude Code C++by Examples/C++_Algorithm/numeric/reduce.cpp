...
// 【面試題】std::reduce (C++17)
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. std::reduce 和 std::accumulate 差在哪?
//     答:三點。(1) reduce 不保證求值順序、也不保證結合方式,accumulate 保證嚴格左至右;
//         (2) 因此 reduce 要求 binary op 同時滿足 associative 與 commutative,否則行為未定;
//         (3) 只有 reduce 有 execution policy 多載,可寫 std::reduce(std::execution::par, ...)。
//     追問:reduce 的 init 可以省略嗎?(可以,省略時等同以元素型別 value-initialize 的值)
//
// ⚠️ 陷阱. 不加 execution policy 的 std::reduce,是不是就等於 accumulate?
//     答:不是。「順序未定」是 reduce 的介面語意,與有沒有傳 policy 無關 —— 即使不帶 policy
//         的多載,標準也允許實作任意分組與重排。所以拿 reduce 做字串串接、減法這類非交換
//         運算,一開始就是錯的寫法,不能因為「我沒開平行、現在跑起來是對的」就當作安全。
//     為什麼會錯:把「順序未定」誤當成只有平行時才會發生的執行期現象,
//         但它其實是編譯期就成立的契約。
//
// 🔥 Q2. 為什麼浮點數用 reduce 和 accumulate 可能得到不同結果?
//     答:因為浮點加法不滿足結合律 —— (a+b)+c 與 a+(b+c) 的捨入誤差不同。reduce 允許任意
//         結合方式,所以同一份資料在不同執行下都可能有微小差異;需要可重現結果的場合要留意。
//
// Q3. 想真的跑平行版需要什麼?
//     答:include <execution> 並傳執行策略。注意「能編譯」不等於「真的平行」——
//         libstdc++ 目前把平行策略建在 Intel TBB 之上、連結時常需 -ltbb,
//         但這是 implementation-defined、標準未規定。
// ═══════════════════════════════════════════════════════════════════════════
...
// 編譯: g++ -std=c++20 -Wall -Wextra reduce.cpp -o reduce
