...
// ═══════════════════════════════════════════════════════════════════════════
// 【面試題】std::adjacent_difference
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. adjacent_difference 的輸出長什麼樣?第 0 個元素為什麼不是 0?
//     答:out[0] = in[0] (直接拷貝)、out[i] = in[i] - in[i-1]。第 0 個保留原值是刻意設計 ——
//         為的是保住與 partial_sum 的互逆性:有這個起點,partial_sum 才能完整還原原序列。
//         在 <numeric>,恰好 N-1 次運算,C++20 起 constexpr。
//     追問:那要取「真正的差分」時要注意什麼?(通常從 index 1 開始用,別把 out[0] 當差值)
//
// 🔥 Q2. 它和 partial_sum 是什麼關係?
//     答:互為逆運算,像離散版的微分與積分:partial_sum(adjacent_difference(v)) == v,
//         反向亦然。實務對應是「累計確診 → 每日新增」用 adjacent_difference,
//         「每日銷售 → 累計銷售」用 partial_sum。
//
// ⚠️ 陷阱. 傳自訂 BinaryOp 時,兩個參數的順序是什麼?
//     答:是 op(當前元素, 前一個元素),即 out[i] = op(in[i], in[i-1])。所以
//         std::divides<>{} 得到的是 in[i] / in[i-1] (相鄰比值),不是反過來。
//     為什麼會錯:依「相鄰」的語感假設成 op(前, 後),用減法時剛好正負顛倒,
//         而結果看起來仍然「像」差分序列,不容易第一時間發現。
//
// Q3. 平行化的支援上,partial_sum 和 adjacent_difference 一樣嗎?
//     答:不一樣,這點相當反直覺。adjacent_difference 在 C++17 有 execution policy 多載,
//         partial_sum 卻沒有 —— 想要平行前綴和,必須改用 inclusive_scan / exclusive_scan。
//         道理上也說得通:差分每個位置只依賴相鄰兩項、天生可切塊;前綴和則有跨位置的依賴。
// ═══════════════════════════════════════════════════════════════════════════
