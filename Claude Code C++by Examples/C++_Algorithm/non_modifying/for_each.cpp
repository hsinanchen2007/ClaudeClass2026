...
// 【面試題】std::for_each
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. std::for_each 的回傳值是什麼?有什麼用?
//     答:非平行版回傳它的 functor(by value,標準規定為 std::move(f))。
//         因為 functor 是「按值傳入」的,函式內部對狀態的改動不會反映回呼叫端,
//         所以標準把最終狀態當回傳值交還給你 —
//         這是 C++11 之前用有狀態 functor 蒐集統計(計數、加總)的慣用法。
//     追問:現在還常用嗎?(少見了 — 有 lambda 捕獲外部變數後通常更直觀)
//
// 🔥 Q2. for_each 保證套用順序嗎?和 std::transform 差在哪?
//     答:for_each 保證「由前往後依序」對 [first, last) 的每個元素套用 f。
//         std::transform 則不保證套用順序,所以它的 op 不應有副作用。
//         因此「有副作用、且順序有意義」的操作要用 for_each,不要用 transform。
//     追問:那和 range-based for 怎麼選?(多數情況 range-for 更清楚;
//           for_each 的價值在於能指定子區間、以及能接執行策略)
//
// ⚠️ 陷阱. 加上 C++17 的執行策略之後,上面兩個性質還在嗎?
//     答:都不在了。執行策略多載回傳 void(不再回傳 functor),
//         而且平行執行本來就不保證順序。
//         所以「靠回傳的 functor 拿累計結果」與「依序產生副作用」這兩種寫法,
//         一旦改成 std::execution::par 就會失效,甚至有資料競爭。
//     為什麼會錯:多數人以為加 policy 只是「同一個函式跑得比較快」,
//         沒注意到它是簽章與保證都不同的另一組多載。
//
// Q4. 為什麼 for_each 被歸類在「非修改型」,但 f 卻可以改元素?
//     答:分類講的是「演算法本身」不改動元素 — 它只是把元素交給 f。
//         若 f 以參考接收並修改,那是 f 的行為。
//         不過要表達「修改 / 映射」的意圖,用 std::transform 語意更精準。
// ═══════════════════════════════════════════════════════════════════════════
