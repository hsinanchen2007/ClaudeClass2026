...
// 【面試題】std::search_n
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. search_n 和 search、count 分別解決什麼問題?
//     答:search 找「一段指定的子序列」(按順序的多個、可不同的元素,例如 "ABC");
//         search_n 找「同一個值『連續』出現 n 次」的第一段(例如 "000");
//         count 只算「總共出現幾次」,完全不要求連續、也不給位置。
//         「連續」是 search_n 的核心限制 — 只要總數請用 count。
//     追問:找到時回傳什麼?(指向該連續段「第一個元素」的 iterator;找不到回 last)
//
// Q2. count 參數為 0 或負數時會怎樣?
//     答:回傳 first(C++20 起標準明確規定此行為)。
//         也就是「要求連續 0 個」被視為在開頭立即滿足,而不是視為找不到。
//         在此之前各家實作的行為未必一致,寫可攜程式碼時應自己先擋掉非正的 count。
//
// ⚠️ 陷阱 Q3. 述詞版 search_n(first, last, count, value, p) 的 p 收到的是哪兩個東西?
//     答:是 p(element, value) — 「區間中的元素」和「你傳入的那個 value」,
//         不是像 adjacent_find 那樣比較「相鄰的兩個元素」。
//         所以要找「連續 3 個大於 90 的值」得寫成
//         search_n(b, e, 3, 90, std::greater<>{}),由 p(element, 90) 判斷。
//     為什麼會錯:同屬「連續 / 相鄰」主題,
//         很容易把它和 adjacent_find 的二元述詞語意搞混。
// ═══════════════════════════════════════════════════════════════════════════
