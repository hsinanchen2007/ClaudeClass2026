...
// 【面試題】std::mismatch
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. mismatch 回傳什麼?可以從中推導出哪些資訊?
//     答:回傳 std::pair<It1, It2> — 分別是兩段區間中「第一個不相等」的位置。
//         由它可推出:共同前綴長度 = distance(first1, p.first)、
//         差異處的兩個值 = *p.first 與 *p.second。
//         若整段都相等,p.first 會等於 last1。
//     追問:和 std::equal 怎麼選?(只要 yes/no 用 equal;要知道「差在哪」用 mismatch —
//           兩者掃描邏輯相同,差別只在想拿回什麼)
//
// 🔥 Q2. 三參數版的 mismatch 為什麼危險?
//     答:三參數版 mismatch(first1, last1, first2) 假設第二段長度 >= 第一段,
//         第二段較短時會越界讀取,是 UB。
//         C++14 起加入四參數版 mismatch(first1, last1, first2, last2),
//         會同時看兩端邊界,任一端到底就停下。新程式碼一律用四參數版。
//     追問:std::equal 有同樣的問題嗎?(有,而且是同一批 C++14 的安全多載一起補上的)
//
// Q3. 四參數版「兩段完全相等且等長」時,回傳的 pair 是什麼?
//     答:p.first == last1 且 p.second == last2。
//         注意若只用三參數版,你只能判斷 p.first == last1,
//         無法確認第二段是不是還有剩,所以「相等」的判定要小心。
// ═══════════════════════════════════════════════════════════════════════════
