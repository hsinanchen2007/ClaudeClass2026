...
// 【面試題】std::find_end
// ───────────────────────────────────────────────────────────────────────────
// ⚠️ 陷阱 Q1. find_end 的 "end" 是什麼意思?它和 std::search 差在哪?
//     答:"end" 不是指「子序列的尾端」,也不是「從尾端找單一元素」—
//         它指的是「子序列在主序列中『最後一次』出現」。
//         std::search 找第一次出現、find_end 找最後一次出現;
//         兩者的參數、回傳值(都指向該次出現的『開頭』)、複雜度幾乎一致。
//     為什麼會錯:命名直覺會把 find_end 讀成 "find the end",
//         又容易和 std::string::rfind 的「反向找單一字元」混在一起。
//         實際上它的對照組是 search,不是 find。
//
// ⚠️ 陷阱 Q2. 子序列為空時,find_end 和 search 各回傳什麼?
//     答:不一樣 — find_end 回傳 last,std::search 回傳 first。
//         也就是說 search 把「空 pattern」視為在開頭就匹配成功,
//         find_end 則不給你這種技術性命中。這是兩者最常被拿來考的邊界差異。
//     為什麼會錯:大家假設「同一族演算法的邊界語意應該一致」,
//         但這裡剛好相反,只能分開記。
//
// 🔥 Q3. find_end 的複雜度是多少?為什麼標準庫不用 KMP?
//     答:最多 S × (N - S + 1) 次比較(N 為主序列長、S 為子序列長),空間 O(1)。
//         標準庫採用的是通用的樸素比對,因為演算法必須對任意 ForwardIterator 與
//         任意可比較型別成立,無法假設字母表有限或能事先建索引表。
//     追問:那 search 有加速手段嗎?(有 — C++17 為 search 加了 searcher 物件多載
//           (Boyer-Moore 等);但 find_end 並沒有對應的 searcher 多載)
//
// Q4. 要取「路徑最後一個 '/' 之後的檔名」,find_end 是最好的選擇嗎?
//     答:能做到,但若對象是 std::string,std::string::rfind 通常更直接。
//         find_end 的價值在於它適用於「任何 iterator 區間」與「多元素子序列」,
//         而不只是字串裡的單一字元。
// ═══════════════════════════════════════════════════════════════════════════
