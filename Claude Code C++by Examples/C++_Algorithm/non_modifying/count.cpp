...
// ═══════════════════════════════════════════════════════════════════════════
// 【面試題】std::count / std::count_if
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. 只想判斷「有沒有符合的元素」,為什麼不該寫 count_if(...) > 0?
//     答:count_if 沒有短路 — 它必須算出確切個數,所以永遠走完整個區間,
//         恰好做 last - first 次述詞呼叫。
//         只要 yes/no 的話 any_of 找到第一個就停,平均快得多;
//         想同時知道「在哪裡」則用 find_if。
//     追問:那「至少 k 個」呢?(count_if 一樣會走完全程;要提早退出得自己寫迴圈)
//
// 🔥 Q2. std::count 的回傳型別是什麼?有什麼要注意?
//     答:是 iterator_traits<It>::difference_type,一般即 std::ptrdiff_t —
//         這是「有號」整數,不是 size_t。
//         因此拿去和 v.size()(無號)比較會觸發 -Wsign-compare,
//         需要明確 static_cast 再比。
//     追問:為什麼標準選有號型別?(它是「兩個 iterator 相減」的型別,差值本來就可能為負)
//
// ⚠️ 陷阱. 對 std::set / std::map 用 std::count 有什麼問題?
//     答:std::count 是線性 O(n);而 set::count() / map::count() 成員函式走樹是 O(log n)。
//         multiset 的成員 count(k) 是 O(log n + 該 key 出現次數),同樣優於通用版。
//         通則和 find 一樣:關聯容器優先用同名成員函式。
//     為什麼會錯:很多人以為「演算法是通用的,套哪個容器都一樣」,
//         但通用演算法只看得到 iterator,用不到容器底層的樹或雜湊結構。
// ═══════════════════════════════════════════════════════════════════════════
...
// 編譯: g++ -std=c++20 -Wall -Wextra count.cpp -o count
