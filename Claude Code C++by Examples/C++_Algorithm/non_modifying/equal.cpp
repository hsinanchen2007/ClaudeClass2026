...
// 【面試題】std::equal
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. std::equal 只傳三個 iterator 有什麼風險?
//     答:三參數版 equal(first1, last1, first2) 「假設」第二段至少和第一段一樣長,
//         標準不會幫你檢查。若第二段較短就會越界讀取 — 未定義行為(UB)。
//         C++14 起新增四參數版 equal(first1, last1, first2, last2),
//         會自己看兩端長度,不同就直接回 false,安全得多,應優先使用。
//     追問:同樣的雙區間安全版本還加給了誰?(std::mismatch 與 std::is_permutation)
//
// 🔥 Q2. 四參數版在什麼情況下可以 O(1) 就回答?
//     答:當兩端都是 random access iterator 時,實作可以先 O(1) 比長度,
//         長度不同直接回 false,連一次元素比較都不做。
//         若只是 input / forward iterator 就沒有這個捷徑,
//         仍是最多 min(N1, N2) 次比較。
//     追問:三參數版有這個最佳化嗎?(沒有 — 它根本不知道第二段有多長,
//           這正是它不安全的原因)
//
// ⚠️ 陷阱. 兩個空區間比較,std::equal 回傳什麼?
//     答:true。兩段都沒有元素,就沒有任何一對配不起來,自然「相等」。
//         這和 all_of 的 vacuous truth 是同一種邏輯。
//     為什麼會錯:直覺上會覺得「沒東西可比,應該回 false 或無意義」,
//         但 equal 問的是「有沒有任何一對不相等」,空集合上找不到反例。
//
// Q4. 比較浮點數容器可以直接用預設的 std::equal 嗎?
//     答:不建議。預設用 operator==,浮點數受表示誤差影響常常不精確相等。
//         應傳一個帶容差的 BinaryPred(三參數與四參數版都有對應多載)。
// ═══════════════════════════════════════════════════════════════════════════
...
// 編譯: g++ -std=c++20 -Wall -Wextra equal.cpp -o equal
