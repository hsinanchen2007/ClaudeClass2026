...
// ═══════════════════════════════════════════════════════════════════════════
// 【面試題】std::is_heap / std::is_heap_until
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. is_heap 和 is_heap_until 的差別?複雜度?
//     答:兩者是同一個演算法、只差在回傳的資訊量。is_heap 回 bool
//     (整段是否為合法 heap);is_heap_until 回迭代器,指向「第一個破壞 heap
//     property 的位置」,因此 is_heap(f, l) 等價於 is_heap_until(f, l) == l。
//     兩者都是 O(N) 比較、O(1) 空間。兩個函式都是 C++11 加入,C++20 起為
//     constexpr。
//     追問:知道破壞位置有什麼用?(答:debug 時能直接定位;也可用來量化「已
//     heap 化的前綴長度」,例如批次 push_back 之後判斷要從哪裡開始重建。)
//
// 🔥 Q2. 為什麼「驗證是不是 heap」只要 O(N)?
//     答:heap property 是純粹的局部條件 — 只需對每個索引 i 檢查它與子節點
//     2i+1、2i+2 的關係,總共約 N 次比較就能涵蓋全部父子對,不需要任何排序或
//     遞迴。這也呼應 make_heap 為何能做到 O(N):建堆與驗堆都只處理局部關係。
//     追問:那實務上該怎麼用?(答:當作不變式檢查 —— 在 debug build 用
//     assert(std::is_heap(...)),release 可條件編譯掉,避免每次操作多付 O(N)。)
//
// ⚠️ 陷阱一. 一個「升序排好」的 vector,is_heap 回傳什麼?
//     答:預設(max-heap)回 false,除非長度 ≤ 1。升序陣列的根是最小值,
//     馬上違反「父 ≥ 子」。反過來**降序**陣列才是合法的 max-heap,
//     而升序陣列是合法的 min-heap(即 is_heap(f, l, std::greater<>{}) 為 true)。
//     為什麼會錯:很多人把 heap 想成「排序的弱化版」,於是推論「排好序的當然
//     也算 heap」—— 方向搞反了。
//
// ⚠️ 陷阱二. 空範圍或單元素範圍,is_heap 回傳什麼?
//     答:**true**(vacuous truth)。沒有任何父子對可以違反條件,自然成立。
//     這也是「從空容器開始逐一 push_back + push_heap」能維持不變式的起點。
//     順帶一提:comp 必須與 make_heap / push_heap / pop_heap 用的是同一個,
//     用預設 less 建的 heap 卻拿 greater 去驗,結果毫無意義。
// ═══════════════════════════════════════════════════════════════════════════
