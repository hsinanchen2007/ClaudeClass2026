...
// 【面試題】std::transform
// ---------------------------------------------------------------------------
// 🔥 Q1. std::transform 有哪兩種 overload?
//     答:一元版 transform(first1, last1, d_first, unary_op) 對每個元素套函式後寫到目的;
//         二元版 transform(first1, last1, first2, d_first, binary_op) 對兩個區間逐對運算。
//         二元版只用第一個區間的長度決定次數,所以第二個區間長度必須 >= 第一個,
//         否則越界 UB(標準不會幫你檢查)。
//
// 🔥 Q2. transform 可以原地(in-place)操作嗎?
//     答:可以。標準明確允許 d_first 等於 first1,所以
//         std::transform(v.begin(), v.end(), v.begin(), f); 是合法的。
//         但只允許「完全對齊」這一種重疊;目的區間若與來源以其他偏移量部分重疊就是 UB。
//
// ⚠️ 陷阱. 在 transform 的 lambda 裡放一個遞增計數器,可靠嗎?
//     答:不可靠。標準不保證套用順序,所以 unary_op / binary_op 應該是無副作用的純函式。
//         需要「依序」且有副作用的操作請用 std::for_each(它保證由前往後),
//         或改用 std::generate 這類本來就以狀態為前提的演算法。
//     為什麼會錯:單執行緒的 libstdc++ 實際上就是一個由前往後的迴圈,測起來永遠會過;
//         但那是實作行為不是標準保證,換成 C++17 的平行 policy 立刻出事。
//
// Q3. 目的端空間不足怎麼辦?transform 和 for_each 何時各用哪個?
//     答:空間不足就用 std::back_inserter(out) 讓它邊寫邊 push_back,或先 out.resize()。
//         選擇原則:需要「由舊值算出新值並寫出去」用 transform;
//         只是「對每個元素做一件事」用 for_each。
//         想「映射 + 累加一次完成」則用 C++17 的 std::transform_reduce。
// ===========================================================================
