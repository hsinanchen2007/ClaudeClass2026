...
// ===========================================================================
// 【面試題】std::fill / std::fill_n
// ---------------------------------------------------------------------------
// 🔥 Q1. std::vector<int> v(5, 0); 和 std::fill(v.begin(), v.end(), 0); 差在哪?
//     答:前者是「建構」——配置記憶體並初始化 5 個元素,容器這時才誕生;
//         後者是「覆寫」——容器已存在,只是把既有 5 個位置的值改掉。
//         關鍵差異:fill 完全不會改變 size(),也不會幫你配置空間。
//
// 🔥 Q2. fill 和 fill_n 的差別?回傳值各是什麼?
//     答:fill(first, last, v) 用兩個 iterator 描述範圍,回傳 void;
//         fill_n(first, n, v) 用「起點 + 個數」描述,回傳最後寫入位置的下一個
//         (即 first + n),n <= 0 時什麼都不寫、直接回傳 first。
//         fill_n 搭配 std::back_inserter 就是「動態追加 n 個相同值」的標準寫法。
//
// ⚠️ 陷阱. 對空 vector 呼叫 fill_n(v.begin(), 10, 0) 會怎樣?
//     答:UB(越界寫入)。fill 家族和 copy 家族一樣不會擴容,begin() 到 end() 是空區間。
//         正解:先 v.resize(10),或用 std::fill_n(std::back_inserter(v), 10, 0)。
//     為什麼會錯:fill_n 只吃「起點 + 個數」,看起來像在說「我要 10 格」,
//         讓人誤以為它會負責生出這 10 格。個數只決定寫幾次,空間得你自己準備好。
//
// Q3. 為什麼 fill 對 int 陣列通常比手寫迴圈快?
//     答:對 trivially copyable 的型別,標準函式庫的實作常會把它特化成 memset 或
//         SIMD 指令。但這是實作品質(QoI)的最佳化,標準只保證「恰好 N 次指派」,
//         並未規定要用 memset。對自訂型別則是逐一呼叫 operator=,沒有這個好處。
// ===========================================================================
...
// 編譯: g++ -std=c++20 -Wall -Wextra fill.cpp -o fill
