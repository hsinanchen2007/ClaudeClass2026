...
// 【面試題】std::swap_ranges
// ---------------------------------------------------------------------------
// 🔥 Q1. swap_ranges 的前置條件有哪些?違反會怎樣?
//     答:(1) 兩段範圍「不可重疊」,重疊是 UB;(2) 第二段長度必須 >= 第一段——演算法只用
//         [first1, last1) 決定次數,第二段短了就越界寫入,而且標準完全不檢查。
//         回傳值是第二段中「最後被交換元素的下一個」,即 first2 + (last1 - first1)。
//         複雜度恰好 N 次 swap,O(n)。
//
// 🔥 Q2. 想交換「兩個容器的全部內容」,該用 swap_ranges 還是 v1.swap(v2)?
//     答:用成員 swap。容器的 swap 只換內部指標,是 O(1);swap_ranges 是逐元素交換,
//         O(n)。swap_ranges 的定位是「交換兩段子區間」,或是在兩個不同型別的容器
//         之間交換一段內容(兩個 iterator 型別允許不同)。
//     追問:兩個容器元素個數不同時,成員 swap 還能用嗎?(可以,size 也一起換過去)
//
// Q3. swap_ranges 和 iter_swap 的關係?
//     答:swap_ranges 概念上就是對範圍中每一對位置呼叫 iter_swap
//         (逐一交換兩個 iterator 指向的元素)。iter_swap 是單點 O(1) 的零件,
//         swap_ranges 是它的區間版。
// ===========================================================================
