...
// 【面試題】std::shuffle
// ---------------------------------------------------------------------------
// 🔥 Q1. std::random_shuffle 為什麼被淘汰了?
//     答:C++14 被標記為 deprecated、C++17 正式移除。原因:它依賴 rand(),
//         品質差又不是執行緒安全;而且它允許使用者傳入自製的 RNG 函式,
//         那些自製 RNG 多半有 modulo bias 等問題。取而代之的 std::shuffle(C++11 加入)
//         強制要求傳入一個 URBG。
//     追問:std::shuffle 的第三個參數必須符合什麼概念?
//
// 🔥 Q2. std::shuffle 需要什麼參數和什麼迭代器?
//     答:必須傳入一個 UniformRandomBitGenerator(URBG),例如 std::mt19937、
//         std::mt19937_64、std::random_device——不能傳 rand。
//         迭代器需求是 RandomAccessIterator,所以 std::list 不能直接洗,
//         得先複製到 vector 洗完再搬回去。複雜度 O(n)、空間 O(1)。
//
// Q3. 標準實作通常用什麼演算法?
//     答:典型是 Fisher-Yates(從尾端往前,對每個 i 取 [0, i] 的隨機索引 j 後 swap)。
//         它保證每種排列的機率相同,恰好 n-1 次 swap、每步一次隨機數。
//         自己手寫成「每個位置和 [0, n) 的隨機位置交換」則會產生不均勻的分佈,
//         是經典錯誤。
//
// ⚠️ 陷阱. 每次要洗牌時就在函式裡建一個新的 std::mt19937,有什麼問題?
//     答:預設建構的 mt19937 種子固定,所以每次呼叫都會得到「完全相同」的洗牌結果。
//         引擎應該建立一次並重複使用(例如 static 或以參數傳入),要真隨機就用
//         std::random_device 提供 seed。反過來說,要可重現的測試就刻意固定 seed。
//     為什麼會錯:大家把 engine 當成無狀態的工具函式。它其實是有狀態的序列產生器,
//         重建等於把序列倒回起點。另外 engine 不是執行緒安全的,
//         多執行緒環境每個執行緒該有自己的一份。
// ===========================================================================
