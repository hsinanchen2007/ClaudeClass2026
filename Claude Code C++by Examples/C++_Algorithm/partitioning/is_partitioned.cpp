...
// ═══════════════════════════════════════════════════════════════════════════
// 【面試題】std::is_partitioned
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. is_partitioned 檢查的是什麼？和 is_sorted 差在哪？
//     答：它只回答「滿足 pred 的元素是否全部排在不滿足的元素之前」，
//         也就是 true...true | false...false 這個形狀，回傳 bool。
//         is_sorted 檢查的是元素之間的大小關係；is_partitioned 只看
//         一個一元述詞的真假分界，完全不涉及排序。
//         {6, 4, 2, 5, 3, 1} 依「是偶數」是已分割，但它並沒有排序。
//     追問：已排序的範圍一定是已分割的嗎？（對「x < value」這種述詞是的，
//           這正是 lower_bound 能用二分搜尋的理由）
//
// 🔥 Q2. 為什麼要有這個函式？誰會用到它？
//     答：它是 partition_point 的前置條件檢查器。partition_point 要求
//         輸入範圍「已依同一述詞分割」，違反就是 UB；is_partitioned 讓你
//         能在 assert 或除錯模式下驗證這個前提。另一個用途是驗收某個
//         分割步驟的結果形狀是否符合預期。
//     追問：正式產品裡每次都先呼叫它划算嗎？（不划算——它是 O(N)，
//           而 partition_point 本身才 O(log N)，等於白白抵銷掉優勢；
//           一般只放在 assert 或測試中）
//
// Q3. 複雜度與迭代器需求？
//     答：最多 N 次述詞呼叫，找到反例即可短路提早結束；空間 O(1)。
//         只需要 InputIterator，是整個 partitioning 家族裡要求最寬鬆的。
//         C++11 引入，C++17 起有執行策略多載，C++20 起為 constexpr。
//
// ⚠️ 陷阱. 空範圍、全部滿足、全部不滿足，這三種各回傳什麼？
//     答：三種**全部回傳 true**。空範圍是 vacuous truth；全部滿足等於
//         false 段為空；全部不滿足等於 true 段為空——都符合「滿足者
//         全在不滿足者之前」的定義。
//     為什麼會錯：多數人腦中的模型是「要真的看到兩段才算分割」，
//         於是直覺答空範圍是 false 或全滿足是 false。定義是一條
//         「不存在反例」的全稱命題，邊界情況自然成立。
// ═══════════════════════════════════════════════════════════════════════════
