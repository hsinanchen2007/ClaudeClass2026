...
// ═══════════════════════════════════════════════════════════════════════════
// 【面試題】std::partition_copy
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. partition_copy 和 partition 的根本差別是什麼？
//     答：partition 是**原地重排**，改變原資料，得到一個分好組的容器；
//         partition_copy（C++11 起）**不修改來源**，而是把滿足 pred 的
//         元素複製到第一個輸出範圍、不滿足的複製到第二個輸出範圍。
//         想保留原資料（稽核、還要再用），或想得到兩個實體分開的容器時，
//         就用 partition_copy。
//     追問：回傳值是什麼？（std::pair，分別是兩個輸出範圍的寫入結束位置；
//           兩段長度相加等於來源長度）
//
// 🔥 Q2. 為什麼不直接呼叫兩次 copy_if 就好？
//     答：兩次 copy_if 要走訪來源兩遍、做 2N 次述詞呼叫，而且第二次還得
//         自己把述詞取反（容易寫錯、或述詞有副作用時語意不同）。
//         partition_copy 一次掃描就完成，**恰好 N 次述詞呼叫 + N 次指派**。
//         語意上也更清楚：它明確表達「分流成互斥的兩組」。
//     追問：只需要其中一組時呢？（那就用 copy_if，別為了對稱而多寫一個
//           永遠不看的輸出容器）
//
// Q3. 對輸出範圍有什麼要求？
//     答：兩個目的端**都必須有足夠空間**——演算法只是往 output iterator 寫，
//         不會也不能幫你 resize，空間不足就是 buffer overrun（UB）。
//         而且兩個目的端彼此不可重疊，也不可與來源範圍重疊。
//         實務上最省事的作法是配 std::back_inserter，讓容器自己長大。
//
// ⚠️ 陷阱. 先 dst.reserve(src.size()) 再 partition_copy(..., dst.begin(), ...)
//          安全嗎？
//     答：**不安全，是 UB**。reserve 只給 capacity，size 仍是 0，
//         begin() == end()，寫進去就越界。要嘛用 resize 真的把元素造出來，
//         要嘛改用 back_inserter。
//     為什麼會錯：腦中的錯誤模型是「reserve 過了記憶體就在那裡，寫進去
//         應該沒事」——但 vector 的 size 才是它認知的有效元素數，
//         繞過 size 直接寫 capacity 區域，容器本身完全不知情。
//         這和 std::copy 寫進空 vector 是同一個經典陷阱。
// ═══════════════════════════════════════════════════════════════════════════
