...
// ═══════════════════════════════════════════════════════════════════════════
// 【面試題】auto 回傳型別推導（C++14）
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. 「auto 當回傳型別」是 C++11 還是 C++14？C++11 能寫嗎？
//     答：C++14。C++11 只有 trailing return type（auto f() -> int），箭頭後仍要
//         明寫型別或寫 decltype；直接寫 auto f() { return 1; } 在 C++11 是編譯錯。
//         唯一例外：lambda 的回傳型別推導 C++11 就有，但一般函式沒有。
//     追問：C++11 想讓回傳型別依賴參數怎麼辦？（auto f(T a, U b) -> decltype(a + b)）
//
// 🔥 Q2. auto 回傳型別有哪些限制？
//     答：(1) 多條 return 必須推導出完全相同的型別，int 與 double 混用即編譯錯；
//         (2) virtual 函式不能用（deduced return type 與 vtable 佈局衝突）；
//         (3) 呼叫端必須先看到「定義」，只有宣告 auto f(); 就呼叫會錯；
//         (4) return {1, 2}; 這種 braced-init-list 推導不出來。
//     追問：(3) 的實務後果？（不能宣告放 .h、定義藏 .cpp，等於逼你把實作攤在標頭）
//
// Q3. 遞迴函式可以用 auto 回傳型別嗎？
//     答：可以，但編譯器必須先看到一個「不遞迴的 return」把型別定下來。
//         把 if (n <= 0) return 0; 寫在遞迴呼叫之前就合法，順序反過來則編譯錯。
//
// ⚠️ 陷阱. auto f() 會原樣回傳 c[i] 的 reference 嗎？
//     答：不會。auto 回傳型別套用的是「auto 變數」的推導規則——丟棄 top-level
//         const 與 reference，永遠回傳「值」。要保留 reference 得用 decltype(auto)。
//     為什麼會錯：多數人把 auto 讀成「編譯器自動填上正確型別」，但它其實是
//         「以值語意推導」。寫轉發 wrapper 時會靜默變成拷貝，呼叫端改不到原物件，
//         而且不會有任何警告——是最典型的「編得過但語意錯」。
// ═══════════════════════════════════════════════════════════════════════════
