...
// ═══════════════════════════════════════════════════════════════════════════
// 【面試題】實務 logger 與 I/O 選擇
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. 寫 logger 時 cout、cerr、clog 怎麼選？endl 該不該用？
//     答：診斷 log 走 clog（有緩衝、效能好）或檔案；致命錯誤走 cerr（unitbuf，即使
//     程式隨後崩潰也保證看得到）。不要在每行 log 都用 endl——那是每行一次 flush，
//     在高頻 log 上是數量級的成本。折衷做法是平常用 '\n'，只在關鍵節點（例如即將
//     abort 前）顯式 flush。
//     追問：程式被 signal 殺掉時緩衝的 log 會怎樣？（會遺失，因為沒有機會 flush；
//     這正是「崩潰前最後幾行 log 不見了」的成因）
//
// 🔥 Q2. 多執行緒中 std::cout << a << b; 安全嗎？
//     答：要分兩層看。不會 data race——標準規定對標準流物件的同步寫入不產生 data race
//     （前提是未解除 sync_with_stdio）。但會交錯：a << b 是兩次獨立的 operator<< 呼叫，
//     中間可能被其他執行緒插入，於是輸出變成穿插的亂序。解法是用 mutex 保護整條語句、
//     先組成單一字串再一次輸出，或用 C++20 的 std::osyncstream（解構時原子地一次輸出）。
//
// ⚠️ 陷阱. 用 ofstream 寫 log，程式 abort() 之後為什麼檔案是空的？
//     答：因為緩衝區內容還沒寫到磁碟。ofstream 的解構函式會 flush，但 abort() 與
//     _exit() 不會執行解構函式與 atexit 處理。要保證 log 落地，關鍵訊息必須顯式 flush，
//     或改用 unitbuf。
//     為什麼會錯：以為「寫進 ofstream 就等於寫進檔案」，中間其實隔著一層使用者空間緩衝。
// ═══════════════════════════════════════════════════════════════════════════
