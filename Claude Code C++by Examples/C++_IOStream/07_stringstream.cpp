...
// ═══════════════════════════════════════════════════════════════════════════
// 【面試題】stringstream 字串流
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. stringstream 有哪些典型用途？
//     答：① 解析——istringstream iss(line); iss >> a >> b >> c;（配合 getline 逐行讀，
//     是最穩健的輸入模式）② 格式化組裝——ostringstream oss; oss << ...; auto s = oss.str();
//     ③ 分割字串——getline(iss, token, ',') 逐個切 CSV 欄位。注意 .str() 回傳的是一份
//     拷貝（C++20 起有右值重載可以 move）。
//     追問：效能如何？（比 std::from_chars／to_chars 慢很多——有 locale、虛擬函式、可能
//     配置，熱路徑上別用）
//
// 🔥 Q2. 為什麼建議「逐行 getline + istringstream 解析」而不是直接 cin >> ？
//     答：因為它把「切行」與「解析欄位」分開：某一行格式錯誤時，錯誤被侷限在那一行的
//     stringstream 裡，主 stream 的位置仍停在乾淨的行邊界上，不會像直接 >> 那樣讓壞字元
//     卡在緩衝區汙染後續讀取。
//
// ⚠️ 陷阱. 重複使用同一個 stringstream，為什麼第二次就失效？
//     答：第一次解析讀到底會設 eofbit（或格式錯誤設 failbit）。只呼叫 ss.str("") 換掉
//     內容，狀態位元還在，於是後續所有讀寫都被忽略。必須兩件事都做：
//     ss.str("");   // 換內容
//     ss.clear();   // 清掉 eofbit / failbit ← 最常被忘記
//     更不易錯的做法是直接在迴圈內建立新的 stringstream。
//     為什麼會錯：以為 str("") 是「重設整個物件」，實際上它只換了緩衝區內容，狀態機的
//     狀態完全沒動。
// ═══════════════════════════════════════════════════════════════════════════
