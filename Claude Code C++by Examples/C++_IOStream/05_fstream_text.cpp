...
// ═══════════════════════════════════════════════════════════════════════════
// 【面試題】ifstream / ofstream 文字檔讀寫
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. 檔案開啟失敗怎麼判斷？為什麼預設不拋例外？
//     答：fstream 家族預設不拋例外，開檔失敗只設 failbit。判斷方式：if (!f) （檢查
//     fail()）或 if (!f.is_open())（語意更明確），兩者在開檔失敗這個情境下等價。要改成
//     拋例外可用 f.exceptions(std::ios::failbit | std::ios::badbit)。
//     追問：為什麼標準選擇不拋？（開檔失敗是可預期的常規狀況，屬於「用回傳狀態表達」
//     比較合適的那一類錯誤，也與 C 的錯誤碼哲學相容）
//
// 🔥 Q2. fstream 需要手動 close() 嗎？
//     答：不需要——它是標準的 RAII 型別，解構時自動 close() 並 flush。但有兩個情境值得
//     手動關：① 想提早釋放（例如接著要重新命名或刪除該檔）② 想檢查關檔錯誤——解構函式
//     不能拋例外，所以解構時 flush 失敗（磁碟滿、網路檔案系統斷線）會被靜默吞掉。
//     資料完整性要求高時應寫 f.close(); 再用 if (!f) 檢查關檔階段是否失敗。
//
// ⚠️ 陷阱. while (!file.eof()) { file >> x; process(x); } 為什麼是錯的？
//     答：eofbit 是在「嘗試讀取並失敗之後」才被設定的，不是「還剩 0 個字元時」就設。
//     所以最後一次讀取失敗（C++11 起 x 被設為 0），但迴圈已經進來了，於是多處理一筆
//     垃圾資料。正確寫法是把讀取本身當條件：while (f >> x) { ... } 或
//     while (std::getline(f, line)) { ... }，這樣同時涵蓋 eof、格式錯誤、裝置錯誤。
//     為什麼會錯：把 eof() 想成「還有沒有資料」的預測，它其實是「上一次讀取有沒有撞到
//     結尾」的回顧。
// ═══════════════════════════════════════════════════════════════════════════
