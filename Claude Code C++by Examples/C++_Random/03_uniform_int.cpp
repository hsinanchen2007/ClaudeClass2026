...
//   * 均勻性有標準保證，不像 rand() % N 會有 modulo bias
//   * 表達意圖清楚：[a, b]、含上界
//
//  ⚠️ 但【跨平台重現性不在優點清單裡】——這是 distribution 的已知限制，不是
//  賣點。標準只規定「分布」，從來沒有規定任何版本的 distribution 要用什麼
//  演算法（不是「C++17 之後才這樣」）。所以：
//    * Engine 可跨實作重現：mt19937 演算法由標準完整規定，同 seed 同序列
//      （本機實測第 10000 個輸出 = 4123659995，正是標準指定值）
//    * Distribution 不可跨實作重現：各家演算法不同，同 engine 同 seed 會得到
//      不同數值。本機以計數 engine 實測 libstdc++ 的 normal_distribution：
//      第 1 次抽取消耗 4 個 engine 輸出、第 2 次消耗 0 個（它快取了 polar
//      method 成對產生的第二個值）——這種快取純屬實作選擇，換一家實作就變。
//  需要跨平台重現（遊戲 replay、科學模擬、測試）時：用標準 engine，但
//  distribution 自己實作或固定用某個第三方實作。詳見 07_seeding.cpp Q2。
//
//  ┌────────────────────────────────────────────────────────────┐
