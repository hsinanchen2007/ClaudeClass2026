...
// ═══════════════════════════════════════════════════════════════════════════
// 【面試題】<utility> 總覽（ref / reference_wrapper 與移動語意）
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. 為什麼需要 std::ref／std::reference_wrapper？
//     答：因為 std::bind、std::thread、std::make_pair、std::async 這些設施會對引數做
//     decay 並以「值」儲存——即使你寫的是參考，它們也會複製一份。
//     std::reference_wrapper<T> 是可複製、可賦值的「參考」包裝（內部存指標），讓參考語意
//     能進入這些以值儲存的場合，也能放進 std::vector（原生參考不行）。
//     追問：std::thread t(f, x); 想傳參考怎麼寫？（std::thread t(f, std::ref(x));
//     而且要自行確保被參考的物件活得比執行緒久）
//
// 🔥 Q2. 為什麼 std::vector<std::reference_wrapper<T>> 可以，std::vector<T&> 不行？
//     答：容器要求元素可賦值、可取位址，而原生參考不可重新綁定（a = b 是改被參考物的
//     值，不是改綁定），本身也不是物件。reference_wrapper 內部存指標，因此可複製、可
//     賦值（重新綁定），又能隱式轉換到 T&，用起來像參考。
//     追問：reference_wrapper 的 operator= 是什麼語意？（重新綁定到另一個物件，不是賦值
//     給被參考物——這與原生參考的行為相反，是易錯點）
//
// ⚠️ 陷阱. v = std::move(v);（自我移動賦值）安全嗎？
//     答：對標準函式庫型別而言不會 UB、不會崩潰，物件之後處於「有效但未指定」狀態
//     （常見結果是變空）。但自己寫移動賦值時必須處理自賦值：典型的「先釋放自己的資源、
