...
// ═══════════════════════════════════════════════════════════════════════════
// 【面試題】建構子的 function-try-block
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. 什麼是 function-try-block？它存在的唯一理由是什麼？
//     答：把整個函式本體——包含建構函式的成員初始化列表——包成一個 try 區塊的語法：
//     Widget::Widget(int n) try : data_(n) { ... } catch (const std::bad_alloc&) { ... }
//     它存在的唯一理由是：成員初始化列表拋出的例外，用一般寫在函式本體內的 try-catch
//     是接不到的。
//     追問：實務上該用嗎？（很少；主要用途是把底層例外轉譯成自家的例外型別）
//
// 🔥 Q2. 建構函式的 function-try-block 有哪三個關鍵限制？
//     答：① catch 區塊到達結尾會「隱式 rethrow」（或你自己 throw 新例外）——不能吞掉
//     例外讓物件「建構成功」，因為那個物件已經無法有效存在 ② catch 區塊中不能存取成員
//     或基底（它們已被解構），所以幾乎只能記 log 或轉換例外型別 ③ 對非建構／解構的
//     普通函式，function-try-block 的 catch 可以正常 return、不會隱式 rethrow（但那種
//     寫法幾乎沒有比一般 try-catch 更好的地方）。
//
// ⚠️ 陷阱. 建構函式拋出例外時，該物件的解構函式會被呼叫嗎？
//     答：不會——物件從未「完整建構」，語言認為它不存在。但所有「已完整建構」的基底類別
//     子物件與成員變數會以相反順序被正確解構。對 new T 而言，若 T 的建構函式拋例外，
//     編譯器會自動呼叫對應形式的 operator delete 釋放那塊原始記憶體，所以那塊記憶體
//     本身不會洩漏；真正會洩漏的是「建構函式內部自己取得、卻尚未交給 RAII 成員管理」的
//     資源。結論：建構函式裡取得的每項資源都應立即交給 RAII 成員。
//     為什麼會錯：以為「建構失敗 ⇒ 解構會來收尾」，於是把清理邏輯全放在解構函式裡。
// ═══════════════════════════════════════════════════════════════════════════
