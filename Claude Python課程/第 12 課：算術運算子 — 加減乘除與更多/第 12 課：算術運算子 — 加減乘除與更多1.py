...
# ───────────────────────────────────────────────────────────────────────────
# 🔥 Q1. `/`、`//`、`%`、`**` 各是什麼？
#     答：`/` 真除法，**恆回傳 float**（實測 `4/2` → `2.0`）；
#     `//` 向下取整除法；`%` 取餘數；`**` 次方。
#     Python 2 的 `/` 對兩個整數是整除，這是最常見的遷移雷。
#     追問：想要「整數相除得整數」該用哪個？（`//`，但注意負數行為）
#
# 🔥 Q2. `-7 // 2` 等於多少？`-7 % 2` 呢？
#     答：實測 `-7 // 2` → **-4**（不是 -3！`//` 是**向下取整 floor**，不是截斷）；
#     `-7 % 2` → **1**（結果符號跟**除數**走，與 C/Java 給 -1 不同）。
#     追問：為什麼要這樣設計？（保證 `a == (a//b)*b + a%b` 恆成立，
#     而且模運算結果非負，拿來當索引特別方便）
#
# ⚠️ 陷阱. `int(-3.99)` 也是 -4 嗎？
#     答：**不是**，實測是 **-3**。`int()` 對浮點數是**向零截斷**，
#     `//` 是**向下取整**——對負數兩者方向剛好相反，對正數才碰巧一樣。
#     為什麼會錯：把「取整」當成單一概念，
#     沒意識到 truncate、floor、round 是三種不同的取整方式。
# ═══════════════════════════════════════════════════════════════════════════
...
# 執行: python3 第 12 課：算術運算子 — 加減乘除與更多1.py
