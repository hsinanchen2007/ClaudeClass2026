...
# ═══════════════════════════════════════════════════════════════════════════
# 【面試題】整數 int 與浮點數 float
# ───────────────────────────────────────────────────────────────────────────
# 🔥 Q1. int 與 float 的核心差別？為什麼 0.1 + 0.2 != 0.3？
#     答：Python 的 `int` 是**任意精度**（不會溢位，只受記憶體限制）；
#     `float` 是 IEEE 754 雙精度**二進位**浮點，0.1 與 0.2 都無法精確表示。
#     實測 `0.1 + 0.2` → `0.30000000000000004`，`== 0.3` → False。
#     追問：那要怎麼安全比較浮點？（`math.isclose()`；金額改用 `decimal.Decimal`）
#
# 🔥 Q2. 為什麼 4 / 2 得到 2.0 而不是 2？
#     答：Python 3 的 `/` **恆回傳 float**，即使整除也一樣。
#     要得到整數結果用 `//`（向下取整）。這是 Python 2 → 3 最常見的遷移雷。
#     追問：`7 // 2` 與 `-7 // 2` 分別是多少？（3 與 -4，見下面的陷阱）
#
# ⚠️ 陷阱. round(0.5) 等於多少？
#     答：實測 **0**，而 `round(1.5)` → 2、`round(2.5)` → 2。
#     這是 **banker's rounding（四捨六入五成雙）**：遇到剛好 .5 時進位到最近的
#     偶數，目的是消除大量捨入造成的統計偏差。
#     為什麼會錯：直覺套用小學的「四捨五入」，就會預期 0.5 → 1、2.5 → 3。
#     要傳統四捨五入得用 decimal 模組指定 ROUND_HALF_UP。
# ═══════════════════════════════════════════════════════════════════════════
...
# 執行: python3 第 6 課：基本資料型別（一）—— 整數 int 與浮點數 float1.py
