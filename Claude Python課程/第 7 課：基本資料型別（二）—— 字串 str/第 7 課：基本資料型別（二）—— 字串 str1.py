...
# 【面試題】字串 str 與不可變性
# ───────────────────────────────────────────────────────────────────────────
# 🔥 Q1. 字串為什麼是不可變的？「修改」字串時發生什麼事？
#     答：每次「修改」都是**建立新物件**。實測 `a = 'abc'; a += 'd'` 之後
#     `id(a)` 改變；`'abc'[0] = 'X'` 直接拋
#     `TypeError: 'str' object does not support item assignment`。
#     追問：`'aaa'.replace('a','b')` 會就地改嗎？（不會，回傳新字串，原字串不變）
#
# 🔥 Q2. split() 與 join() 怎麼用？
#     答：`split(sep)` 把字串切成 list（`'a,b'.split(',')` → `['a','b']`）；
#     `sep.join(iterable)` 把可迭代物件接成字串（`'-'.join(['a','b'])` → `'a-b'`）。
#     易錯點：`join` 是**字串**的方法不是 list 的，且元素必須全都是 str。
#     追問：`'a b  c'.split()` 不給參數有何不同？（依任意空白切分並忽略連續空白）
#
# ⚠️ 陷阱. 在迴圈裡用 `s += x` 累加字串有什麼問題？
#     答：因為字串不可變，每次 `+=` 都要配置新物件並複製舊內容，
#     整個迴圈是 O(n²)。正確做法是先收集到 list，最後一次 `''.join(parts)`。
#     為什麼會錯：`+=` 的寫法看起來像「就地追加」，讓人以為成本是 O(1)；
#     實際上對不可變型別，`+=` 只能重新綁定到一個新物件。
# ═══════════════════════════════════════════════════════════════════════════
...
# 執行: python3 第 7 課：基本資料型別（二）—— 字串 str1.py
