...
# ═══════════════════════════════════════════════════════════════════════════
# 【面試題】串列 List（三）：常用方法與切片
# ───────────────────────────────────────────────────────────────────────────
# 🔥 Q1. `list.sort()` 與 `sorted()` 差在哪？
#     答：`L.sort()` **就地排序**、回傳 **None**、只能用在 list；
#     `sorted(x)` 回傳**新的 list**、原物件不變、接受**任何可迭代物件**
#     （實測 `sorted((3,1,2))` → `[1,2,3]`，型別是 list 不是 tuple）。
#     追問：Python 的排序穩定嗎？（**穩定**——實測相同 key 的元素
#     維持原有相對順序，這是官方文件明訂的**語言保證**，不是實作細節）
#
# 🔥 Q2. 切片 `L[start:stop:step]` 的規則？
#     答：**左閉右開**，step 可為負。實測 `L=[0,1,2,3,4]`：
#     `L[::-1]` → `[4,3,2,1,0]`（反轉）、`L[1:100]` → `[1,2,3,4]`
#     （**切片越界不報錯**）、`L[10:]` → `[]`。
#     追問：`L[:]` 是什麼？（淺拷貝——外層是新 list，內層元素仍共用）
#
# ⚠️ 陷阱. `L = L.sort()` 之後 L 是什麼？
#     答：**None**。`sort()` 就地排序並回傳 None，
#     賦值回去等於把整個 list 弄丟了，而且**不會報錯**——
#     直到後面某行對 None 做操作才爆炸，離真正的錯誤位置很遠。
#     為什麼會錯：套用「函式都會回傳結果」的直覺。
#     正解：要就地排序寫 `L.sort()`（不賦值），
#     要新 list 寫 `L = sorted(L)`。
# ═══════════════════════════════════════════════════════════════════════════
...
# 執行: python3 第 27 課：串列 List（三）— 常用方法與切片1.py
