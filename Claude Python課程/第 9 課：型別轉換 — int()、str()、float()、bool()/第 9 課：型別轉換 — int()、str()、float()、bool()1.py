...
# ═══════════════════════════════════════════════════════════════════════════
# 【面試題】型別轉換 int()／str()／float()／bool()
# ───────────────────────────────────────────────────────────────────────────
# 🔥 Q1. 隱式與顯式型別轉換差在哪？
#     答：隱式＝Python 自動提升，如 `1 + 2.0` → `3.0`（int 併入 float）；
#     顯式＝自己呼叫 `int()`、`str()`、`float()`、`bool()`。
#     因為 Python 是強型別，**不會**在 str 與 int 之間隱式轉換。
#     追問：`"5" * 3` 會怎樣？（實測 `'555'`——字串重複，不是乘法，這正是
#     忘記轉型時最難察覺的 bug：它不報錯）
#
# 🔥 Q2. input() 回傳什麼型別？
#     答：**永遠是 str**，即使使用者輸入的是數字。
#     所以要做算術一定得先 `int(input())` 或 `float(input())`，
#     否則 `'5' * 3` 會得到 `'555'` 而不是 15。
#     追問：使用者輸入非數字怎麼辦？（用 `try/except ValueError` 包起來）
#
# ⚠️ 陷阱. `int('3.5')` 會得到什麼？
#     答：實測 **ValueError: invalid literal for int() with base 10: '3.5'**。
#     `int()` 只接受**整數字面值的字串**，小數點字串會直接失敗；
#     要轉得先 `float('3.5')` 再 `int(...)`。
#     為什麼會錯：以為 `int()` 會「幫忙取整」，把它想成 `int(float(x))` 的簡寫。
#     實際上對**字串**它是嚴格剖析，對**浮點數**才是截斷。
# ═══════════════════════════════════════════════════════════════════════════
...
# 執行: python3 第 9 課：型別轉換 — int()、str()、float()、bool()1.py
