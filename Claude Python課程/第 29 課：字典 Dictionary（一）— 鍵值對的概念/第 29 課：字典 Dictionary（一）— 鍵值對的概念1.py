...
# ═══════════════════════════════════════════════════════════════════════════
# 【面試題】字典 Dictionary（一）：鍵值對
# ───────────────────────────────────────────────────────────────────────────
# 🔥 Q1. dict 是有序的嗎？
#     答：**看版本**。3.6 因新雜湊表實作而保有插入序，
#     但官方當時僅視為**實作細節**、不可依賴；
#     **3.7 起正式納入語言規範**，所有相容實作都必須保證插入序。
#     實測 3.14.4：依 `z`、`a`、`m` 插入後 `list(d)` → `['z','a','m']`。
#     追問：那 `OrderedDict` 還有用嗎？（有——它的 `==` **考慮順序**，
#     還提供 `move_to_end()`；一般 dict 的 `==` 忽略順序，
#     實測 `{'z':1,'a':2} == {'a':2,'z':1}` → True）
#
# 🔥 Q2. 為什麼 list 不能當 dict 的 key？什麼叫 hashable？
#     答：dict 靠 **hash 值**定位。可變物件一旦被修改，hash 就變了，
#     原本存進去的東西再也找不到——所以 Python 直接禁止。
#     實測 `{[1,2]}` → `TypeError: unhashable type: 'list'`。
#     追問：`frozenset` 呢？（可 hash，實測 `{frozenset([1,2])}` 成立）
#
# ⚠️ 陷阱. 「字典是無序的」這句話現在還對嗎？
#     答：**不對了**。這是 Python 3.7 之前的說法。
#     大量中文舊資料仍這樣寫，面試講出來會被認為知識過期。
#     精確的講法是：**3.7 起 dict 保證插入序**（語言保證），
#     而 **set 到今天仍然不保證任何順序**。
#     為什麼會錯：把「dict 與 set 都是雜湊結構」推論成「兩者都無序」。
#     兩者的順序保證完全不同，必須分開記。
# ═══════════════════════════════════════════════════════════════════════════
...
# 執行: python3 第 29 課：字典 Dictionary（一）— 鍵值對的概念1.py
...
# 0987-654-321
# dict_keys(['小明', '小華'])
# dict_values(['0987-654-321', '0923-456-789'])
# dict_items([('小明', '0987-654-321'), ('小華', '0923-456-789')])
# {'小明': '0987-654-321'}
# {}
# <class 'dict'>
# {'小明': '0912-345-678', '小華': '0923-456-789', '小美': '0934-567-890'}
