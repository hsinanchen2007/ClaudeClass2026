...
# ───────────────────────────────────────────────────────────────────────────
# 🔥 Q1. list 有哪些核心特性？
#     答：**可變、有序、可重複、可混型別**，用 `[]` 建立、用索引存取。
#     索引從 0 開始，支援負索引（`L[-1]` 是最後一個）。
#     因為可變，所以 list **不可 hash**，不能當 dict 的 key 或 set 的元素。
#     追問：那什麼時候該改用 tuple？（元素固定不變、或需要當 key 時）
#
# 🔥 Q2. 索引越界與切片越界的行為一樣嗎？
#     答：**完全不同**。實測 `L=[0,1,2,3,4]`：
#     索引 `L[10]` → **IndexError**；
#     切片 `L[1:100]` → `[1,2,3,4]`、`L[10:]` → `[]`，**都不報錯**。
#     追問：這樣設計的好處是什麼？（切片可以安全地「取前 n 個」
#     而不必先檢查長度）
#
# ⚠️ 陷阱. `[[]] * 3` 建出來的三個空 list 是獨立的嗎？
#     答：**不是**。實測 `g = [[]] * 3; g[0].append('X')` 之後
#     三個都變成 `['X']`——`*` 複製的是參考，它們是同一個物件。
#     正解是推導式 `[[] for _ in range(3)]`（實測各自獨立）。
#     為什麼會錯：把 `*` 理解成「複製內容 n 份」。
#     對不可變元素（如 `[0]*3`）看不出差別，一旦內層是可變物件就出事。
# ═══════════════════════════════════════════════════════════════════════════
...
# 執行: python3 第 25 課：串列 List（一）— 建立與存取1.py
...
# [0, 0, 0, 0, 0]
# ['空', '空', '空', '空', '空', '空', '空', '空']
# [[85, 92, 78, 90], [88, 76, 95, 82], [91, 85, 89, 94]]
# 蘋果
# 芒果
# 西瓜
# 西瓜
