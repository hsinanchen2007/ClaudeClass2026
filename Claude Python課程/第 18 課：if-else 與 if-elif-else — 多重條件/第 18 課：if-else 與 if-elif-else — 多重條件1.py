...
# ═══════════════════════════════════════════════════════════════════════════
# 【面試題】if-else 與 if-elif-else
# ───────────────────────────────────────────────────────────────────────────
# 🔥 Q1. `elif` 與「連續寫好幾個 if」差在哪？
#     答：`if/elif` 鏈是**互斥**的——一旦某條成立，後面全部跳過；
#     連續的獨立 `if` 則每條都會被檢查、可能同時成立好幾條。
#     分級距（成績、票價、稅率）一定要用 elif，否則會被後面的條件覆蓋掉。
#     追問：那 elif 的順序重要嗎？（非常重要，見下面的陷阱）
#
# 🔥 Q2. Python 有 switch 嗎？
#     答：3.10 之前沒有，慣用 `if-elif-else` 或 dict 分派
#     （`handlers.get(key, default)()`，O(1) 查找又能動態註冊）。
#     3.10 起有 **`match`/`case`**（結構化模式比對，比 switch 強大）。
#     注意 `match` 是 **soft keyword**——實測 3.14.4 仍可拿它當變數名。
#     追問：dict 分派相較 if-elif 有什麼優勢？（查找是 O(1)、
#     而且能在執行期新增分支）
#
# ⚠️ 陷阱. 分級距時把範圍最大的條件寫在最前面會怎樣？
#     答：後面的分支永遠不會被執行。例如先寫 `if score >= 60:` 再寫
#     `elif score >= 90:`，那 95 分會在第一條就被攔截，判成「及格」而非「優等」。
#     為什麼會錯：以為 elif 會「挑最符合的那一條」。
#     實際上它是**由上而下、遇到第一個為真就結束**，
#     所以條件必須由嚴格到寬鬆排列。
# ═══════════════════════════════════════════════════════════════════════════
...
# 執行: python3 第 18 課：if-else 與 if-elif-else — 多重條件1.py
