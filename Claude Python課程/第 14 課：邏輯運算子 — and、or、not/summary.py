...
# 【面試題】邏輯運算的組合與可讀性
# ───────────────────────────────────────────────────────────────────────────
# 🔥 Q1. `is not` 與 `not ... is` 一樣嗎？`not in` 呢？
#     答：語意相同，但 `is not`／`not in` 是**單一運算子**，
#     PEP 8 明確建議用它們。`x is not None` 比 `not x is None` 好讀太多。
#     追問：`not x is None` 為什麼容易誤讀？
#     （容易被讀成 `(not x) is None`；雖然實際優先序讓它等於前者，
#     但「需要停下來想一下」本身就是該避免的寫法）
#
# 🔥 Q2. `and` 與 `or` 的優先序誰高？
#     答：**`and` 高於 `or`**（`not` 又高於 `and`）。
#     實測 `True or False and False` → **True**，
#     因為它是 `True or (False and False)`，不是 `(True or False) and False`。
#     追問：不確定時最好的做法？（加括號——執行結果不變，但讀的人不用猜）
#
# ⚠️ 陷阱. `not True == False` 是什麼？
#     答：實測 **True**。因為 `==` 的優先序**高於** `not`，
#     整個式子是 `not (True == False)` = `not False` = True，
#     而不是直覺上的 `(not True) == False`（那也是 True，但推理過程完全不同）。
#     為什麼會錯：以為 `not` 是「緊貼著後面那個名字」的一元運算子。
#     實際上比較運算子綁得比它更緊。
# ═══════════════════════════════════════════════════════════════════════════
