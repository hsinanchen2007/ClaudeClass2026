...
# ═══════════════════════════════════════════════════════════════════════════
# 【面試題】邏輯運算子 and / or / not
# ───────────────────────────────────────────────────────────────────────────
# 🔥 Q1. `and` 與 `or` 回傳什麼？
#     答：**不是布林值，而是運算元本身**（短路求值）。
#     實測 `0 or 'a'` → `'a'`、`'x' and 'y'` → `'y'`、`[] or {}` → `{}`。
#     規則：`or` 回傳第一個 truthy（都假就回最後一個）；
#     `and` 回傳第一個 falsy（都真就回最後一個）。
#     追問：那 `not` 呢？（`not` 一律回傳真正的 bool，這點與 and/or 不同）
#
# 🔥 Q2. 什麼是短路求值？有什麼實際用途？
#     答：`A and B` 在 A 為假時**不會求值 B**；`A or B` 在 A 為真時不求值 B。
#     用途一是效能，用途二是**安全防護**：
#     `if d and d['k']` 或 `if i < len(L) and L[i] > 0` 靠短路避免例外。
#     追問：把它改寫成巢狀 if 還安全嗎？（安全，但可讀性差；短路正是為此而生）
#
# ⚠️ 陷阱. `x = a or default` 這個慣用法什麼時候會出錯？
#     答：當 `a` 是**合法但 falsy 的值**時。`a = 0`、`a = ''`、`a = []`
#     都會被誤判成「沒給值」而套用 default。
#     正解：`x = a if a is not None else default`。
#     為什麼會錯：把「falsy」與「未提供」畫上等號。
#     0 是一個完全合法的數值，空字串也可能是使用者真正的輸入。
# ═══════════════════════════════════════════════════════════════════════════
...
# 執行: python3 第 14 課：邏輯運算子 — and、or、not1.py
