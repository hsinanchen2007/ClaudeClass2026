...
# 【面試題】賦值運算子與就地修改
# ───────────────────────────────────────────────────────────────────────────
# 🔥 Q1. `x += y` 與 `x = x + y` 完全一樣嗎？
#     答：**對可變物件不一樣**。對 list，`+=` 會呼叫 `__iadd__` **就地修改**，
#     實測 `id` 不變；`x = x + y` 則建立**新物件**，`id` 改變。
#     對不可變的 tuple／str，`+=` 只能重新綁定，實測 `id` 一定改變。
#     追問：這對「函式內修改參數」有何影響？（`+=` 改的是呼叫端看得到的
#     同一個 list；重新賦值則只動到區域名稱，外面看不到）
#
# 🔥 Q2. Python 的多重賦值與交換變數是怎麼運作的？
#     答：`a, b = b, a` 右邊先被求值成一個 tuple，再拆包給左邊，
#     所以不需要暫存變數，也不會有「先覆蓋掉 a」的問題。
#     `a = b = 0` 則是把**同一個物件**綁到兩個名字上。
#     追問：`a = b = []` 有什麼風險？（兩個名字指向同一個 list，
#     改其中一個另一個也會變）
#
# ⚠️ 陷阱. `a = [1]; b = a; b += [2]` 之後 `a` 是什麼？
#     答：實測 `a` 也變成 `[1, 2]`——因為 `+=` 就地修改了那個 list，
#     而 a 與 b 指向同一個物件。
#     但若寫成 `b = b + [2]`，`a` 仍是 `[1]`（b 被重新綁到新物件）。
#     為什麼會錯：把賦值想成「複製一份值過去」。
#     Python 的賦值只是「多一個名字指向同一個物件」。
# ═══════════════════════════════════════════════════════════════════════════
