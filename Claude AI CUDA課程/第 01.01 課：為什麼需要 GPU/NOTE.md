...
> ⚠️ **這裡的 N 是「理論處理器數」，不是效能預測**。40960 是**常駐執行緒數**，
> 但 GPU 的執行緒**不等於 40960 個獨立、對等的處理器**：warp 內 32 條 lock-step 前進、
> 每個 SM 每 cycle 能發的指令有限、還有 occupancy 與記憶體停頓。
> 所以 **36864× 是「把 N 代進公式」的示意值，不是這張卡能跑出來的加速比**。
> 這兩條定律要看的是**趨勢與天花板的形狀**，不是拿來報數字。
