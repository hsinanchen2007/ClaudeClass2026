// =============================================================================
//  第 22 課：const 成員函數 7  —  本課講義 + 綜合實作
// =============================================================================
//
//  本檔結構：
//    (1) 開頭的 /* ... */ 區塊 = 第 22 課完整講義（Markdown 原文）
//    (2) 講義之後 = 可執行的綜合實作
...
//     void g(const T& obj);                 // const 參考參數(最常用的傳參法)
//   語意：把隱含的 this 由 T* const 變成 const T* const,
//         因此函式內對成員的任何寫入都會在編譯期被拒絕。
...
// 【詳細解釋 Explanation】
//
// 【1. 本課的核心命題:const 是契約,而且由編譯器執行】
//   在成員函式尾端加上 const,不是寫給人看的註解,而是修改了隱含 this
//   的型別。之後任何 member = value 都等價於 this->member = value,
//   而 this 指向 const 物件,於是編譯直接失敗。
//   「承諾」與「強制」在此合而為一 —— 這是 C++ 型別系統最實用的設計之一。
//
// 【2. const 正確性決定「這個型別能不能用」】
//   C++ 生態預設以 const& 傳遞物件。若某個查詢函式漏了 const,
//   該型別在所有 const 語境下就完全不可用 —— 不是慢,是編不過。
//   而且 const 會傳染:底層漏標,上層整條鏈跟著不能是 const。
//   修法一律「由內而外」補 const,不是在外層用 const_cast 硬過。
//
// 【3. 判準只有一條:有沒有改變可觀察狀態】
//   不是「函式短不短」、不是「裡面有沒有運算」。
//   一個組字串、跑迴圈、做浮點運算的函式,只要不寫成員,就該是 const。
//   反之,一個只有一行 ++counter_ 的函式,再短也不能是 const。
//
// 【4. const 的邊界:它只有一層深】
//   const 成員函式保證不改「本物件的非 mutable 直接成員」,
//   但若成員是指標 T* p,它只保護 p 本身,*p 的內容照樣能改。
//   const 成員函式也照樣可以做 I/O、改全域變數、改 static 成員。
//   所以 const ≠ 純函式,更 ≠ 執行緒安全的萬用保證。
//
// 【概念補充 Concept Deep Dive】
//   * const 資格會被編入 mangled name,因此 const 版與非 const 版在
//     連結器眼中是兩個不同符號 —— 這是兩者能構成重載的底層原因。
//   * 定義在類別內部的 const getter 隱含 inline;開 -O2 後通常被完全展開,
//     與直接存取 public 成員的機器碼相同。const 正確性不帶執行期代價。
//   * 標準庫保證「同時對同一物件呼叫 const 成員函式」不會產生資料競爭,
//     但這個保證的前提是型別確實遵守 const 語意 —— 這也是下一課
//     (第 23 課 mutable)必須格外謹慎的原因。
//
// 【注意事項 Pay Attention】
//   1. 講義區塊(/* ... */)內的程式碼不會被編譯;要改行為請改講義之後的實作。
//   2. const 寫在參數列之後;宣告與定義分離時兩邊都要寫,否則連結失敗。
//   3. 建構子與解構子不可為 const。
//   4. 回傳「算出來的新值」時要回傳值,不可回傳區域變數的參考
//      (那是懸空參考,屬未定義行為,不保證任何特定結果)。
//   5. const 只有一層深;要保護指標指向的內容,需宣告成 const T*。
//
// ═══════════════════════════════════════════════════════════════════════════
// 【面試題】const 成員函數(本課綜合)
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. 成員函式尾端的 const 改變了什麼?為什麼它是「強制」而非「約定」?
//     答：它把隱含的 this 從 T* const 變成 const T* const。
//         函式內寫 member = v 實際上是 this->member = v,
//         而 this 指向 const 物件,所以編譯期直接失敗。
//         這不是靠團隊自律,是型別系統的規則 —— 違規的程式碼
//         根本產生不出可執行檔。
//     追問：有執行期成本嗎?→ 完全沒有。const 是純編譯期資訊,
//         不產生額外指令,也不改變物件佈局。
//
// 🔥 Q2. 怎麼判斷一個成員函式該不該加 const?
//     答：唯一判準是「它會不會改變物件的可觀察狀態」。
//         會改 → 不能加;不會改 → 就該加,不管函式裡面多複雜。
//         常被誤判的是「衍生查詢」:那種要做運算、組字串、跑迴圈才算得出
//         結果的函式,直覺上「好像在做事」,但只要不寫成員,就該是 const。
//     追問：那需要快取計算結果的 const 函式怎麼辦?→ 用 mutable 成員,
//         那是「邏輯 const」的範疇,也是第 23 課的主題。
//
// ⚠️ 陷阱. 「函式標了 const,就代表它不會改到任何東西。」
//     答：不對,範圍比想像的小很多。const 只保證不改「*this 的非 mutable
//         直接成員」。它照樣可以:做 I/O、改全域變數、改 static 成員、
//         改 mutable 成員,以及在成員是 T* 時透過它修改所指向的內容
//         —— 因為 const 只有一層深,保護的是指標本身而非目標。
//     為什麼會錯：把 const 讀成「純函式 / 無副作用 / 執行緒絕對安全」。
//         實際上它的作用域嚴格限定在「本物件的直接成員」這一層,
//         其餘一切都不在保護範圍內。
// ═══════════════════════════════════════════════════════════════════════════
//
// 【LeetCode 實戰範例】—— 從缺,理由如下
//   本檔是整課講義 + 綜合實作,篇幅已足。與本課主題真正相關的一題
//   (705. Design HashSet:contains 是查詢 → 該為 const,
//   add/remove 是修改 → 不能為 const)已在同課 summary.cpp 完整示範,
//   在此重複只會稀釋本檔主線,故不重覆掛題。
//
// =============================================================================
...
// 編譯: g++ -std=c++17 -Wall -Wextra "第 22 課：const 成員函數7.cpp" -o l22_7
// 執行: ./l22_7        (rc=0)
