// =============================================================================
//  第 21 課：getter 與 setter 設計模式 7  —  本課總整理 + 裝備強化系統實作
// =============================================================================
//
//  本檔結構：
//    (1) 開頭的 /* ... */ 區塊 = 第 21 課完整講義（Markdown 原文）
//    (2) 講義之後 = 可執行的綜合實作：裝備強化系統
//  講義區塊內的程式碼片段是「教學用示意」，真正會被編譯的是講義之後那份。
//
// 【主題資訊 Information】
//   本課涵蓋的介面設計手法:
//     T            getX() const;                 // 回傳值:小型別
//     const T&     getX() const;                 // 回傳唯讀視圖:大型別
//     bool         doSomething(args);            // 帶驗證的行為(取代 setter)
//     Self&        setX(v) { ...; return *this; }// 鏈式呼叫
//   標準版本:全部為 C++98 起即有的語法;本檔以 C++17 編譯。
//   複雜度:本檔所有存取器與行為函式皆 O(1)。
//   標頭檔:<iostream>、<string>
//
// 【詳細解釋 Explanation】
//
// 【1. 本課的核心命題:getter/setter 不是樣板碼,是介面設計】
//   整課從頭到尾在回答同一個問題——「外界到底需要對這個成員做什麼?」
//   而不是「這個成員要配幾個存取器?」。答案有四種,對應四種寫法:
//     需要讀,且型別小        → 回傳值的 getter
//     需要讀,且型別大        → 回傳 const& 的 getter
//     需要改,但有規則要維護  → 不給 setter,給有名字的行為函式
//     完全是內部細節          → 兩個都不給
//   本檔的 Equipment 類別把四種情況一次示範完。
//
// 【2. 從「資料語言」走向「領域語言」】
//   setPower(int) 是資料語言:它描述你動了哪個欄位。
//   enhance() 是領域語言:它描述使用者做了什麼事。
//   差別在於,enhance() 有地方可以放「強化上限」「成功率」「失敗提示」
//   這些規則;setPower() 只能把規則推給每一個呼叫端各自實作。
//   本檔的強化系統正是這個轉換的完整示範:外界只能呼叫 enhance(),
//   不可能繞過上限直接把攻擊力設成 9999。
//
// 【3. 驗證要放在「唯一入口」】
//   rename("") 被攔截、enhance() 超過上限被拒絕,這些檢查之所以有效,
//   是因為它們是修改該狀態的唯一途徑。只要多開一個 int& getPower(),
//   上述所有檢查就全部失效——這是本課第 3 號檔案示範過的教訓。
//
// 【4. 稀有度用 enum class 而非 int】
//   Rarity 是 enum class(C++11 起的強型別列舉),它不會隱式轉成 int,
//   也不會污染外層命名空間。這同樣是封裝思維:讓不合法的值
//   「在編譯期就寫不出來」,比在執行期檢查更好。
//
// 【概念補充 Concept Deep Dive】
//   * 定義在類別內部的 getter 隱含 inline,開 -O2 後通常被完全展開,
//     與直接存取 public 成員的機器碼相同。封裝在此不帶執行期成本。
//   * rarityToString() 用回傳 std::string 而非 const char*,
//     每次呼叫會建構一個 string(可能觸發小字串最佳化 SSO 而不配置 heap)。
//     libstdc++ 的 SSO 門檻是 15 bytes(實作定義值,標準未規定);
//     本檔的中文字串以 UTF-8 編碼,一個中文字元佔 3 bytes,
//     因此「傳說」這種兩字詞是 6 bytes,仍落在 SSO 範圍內。
//   * switch 對 enum class 做完整列舉時,若漏掉某個列舉值,
//     -Wall 會發出 -Wswitch 警告。本檔已涵蓋全部列舉值,故無警告。
//
// 【注意事項 Pay Attention】
//   1. 講義區塊(/* ... */)內的程式碼不會被編譯;修改那裡不會影響
//      實際執行結果。要改行為請改講義之後的實作。
//   2. 強化上限、成功與否等規則一律留在類別內;若日後要加「強化石消耗」
//      或「失敗降級」,也只該改 enhance() 一處。
//   3. 本檔沒有動態配置,不涉及拷貝/解構的資源管理問題;
//      那是第 27–29 課(淺拷貝/深拷貝、拷貝建構、拷貝賦值)的主題。
//   4. 用 enum class 時要記得它不能隱式轉成 int,需要輸出數值時
//      得明確 static_cast<int>(r)。
//
// ═══════════════════════════════════════════════════════════════════════════
// 【面試題】getter/setter 設計總結
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. 設計一個類別時,你怎麼決定某個成員要不要配 getter / setter?
//     答：先問「外界需要對它做什麼」,而不是「它是不是 private」。
//         需要讀 → 給 getter(小型別回傳值,大型別回傳 const&);
//         需要改但有規則 → 不給 setter,改提供有領域意義的行為函式,
//         把驗證與副作用收斂在那個唯一入口;
//         純內部細節 → 兩個都不給。機械式地每個成員都配一組 getter/setter,
//         等於把 private 成員原封不動公開,封裝的實質保護是零。
//     追問：那什麼情況 setter 反而是正確選擇?→ 純組態值,
//         例如視窗寬高、逾時毫秒數,本身沒有需要一起維護的不變式。
//
// 🔥 Q2. enum class 相較於傳統 enum 有什麼好處?為什麼封裝會偏好它?
//     答：(a) 強型別:不會隱式轉成 int,少了一整類「拿列舉去做算術」的錯誤;
//         (b) 有作用域:列舉值不會洩漏到外層命名空間,不會撞名;
//         (c) 可指定底層型別。對封裝而言最重要的是 (a) ——
//         它讓「不合法的值在編譯期就寫不出來」,比執行期檢查更早、更可靠。
//     追問：那要輸出它的數值怎麼辦?→ 明確寫 static_cast<int>(r);
//         這個「必須明講」正是強型別的目的。
//
// ⚠️ 陷阱. 「我全部成員都寫了 getter 和 setter,所以我的類別封裝做得很好。」
//     答：恰恰相反。若每個 private 成員都配上無條件的 getX()/setX(),
//         外界能做的事和成員直接宣告成 public 幾乎完全一樣,只是多繞一層
//         函式呼叫。封裝的價值不在「有沒有經過函式」,而在
//         「那個函式有沒有攔下不合法的狀態」。
//     為什麼會錯：把封裝理解成「語法上藏起來」(private + 存取器),
//         而不是「語意上守得住」(不變式有唯一守門人)。
//         判斷方式很簡單:如果每個 setter 內部都只有一行單純賦值,
//         那這個類別的封裝其實等於零。
// ═══════════════════════════════════════════════════════════════════════════
//
// 【LeetCode 實戰範例】—— 從缺,理由如下
//   本檔是整課的綜合實作(裝備強化系統),重點在完整領域模型的介面設計,
//   篇幅已足。與本課主題真正對應的兩題 LeetCode 已在同課檔案中示範:
//     * 第 3 號檔案 → 155. Min Stack(不變式需要唯一守門人)
//     * 第 4 號檔案 → 1603. Design Parking System(用行為取代 setter)
//   在此重複只會稀釋本檔的主線,故不重覆掛題。
//
// 【日常實務範例】—— 即本檔的主體
//   講義之後的裝備強化系統本身就是完整的實務範例:稀有度、強化上限、
//   改名驗證、裝備狀態,都是遊戲/電商後台真實會有的領域規則。
//
// =============================================================================
...
// 編譯: g++ -std=c++17 -Wall -Wextra "第 21 課：getter 與 setter 設計模式7.cpp" -o l21_7
// 執行: ./l21_7        (rc=0)
