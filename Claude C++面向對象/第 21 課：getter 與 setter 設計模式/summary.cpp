// =============================================================================
//  summary.cpp  —  第 21 課總複習：getter 與 setter 設計模式
// =============================================================================
//
// 【主題資訊 Information】
//   語法    ：T getX() const;          const T& getX() const;   void setX(T);
//   標準版本：C++98 起；本檔以 C++17 編譯
//   標頭檔  ：<iostream>、<string>、<vector>
//   複雜度  ：存取 O(1)；標頭中定義的短函式通常被 inline，屬零成本抽象
//
// 【詳細解釋 Explanation】
//
// 【1. 這一課真正的主題不是「怎麼寫 getter/setter」，而是「該不該寫」】
//   把成員設 private、再機械式地替每一個都配一組 get/set，
//   是這個模式最常見的誤用 —— 那只是把 public 成員換了個寫法，
//   多打了很多字，卻沒有得到任何保護。
//   getter/setter 的價值在於成為「類別不變量的唯一入口」：
//   不變量是「物件在任何時刻都必須成立的性質」
//   （血量不得超過上限、名字不得為空、餘額不得為負）。
//   只要所有修改都必須經過帶驗證的 setter，這些規則就不可能被破壞。
//   反過來說，若一個型別根本沒有不變量（例如單純的 Point{x, y}），
//   老實用 public 成員或 struct 反而更清楚。
//
// 【2. getter 的回傳方式：三選一，選錯後果差很多】
//   (a) 回傳值（int、double 等小型別）
//       複製成本極低，呼叫端拿到的是獨立的拷貝，改它不影響原物件。
//   (b) 回傳 const 參考（string、vector 等大型別）
//       避免深複製，同時保證呼叫端不能透過它修改內部狀態。
//   (c) 回傳非 const 參考 —— **這是本課最重要的反面教材**
//       `string& getName() { return name_; }`
//       呼叫端可以寫 `obj.getName() = "";`，
//       直接繞過所有驗證改到內部成員。封裝當場破功。
//       同理，回傳內部容器的非 const 參考、回傳成員指標、
//       回傳 .data()，都會把寫入權洩漏出去。
//   判準很簡單：檢查每個 public 函式的回傳型別，問
//   「呼叫端拿到它之後，能不能繞過驗證改到內部狀態？」能 → 封裝已破。
//
// 【3. 用「行為」取代 setter（本課最值得帶走的觀念）】
//   比較這兩種介面：
//       obj.setHp(obj.getHp() - 30);      // 貧血介面：呼叫端自己算
//       obj.takeDamage(30);               // 行為介面：類別自己負責
//   後者更好，理由有三：
//     * 呼叫端不必知道血量的內部規則（下限、護甲減免、無敵幀）；
//     * 相關邏輯集中在類別內，改規則只要改一個地方；
//     * 語意清楚 —— 讀程式碼的人看到的是「受到傷害」而非「設定數值」。
//   一個類別若只有 get/set 而沒有任何行為，通常代表職責放錯了地方
//   （邏輯散落在各個呼叫端），這種設計常被稱為「貧血模型」。
//
// 【4. 鏈式呼叫（Method Chaining）】
//   讓 setter 回傳 `*this` 的參考，就能寫成
//       config.setHost("localhost").setPort(8080).setTimeout(30);
//   要點是回傳 `T&`（非 const 參考、且回傳的是自己），
//   不是回傳副本 —— 回傳副本會導致每一步都作用在不同物件上。
//   這是 builder 模式的基礎，適合「參數很多、且多為選用」的設定型物件。
//
// 【概念補充 Concept Deep Dive】
//   getter/setter 有沒有執行期成本？在標頭檔中定義的短函式，
//   編譯器幾乎必定 inline，產生的機器碼與直接存取成員相同 ——
//   所以這是典型的「零成本抽象」，不必為了效能而放棄封裝。
//   真正的成本在介面層面：每公開一個 getter，就等於簽下一份長期契約。
//   `getHp()` 回傳 int 會被所有呼叫端依賴，
//   日後想改成浮點血量或加入減免計算，就得同時處理所有呼叫端。
//   這正是「不要無腦替每個成員開 getter/setter」的實際理由 ——
//   少開一個，就少綁住自己一次。
//
//   另一個值得注意的細節是 const 正確性的傳染性：
//   getter 沒標 const，`const T&` 就無法呼叫它；
//   而一旦某個函式忘了標 const，所有想以 const 參考傳遞該型別的地方
//   都會連鎖失敗。所以 const 應該從一開始就標好，事後補會很痛。
//
// 【注意事項 Pay Attention】
// 1. getter 一律加 const；大型成員回傳 const 參考。
// 2. **絕對不要**回傳非 const 參考、成員指標或內部容器的可寫入視圖，
//    那等於公開了成員（本檔第三節即為此反面示範）。
// 3. 回傳 const 參考時，呼叫端若用參考接住「暫時物件的成員」會懸空；
//    以值接住（`string n = obj.getName();`）即可避免。
// 4. 只做 `x_ = value;` 而無驗證的 setter，效果等同 public 成員；
//    真的不需要驗證時，該考慮直接用 public 或 struct。
// 5. 鏈式 setter 必須回傳 `*this` 的參考（T&），回傳副本會失效。
// 6. 本檔使用 `using namespace std;`，教學檔可接受，
//    標頭檔與大型專案應避免。
//
// ═══════════════════════════════════════════════════════════════════════════
// 【面試題】getter / setter 設計
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. getter 回傳「值」、「const 參考」、「非 const 參考」各有什麼影響？
//     答：回傳值適合基本型別，呼叫端拿到獨立拷貝，改它不影響原物件；
//         回傳 const 參考適合 string／vector 等大型別，避免深複製，
//         且呼叫端無法透過它修改內部；
//         回傳非 const 參考則等於把成員的寫入權整個交出去 ——
//         `obj.getName() = "";` 就能繞過所有驗證，封裝完全失效。
//     追問：那回傳 const 參考有沒有風險？
//         → 有懸空風險。若呼叫端寫
//           `const string& n = makeTemp().getName();`，
//           暫時物件在該完整運算式結束就銷毀，n 隨即懸空。
//           以值接住即可避免。
//
// 🔥 Q2. 什麼時候「不」應該提供 setter？
//     答：當這個欄位不該由外界任意指定時。例如等級應該由「獲得經驗」
//         這個行為推動、餘額應該由「存款／提款」推動、
//         訂單狀態應該由狀態機推動。
//         這些情況要提供的是「行為方法」（gainExp、withdraw、ship）
//         而不是 setter —— 讓規則留在類別內，而不是散落到每個呼叫端。
//     追問：那如果外界真的需要改呢？
//         → 先問「為什麼需要」。通常答案會指向一個更好的行為方法名稱。
//           若真的只是單純的資料欄位、且沒有任何不變量，
//           那反而該考慮直接用 public 成員 —— 包一層無驗證的 set
//           只是徒增噪音。
//
// ⚠️ 陷阱. 「我把所有成員都設成 private 並提供了完整的 getter/setter，
//           所以這個類別是良好封裝的物件導向設計。」
//     答：兩個層面都可能不成立。
//         (a) 技術層面：只要有任何一個 getter 回傳非 const 參考或指標，
//             封裝就已經破了 —— private 只擋住「用名字直接存取」，
//             擋不住你親手把內部參考遞出去。
//         (b) 設計層面：一個只有 get/set、沒有任何行為的類別，
//             其實只是「披著類別外衣的 struct」。所有業務規則
//             都被推給了呼叫端，這正是所謂的「貧血模型」，
//             封裝的形式有了，實質沒有。
//     為什麼會錯：把封裝理解成「語法上的 private + 配套存取器」，
//         而不是「把資料與操作它的規則放在一起」。
//         真正的判準是：業務規則寫在類別裡，還是散落在每個呼叫端？
//         若每個呼叫端都要自己記得「扣血後要夾到 0」，那就不是封裝。
// ═══════════════════════════════════════════════════════════════════════════
//
// -----------------------------------------------------------------------------
// 【LeetCode 實戰範例】—— 本檔不附（同課範例 1 已附 LeetCode 1603）
//   理由：本檔是設計原則的完整複習，涵蓋回傳方式、命名慣例、鏈式呼叫
//   等偏向「介面設計」的主題，難以對應到單一題目。
//   本課與 LeetCode 的真正交集（用 private 狀態維護不變量）
//   已在「第 21 課：getter 與 setter 設計模式1.cpp」中以
//   LeetCode 1603. Design Parking System 完整實作，此處不重複。
//   依規格「寧缺勿濫」從缺——本檔第七節的 RPG 裝備系統
//   本身即為整合所有概念的實務級範例。
// -----------------------------------------------------------------------------
...
int main() {
    ...
    cout << "\n\n======== 第二節：getter 的返回方式 ========" << endl;
    {
        ...
        gold = 0;                            // 修改的是拷貝
        // 把「拷貝」與「原物件」都印出來，差異才看得見
        //（順帶讓 gold 真的被使用，避免 -Wunused-but-set-variable 警告）
        cout << "  修改後的拷貝：" << gold << endl;
...
//    （實測連跑 5 次 md5 相同）。
// 2. 第二節「修改後的拷貝：0 / 實際金幣：500」這一組對照是本檔新補的：
//    原始版本只印了原物件，看不出「改拷貝不影響原件」這個重點，
//    而且該變數因未被使用而觸發 -Wunused-but-set-variable 警告。
//    現在兩個值都印出來，重點看得見，警告也消失了。
// 3. 第三節是刻意的反面示範：示範回傳「非 const 參考」如何讓呼叫端
//    繞過驗證直接改到內部狀態。請把它當成「不要這樣寫」的教材，
//    而不是可用的寫法。
// 4. 本檔全程沒有未定義行為，也沒有印出任何不確定值或位址。
...
//
// --- 傳統方式 ---
//   [警告] 你確定要刪除嗎？ (400x200) Visible:Yes
//
// --- 鏈式調用（一氣呵成）---
//   [歡迎] 歡迎回到遊戲世界！ (500x250) Visible:Yes
//
//
// ======== 第七節：綜合範例 RPG 裝備系統 ========
//
// --- 創建裝備 ---
//   [炎龍劍] 史詩 能力:50(基礎:50) 強化:0/20 背包中
//   [橡木盾] 普通 能力:30(基礎:30) 強化:0/5 背包中
//
// --- 裝備操作（行為取代 setter）---
//   裝備了 炎龍劍
//   炎龍劍 已經裝備了
...
//
// --- 最終狀態 ---
//   [烈焰魔劍 +5] 史詩 能力:75(基礎:50) 強化:5/20 已裝備
//   [橡木盾 +5] 普通 能力:45(基礎:30) 強化:5/5 已裝備
//
// ============================================================
//   複習完畢！核心原則：暴露最少的介面，優先用行為取代直接修改。
// ============================================================
