// =============================================================================
//  第 26 課 -11  —  this 指標綜合範例：RPG 隊伍系統
// =============================================================================
//
// 【主題資訊 Information】
//   this 是什麼 ： 每個非靜態成員函式隱含收到的指標，指向「呼叫它的那個物件」
//   *this       ： 解參考 this，得到物件本身（左值，型別 ClassName&）
//   本檔用 this 的六種應用（都在 Hero 類別中）：
//       應用 1 leader_ = this        把自己設為隊長
//       應用 2 this == &other        防止跟隨自己（身分比較）
//       應用 3 return *this          鏈式強化 boostHp().boostAttack()
//       應用 4 betterLeader          比較後回傳 *this 或 other（皆 const 參考）
//       應用 5 this == &target       判斷是否在自我治療
//       應用 6 leader_ == this       判斷自己是否為隊長
//   標準版本   ： this 自 C++98；本檔的 inline static 成員需 C++17
//   標頭檔     ： <string>
//
//   註：本檔開頭有一大段 /* ... */ 的課程講義（Markdown），那是說明文字、
//       不會被編譯；真正執行的是講義之後的 Hero 類別與 main()。
//       （教科書檔常見「同一段程式碼出現兩次，一次在 Markdown 區塊裡」。）
//
// 【詳細解釋 Explanation】
//   本檔把 this 的所有用途放進一個 RPG 隊伍系統：
//   【1. 鏈式強化（應用 3）】boostHp/boostAttack 都 return *this，
//        所以 warrior.boostHp(50).boostAttack(10).boostHp(30) 三次作用在同一個
//        warrior 上。關鍵是回傳型別 Hero&（有 &）—— 少了它會變成操作暫時副本。
//   【2. 身分比較（應用 2、5）】follow 與 heal 都用 this == &other 判斷
//        「對象是不是自己」。這比的是位址（身分）不是內容，正是自我操作防護。
//   【3. 回傳 *this 或 other（應用 4）】betterLeader 依綜合能力回傳較強者的
//        const 參考。此處綜合分 = maxHp + attack*3；強化後 warrior 的
//        530 勝過 mage 的 420，所以回傳的是 warrior（亞瑟）。
//   【4. leader_ == this（應用 1、6）】把自己的 this 存進 leader_ 表示自任隊長，
//        printStatus 再用 leader_ == this 判斷該印「(隊長)」還是「(跟隨:…)」。
//
// 【概念補充 Concept Deep Dive】
//   * this == &other 比的是位址（身分）不是內容 —— 見第 26 課 -10 與
//     第 29、33 課的自我賦值防護，是同一個 idiom。
//   * this 不佔物件空間，是呼叫成員函式時傳入的隱藏參數。
//   * ★ 本檔的執行程式碼刻意「不列印任何原始位址」（位址受 ASLR 影響、
//     每次執行都不同）。開頭 Markdown 講義裡出現的 0x7fffXXXX 只是示意用的
//     佔位符，不是實跑結果 —— 檔尾的【預期輸出】才是本機實跑、可重現的。
//
// 【注意事項 Pay Attention】
//   1. 鏈式呼叫（return *this）回傳型別必須是參考（Hero&），少一個 & 會操作副本。
//   2. betterLeader 回傳 const Hero&，只讀不改；不要回傳區域物件的參考。
//   3. this == &other/&target 比位址（身分），不是比內容。
//   4. 已修正開頭 Markdown 講義中兩處與程式實際輸出不符的舊「預期輸出」：
//      「陳信安 高於 對手」應為「是」（200>150），
//      「更適合當隊長」應為「亞瑟」（強化後 warrior 綜合分較高）。
//
// ═══════════════════════════════════════════════════════════════════════════
// 【面試題】this 指標的綜合應用
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. warrior.boostHp(50).boostAttack(10).boostHp(30) 為什麼能一直串下去？
//     答：因為 boostHp/boostAttack 的回傳型別都是 Hero&，並 return *this，
//         回傳的是 warrior 自己的參考，所以下一個 . 繼續作用在同一個 warrior 上。
//         若回傳型別少了 &（變成回傳值），每一環都會拷貝出新的暫時副本，
//         強化就作用在不同物件上、還多了拷貝成本。
//     追問：這裡的 *this 型別是什麼？
//         → Hero&（左值參考）。this 是 Hero* const，*this 解參考後就是 Hero&。
//
// 🔥 Q2. follow 與 heal 裡的 if (this == &other) 在防什麼？比的是什麼？
//     答：防「對自己做不合理的操作」——跟隨自己、或把自己當治療目標時
//         走不同分支。比的是「位址（身分）」，也就是「other 是不是就是我自己」，
//         不是比內容。這與拷貝/移動賦值運算子的自我賦值防護是同一個 idiom。
//     追問：如果另造一個屬性完全相同的英雄，this == &other 會成立嗎？
//         → 不會。內容相同但位址不同的兩個物件是不同的個體，
//           這個判斷仍是 false —— 而那正是我們要的。
//
// ⚠️ 陷阱. 「betterLeader 回傳的是 *this 或 other，回傳型別寫成 Hero 值也行。」
//     答：不行，會有懸空與拷貝兩個問題。它宣告成 const Hero&：
//         回傳的是「既有物件」的參考（warrior 或 mage），零拷貝、且它們在
//         呼叫端仍活著，所以安全。若改成回傳值 Hero，
//         每次都會拷貝一個新 Hero、語義也從「指出哪一個」變成「複製一份」。
//     為什麼會錯：把「回傳自身/他者」與「回傳一份副本」混為一談。
//         betterLeader 要回答的是「這兩個之中哪一個更適合」，
//         回傳參考才忠實表達「就是那一個」，不是另外做一個複製品。
// ═══════════════════════════════════════════════════════════════════════════
//
// 【LeetCode 實戰範例】—— 本檔從缺，理由如下
//   this 指標與鏈式呼叫是語言機制與類別介面設計，
//   LeetCode 判的是演算法的輸入輸出，評測系統只會建立一個實例並呼叫指定方法，
//   碰不到 return *this、自我操作防護這些情境。硬掛題號只會誤導，故從缺；
//   本檔以 RPG 隊伍系統這個綜合情境展示 this 的全部用途。
//
// =============================================================================
...
// 編譯: g++ -std=c++17 -Wall -Wextra -Wreorder "第 26 課：this 指標11.cpp" -o lesson26
//
// ── 輸出說明（讀之前先看這裡）────────────────────────────────────────────
// * 下面是真正編譯執行（Hero + main）的輸出，完全決定性：沒有位址、
//   沒有耗時、沒有執行緒（本機實測連跑 3 次逐位元組相同）。
//   開頭 Markdown 講義裡的 0x7fffXXXX 只是示意佔位符，不是這裡的實跑結果。
// * 「更適合當隊長：亞瑟」是正確結果：強化後 warrior 綜合分
//   maxHp+attack*3 = 380+150 = 530，勝過 mage 的 150+270 = 420。
//   （已修正開頭講義中誤植為「梅林」的舊輸出。）
// * 「(隊長)」「(跟隨:亞瑟)」由 printStatus 內的 leader_ == this 判定；
//   「亞瑟：不能跟隨自己」由 follow 內的 this == &other 攔下。
// * 「伊蓮 自我治療 +0」：healer.heal(healer,30) 觸發 this == &target，
//   且此時伊蓮滿血，實際治療量被夾成 0。
// ─────────────────────────────────────────────────────────────────────────
//
// === 預期輸出 ===
