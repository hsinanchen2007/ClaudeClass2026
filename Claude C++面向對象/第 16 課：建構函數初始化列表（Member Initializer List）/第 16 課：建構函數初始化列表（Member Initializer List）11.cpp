// =============================================================================
//  第 16 課：初始化列表 11  —  完整課文（講義全文 + 可執行的 RPG 武器系統）
// =============================================================================
//
// 【主題資訊 Information】
//   本檔性質：本課的**完整講義全文**。第 1～953 行是包在區塊註解裡的課文
//             （含大量說明用的程式碼片段，那些片段本身不參與編譯）；
//             第 955 行之後才是真正會被編譯的綜合範例：RPG 武器系統。
//   語法：  Class(參數列) : 成員1(初值1), 成員2(初值2) { 函數體 }
//   標準版本：初始化列表為 C++98；本檔的 Weapon 用到 NSDMI（C++11）
//             與帶預設參數的建構函數
//   複雜度：O(成員數)
//   標頭檔：<iostream>、<string>、<cmath>、<vector>
//
// 【詳細解釋 Explanation】
//
// 【1. 怎麼讀這個檔案】
//   課文部分（區塊註解內）從「什麼是初始化列表」一路講到綜合應用，
//   節次與本課其他檔案一一對應：
//       16.1  什麼是初始化列表        → 對應 1.cpp
//       16.2  賦值 vs 初始化的本質區別 → 對應 1.cpp、10.cpp
//       16.3  必須使用的四種情況       → 對應 2.cpp、3.cpp、4.cpp、5.cpp
//       16.4  初始化順序的陷阱         → 對應 6.cpp、7.cpp
//       16.5  在列表中使用運算式       → 對應 8.cpp
//       16.6  C++11 類別內預設值       → 對應 9.cpp
//   建議先跑過各個小檔累積直覺，再回來把課文從頭讀一遍收斂觀念。
//
// 【2. 檔案末端的 Weapon 為什麼是好的綜合範例】
//   它把本課四個重點同時用在一個類別上，而且每一個都有真實理由：
//     ● const int weaponId
//         武器的身分編號，鍛造出來就固定 → const 成員，只能用初始化列表。
//     ● Rarity rarity
//         Rarity 只有帶參建構函數（沒有預設建構函數）→ 必須在初始化列表
//         用 rarity(rarityName, rarityStars) 就地建構。
//     ● int enhanceLevel = 0;  bool isEquipped = false;
//         強化等級與裝備狀態有明確的初始預設值 → 用 NSDMI（C++11）一次寫好，
//         不必在每個建構函數重複。
//     ● double critRate = 0.05（建構函數的預設參數）
//         暴擊率大多數武器相同 → 用預設參數簡化呼叫端。
//   請特別注意最後兩者的分工：**NSDMI 表達「成員自己的預設狀態」，
//   預設參數表達「呼叫端可以少傳一個引數」**，兩者是不同層次的東西，
//   本範例剛好同時示範。
//
// 【3. Weapon 的初始化順序】
//   宣告順序是 weaponId → name → rarity → baseDamage → critRate
//   → enhanceLevel → isEquipped，初始化列表的書寫順序也完全一致，
//   所以不會有 -Wreorder 警告，讀起來也不會誤解。
//   這正是本課建議的寫法：**書寫順序 = 宣告順序**。
//
// 【4. 一個容易忽略的細節：const 成員讓 Weapon 不可指派】
//   因為 weaponId 是 const 成員，編譯器隱式生成的複製指派運算子被定義為
//   deleted。所以 Weapon 可以複製建構、可以 push_back 進 vector，
//   但不能寫 w1 = w2;，也不能對裝著 Weapon 的 vector 做 sort。
//   若這個類別日後真的需要排序，就得把 weaponId 的 const 拿掉，
//   改用 private + getter 來保護它。
//
// 【概念補充 Concept Deep Dive】
//
//   ● 課文中的程式碼片段不會被編譯
//     第 1～953 行整段是 /* ... */ 區塊註解，裡面的 #include、class、main
//     全部只是文字。所以本檔實際上只有一個 main（在檔案末端），
//     不會有「重複定義 main」的問題。
//
//   ● enhance() 為什麼用乘法而不是加法
//     baseDamage *= 1.1 表示每次強化增加 10%，是複利式成長。
//     連續強化三次的結果是 150 × 1.1³ = 199.65，而不是 150 + 45。
//     浮點數的顯示位數由 iostream 的預設精度決定（預設 6 位有效數字），
//     所以會看到 199.65 而不是 199.650000。
//
//   ● 為什麼 Rarity 刻意不給預設建構函數
//     這是**設計選擇**而非疏漏：一個稀有度如果沒有名稱與星級就沒有意義。
//     不提供預設建構函數，等於用型別系統保證「Rarity 一旦存在就是完整的」，
//     把可能的錯誤從執行期提前到編譯期。
//
// 【注意事項 Pay Attention】
//   1. 課文中的片段僅供閱讀，要執行請看檔案末端的完整程式。
//   2. const 成員會讓類別不可複製指派，設計時要一併考慮容器操作需求。
//   3. NSDMI 與建構函數預設參數是不同機制，不要混為一談。
//   4. 初始化列表的書寫順序請與宣告順序一致（本檔的 Weapon 即為範例）。
//
// ═══════════════════════════════════════════════════════════════════════════
// 【面試題】初始化列表的綜合應用
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. 一個類別同時有 const 成員、無預設建構函數的成員物件、
//        以及 C++11 類別內預設值，建構函數該怎麼寫？
//     答：const 成員與無預設建構函數的成員物件**必須**寫進初始化列表；
//         有 NSDMI 的成員則可以不寫，讓它沿用預設值，需要時再於初始化列表
//         覆蓋。書寫順序建議與宣告順序一致，避免 -Wreorder 與誤解。
//     追問：如果初始化列表沒寫某個沒有 NSDMI 的內建型別成員會怎樣？
//         → 它不會被初始化，值為不定值，讀取是未定義行為。
//           所以建議替所有內建型別成員補上 NSDMI。
//
// 🔥 Q2. 類別內預設值（NSDMI）和建構函數的預設參數，差別在哪？
//     答：NSDMI 是「成員自己的預設狀態」，寫在成員宣告處，與介面無關，
//         所有建構函數共用；預設參數是「呼叫端可以少傳一個引數」，
//         屬於函數介面的一部分，會參與重載決議。
//         本檔的 Weapon 同時用到兩者：enhanceLevel = 0 是 NSDMI，
//         critRate = 0.05 是預設參數。
//     追問：兩者可以同時用在同一個成員上嗎？
//         → 可以。成員有 NSDMI，建構函數又有對應的預設參數並寫進初始化列表，
//           此時初始化列表勝出，NSDMI 被忽略。但這樣容易混淆，
//           通常擇一即可。
//
// ⚠️ 陷阱. Weapon 有 const 成員只是讓 weaponId 不能改，
//          我還是可以把一堆 Weapon 放進 vector 然後照傷害排序，對嗎？
//     答：不行。const 成員會讓隱式的複製指派運算子被定義為 deleted，
//         而 std::sort 需要靠賦值搬移元素，所以編譯就會失敗。
//         push_back 沒問題（那是複製／移動建構），但 sort、resize、
//         erase 造成的元素前移都不行。
//     為什麼會錯：把 const 想成只影響那個欄位的讀寫。實際上它會連帶改變
//         整個類別自動生成的特殊成員函數，進而限制這個型別能用哪些
//         標準演算法——影響範圍遠比想像大。
// ═══════════════════════════════════════════════════════════════════════════
...
//
// 為什麼用到本主題：
//   建構函數 OrderedStream(int n) 拿到參數 n 之後，必須用它決定內部容器
//   的大小。寫成 : data_(n + 1) 是在初始化列表**直接建構出**指定大小的
//   vector；若改成在函數體內 data_.resize(n + 1)，就變成「先預設建構一個
//   空 vector，再 resize」的兩步——正是本課從頭到尾要避免的模式。
//   另外 ptr_ 用 NSDMI 給初值 1，示範「成員自己的預設狀態」寫在宣告處。
//
// 複雜度：每個元素最多被指標掃過一次，整體攤平 O(n)。
class OrderedStream {
private:
    vector<string> data_;   // 大小由建構參數決定 → 在初始化列表一次建好
    int ptr_ = 1;           // NSDMI：題目用 1-based，起點固定為 1

public:
    explicit OrderedStream(int n)
        : data_(static_cast<size_t>(n) + 1)   // 索引 0 不用，方便 1-based
    { }
...
// ============================================================
// 【日常實務範例】裝備欄位（inventory slot）：容量固定、內容可換
// ============================================================
// 情境：角色的背包格數在建立角色時就決定（例如新手 8 格），之後不再改變；
//       每一格可以放不同武器，也可以是空的。
// 重點：
//   ● capacity_ 是 const 成員 → 只能用初始化列表初始化
//   ● slotWeaponId_ 的大小由建構參數決定 → 同樣在初始化列表一次建好
//   ● 「這一格是空的」用 -1 這個哨兵值表示，並以 NSDMI 保證
//     count_ 一開始就是 0，不會是不定值
class Inventory {
private:
    const size_t capacity_;      // 背包格數，建立後不可變
    vector<int> slotWeaponId_;   // 每格放的武器 ID，-1 代表空格
    size_t count_ = 0;           // NSDMI：已放入的數量

public:
    explicit Inventory(size_t capacity)
        : capacity_(capacity)
        , slotWeaponId_(capacity, -1)   // 用參數一次建好，全部填 -1
    { }

    bool add(int weaponId) {
        if (count_ >= capacity_) return false;   // 背包已滿
        slotWeaponId_[count_] = weaponId;
        ++count_;
        return true;
    }

    void print() const {
        cout << "  背包 " << count_ << "/" << capacity_ << "：";
        for (size_t i = 0; i < capacity_; ++i) {
            if (slotWeaponId_[i] < 0) cout << "[空]";
            else                      cout << "[" << slotWeaponId_[i] << "]";
        }
        cout << endl;
    }
};
...
int main() {
    ...
    // ---- 日常實務：背包容量由建構參數決定 ----
    cout << "\n--- 日常實務：角色背包（const 容量 + 參數決定大小）---" << endl;
    Inventory bag(6);
    bag.print();
    bag.add(1001);   // 屠龍劍
    bag.add(1002);   // 風之弓
...
// 編譯: g++ -std=c++17 -Wall -Wextra "第 16 課：建構函數初始化列表（Member Initializer List）11.cpp" -o demo11
//
// ※ 說明（放在預期輸出標記之前）：
//   本檔第 1～1000 餘行是包在區塊註解內的課文，其中的程式碼片段不參與編譯；
//   真正被編譯執行的只有檔案末端的 RPG 武器系統、OrderedStream 與 Inventory。
//   本檔沒有任何未定義行為，輸出完全確定，且以 -Wall -Wextra 編譯無警告。
...
//
// --- 日常實務：角色背包（const 容量 + 參數決定大小）---
//   背包 0/6：[空][空][空][空][空][空]
//   背包 3/6：[1001][1002][1003][空][空][空]
