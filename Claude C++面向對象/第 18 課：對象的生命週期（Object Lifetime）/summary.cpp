// =============================================================================
//  summary.cpp  —  第 18 課總複習：物件的生命週期（Object Lifetime）
// =============================================================================
//
// 【主題資訊 Information】
//   標準版本：C++98 起有四種儲存期；C++11 起 static 區域變數的初始化
//             保證為執行緒安全；C++17 起 if/switch 支援初始化子句
//   標頭檔  ：<iostream>、<string>、<vector>
//   四種儲存期：
//     automatic 自動  ：區域變數，放 stack，離開 scope 即解構
//     static    靜態  ：全域／namespace／static 成員／函式內 static，活到程式結束
//     dynamic   動態  ：new 出來的，放 heap，由 delete 結束 —— 唯一需手動負責
//     thread    執行緒：thread_local（C++11，本檔未示範）
//
// 【詳細解釋 Explanation】
//
// 【1. 貫穿本課的主軸：作用域 ≠ 生命週期】
//   * 作用域 (scope)     ：名字在原始碼的哪些地方看得見 —— 編譯期概念。
//   * 生命週期 (lifetime)：物件在執行期存活多久 —— 執行期概念。
//   本課幾乎每個陷阱都源自把這兩者混為一談：
//     - static 區域變數：作用域是區域的，生命週期卻是靜態的。
//     - 懸空引用：名字還在（作用域內看得見），物件卻已經死了。
//   記住這句話就能解掉本課八成的題目：
//   「名字看得見，不代表物件還活著。」
//
// 【2. 解構順序：所有情境共用同一條規則 —— 反序（LIFO）】
//   同一個 scope 內、成員之間、靜態物件之間，一律是「建構順序的反序」。
//   理由是後建立者可能依賴先建立者，反序才能保證被依賴的一方活得更久。
//   本檔的巢狀 scope 示範就是把這條規則視覺化。
//
// 【3. for 迴圈裡的物件：每一輪都是全新的一次生死】
//   迴圈本體是一個 scope，因此在裡面宣告的物件每次迭代都完整建構一次、
//   解構一次。若建構成本高，應把物件提到迴圈外重複使用（本檔有對照示範）。
//   這是效能調校時第一個該檢查的地方。
//
// 【4. 靜態局部物件：延遲初始化 + 只做一次】
//   第一次執行到宣告那一行才建構，之後每次呼叫都沿用同一個物件，
//   程式結束時才解構。C++11 起標準保證這個初始化是執行緒安全的：
//   多個執行緒同時第一次抵達時，一個負責建構、其餘阻塞等待
//   （這是標準規定的阻塞，不是資料競爭，也不是未定義行為）。
//
// 【5. 全域物件初始化順序陷阱（Static Initialization Order Fiasco）】
//   跨 translation unit 的全域物件，建構順序是「未指定」的。
//   若 A.cpp 的全域物件在建構時使用 B.cpp 的全域物件，可能讀到尚未建構者。
//   標準解法就是本檔示範的 Meyers Singleton：把物件包進函式內的
//   static 區域變數，讓初始化順序由「呼叫關係」決定，因而變得確定。
//
// 【6. 暫時物件的壽命與 const 參考延長】
//   暫時物件預設在「完整運算式」結束（分號）時銷毀。
//   但若用 const 參考（或 C++11 起的右值參考）綁住它，
//   它的壽命會被延長到該參考的作用域結束。
//   注意：這個延長「不會傳遞」—— 綁到函式回傳的參考、
//   或把參考再存進成員，都不會延長，那正是懸空引用的常見來源。
//
// 【概念補充 Concept Deep Dive】
//   為什麼「返回區域物件的參考」危險，但「返回區域物件的值」安全？
//   前者回傳的是一個指向 stack 上已失效位置的別名；
//   後者則是把值搬到呼叫端的空間 —— 而且編譯器通常會做 NRVO，
//   直接在呼叫端就地建構，連一次複製都不做。
//   所以「安全」與「有效率」在這裡並不衝突：正確寫法同時也是最快的寫法。
//   本檔的 safe() 就是這個示範，危險版本則刻意保持註解、絕不執行。
//
//   記憶體位置：全域與 static 區域物件在資料區（.data/.bss），
//   區域物件在 stack，new 出來的在 heap。三者的存續規則彼此獨立。
//
// 【注意事項 Pay Attention】
// 1. 懸空引用／野指標是「未定義行為」，標準不保證任何特定結果
//    （可能看似正常、可能讀到垃圾、可能崩潰）。本檔一律保持註解、不執行，
//    也不印出任何無效指標或不確定的值。
// 2. vector 擴容會使既有的指標／參考／迭代器全部失效。
// 3. 容量成長倍率屬實作定義（libstdc++ 為 2 倍）。
// 4. 動態物件是唯一需要手動配對 new/delete 的；
//    現代 C++ 應優先使用 unique_ptr / make_unique。
...
// 【面試題】物件生命週期
// ───────────────────────────────────────────────────────────────────────────
// 🔥 Q1. 「作用域」和「生命週期」有什麼差別？請各舉一個兩者不一致的例子。
//     答：作用域是編譯期概念（名字在哪裡看得見），生命週期是執行期概念
//         （物件活多久）。兩個經典的不一致：
//         (a) 函式內的 static 區域變數 —— 作用域是區域的，但生命週期到程式結束；
//         (b) 懸空引用 —— 名字還在作用域內看得見，但它指向的物件已經死了。
//     追問：那 new 出來的物件，它的作用域是什麼？
//         → 這個問題本身就是陷阱：物件沒有作用域，只有名字才有。
//           new 回傳的是指標，有作用域的是那個「指標變數」；
//           被指向的物件則是動態儲存期，活到 delete 為止，兩者完全獨立。
//
// 🔥 Q2. 為什麼推薦用「函式內的 static 區域變數」來實作單例，而不是全域物件？
//     答：兩個理由。第一，避開 static initialization order fiasco ——
//         跨 TU 的全域物件建構順序未指定，而函式內 static 的初始化被延後到
//         第一次呼叫，順序由呼叫關係決定，因此是確定的。
//         第二，C++11 起這個初始化本身就保證執行緒安全，不必自己加鎖。
//         附帶好處是延遲初始化：沒用到就不會付出建構成本。
//     追問：這種單例的解構時機有沒有問題？
//         → 有。它在程式結束時才解構，且與其他靜態物件的相對順序仍是反序。
//           若某個靜態物件在自己的解構函數裡使用了這個單例，
//           而單例已先被解構，就會用到已死物件（destruction order fiasco）。
//           需要時可改用「刻意不解構」的洩漏式單例（回傳 new 出來的物件）。
//
// ⚠️ 陷阱 1. 「函式回傳區域物件的參考會編譯失敗，所以不用擔心。」
//     答：不會編譯失敗，只會得到一個警告（GCC/Clang 的
//         -Wreturn-local-addr）。程式照樣編譯成功、照樣執行，
//         而且很可能「看起來正常」—— 因為那塊 stack 記憶體暫時還沒被覆寫。
//         這是未定義行為，標準不保證任何結果。
//     為什麼會錯：把「編譯器有警告」誤記成「編譯器會擋下來」。
//         警告不是錯誤；而且一旦這段程式碼被 inline 或最佳化，
//         症狀還會隨編譯選項改變。本檔因此把危險版本保持註解，
//         只示範安全的替代寫法（回傳值）。
//
// ⚠️ 陷阱 2. 「用 const 參考接住暫時物件可以延長壽命，
//             所以函式回傳 const 參考指向暫時物件也沒問題。」
//     答：錯。壽命延長「不會傳遞」。const 參考延長壽命只發生在
//         「直接用暫時物件初始化一個具名的區域 const 參考」這一種情況。
//         把暫時物件綁到函式的回傳參考、或存進類別成員參考，
//         都不會延長 —— 暫時物件仍在該完整運算式結束時死亡，
//         留下懸空引用。
//     為什麼會錯：把「延長壽命」記成一條通用規則，
//         而不是一條有嚴格適用範圍的特例。
//         正確的心智模型是：這是為了讓
//         `const std::string& s = makeString();` 這種寫法能用，
//         而特別開的一個小口子，僅此而已。
// ═══════════════════════════════════════════════════════════════════════════
//
// -----------------------------------------------------------------------------
// 【LeetCode 實戰範例】—— 本檔不附
//   理由：本課主題是「物件何時誕生、何時死亡」，屬於語言的物件模型與
//   資源管理層面。LeetCode 評測的是演算法的輸入輸出正確性，
//   不會因為某個物件在第幾行被解構而有不同答案。
//   依規格「寧缺勿濫」從缺——本檔各節（延遲初始化的資源、連線池單例、
//   遊戲場景 Entity 管理）本身就是真實工程情境的示範。
// -----------------------------------------------------------------------------
...
void demoControlFlowLifetime() {
    // --- if 語句中的對象 ---
    cout << "\n  --- if 語句 ---" << endl;
    // if 的「初始化 + 條件」寫法：condition 的作用域僅限這個 if 敘述
    // （加上 (void)condition 是為了在 -Wall -Wextra 下不觸發 unused-variable 警告）
...
// 2. 重點對照：
//    (a) 全域物件的 [誕生] 出現在 main() 的第一行輸出之前；
//    (b) for 迴圈每一輪都有一組完整的 [+]／[-]，證明每次迭代都是新物件；
//    (c) 靜態局部物件只 [建構] 一次，第二、三次呼叫都直接沿用；
//    (d) 程式結束時的解構是建構順序的反序。
// 3. vector 那一節的擴容時機與複製次數取決於容量成長策略，
//    屬實作定義（libstdc++ 為 2 倍成長）。
// 4. 懸空引用／野指標的危險版本全程保持註解、未曾執行；
//    那些是未定義行為，標準不保證任何特定結果，
//    本檔只示範安全的替代寫法（回傳值、或改用動態配置）。
...
// =============================================
//
// 【1】四種存儲期（Storage Duration）
//   (全域物件在 main() 之前已建構，見上方輸出)
//
//   --- 第一次調用 demoStorageDuration() ---
//   [誕生] func 局部物件
//   [存活] func 局部物件 正在工作
//   [誕生] func 靜態局部物件
//   [存活] func 靜態局部物件 正在工作
...
//
// 【5】全域對象初始化順序陷阱（Static Initialization Order Fiasco）
//   問題：不同 .cpp 檔案中的全域對象初始化順序是未定義的！
//   解決方案：用函數包裝靜態局部對象
//
//   --- 透過函數包裝保證初始化順序 ---
//   Pool size =   [Config] 初始化: maxConnections = 100
//   [ConnectionPool] 初始化: poolSize = 100
// 100
//   (第二次調用 getPool()，不再重複初始化)
//
// 【6】臨時對象的生命週期
//   --- 情境 1：純臨時對象（語句結束即死亡）---
//   [+] 短命鬼
...
//
//   --- 陷阱總結 ---
//   1. 不要返回局部對象的引用 -> 懸空引用（Dangling Reference）
//   2. 不要返回局部對象的地址 -> 野指標（Dangling Pointer）
//   3. 安全做法：返回值（副本）或用 new 分配動態記憶體
//   [-] Data(42)
//
// 【8】vector 中的對象生命週期
//   push_back 會觸發拷貝，容量不夠時會重新分配記憶體
//   建議：大量插入前先 reserve() 預留空間
//
//   --- push_back(Element("A")) ---
//   [+] A
//   [拷貝+] A(副本)
...
// =============================================
// 本課重點回顧：
//   1. 四種存儲期：自動(棧)、靜態(全域/局部static)、動態(new/delete)
//   2. 自動存儲期：離開作用域自動解構，最常用最安全
//   3. 解構順序與建構順序相反（LIFO，像堆疊）
//   4. for 迴圈：每次迭代都是完整的建構->解構週期
//   5. 靜態局部對象：延遲初始化，只初始化一次，C++11 線程安全
//   6. 全域對象陷阱：不同編譯單元初始化順序未定義
//      解決：用函數包裝靜態局部對象
//   7. 臨時對象：語句結束時死亡，const& 可延長其生命
//   8. 懸空引用：不要返回局部對象的引用或指標
//   9. vector push_back 會觸發拷貝，容量不夠時重新分配
// =============================================
//   [解構] 共享資源
//   [死亡] func 靜態局部物件
//   [死亡] 全域物件
